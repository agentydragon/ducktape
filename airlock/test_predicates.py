"""Tests for airlock predicate system."""
...
from airlock.predicates import Approved, Denied, NeedsHumanDecision, call_predicate, load_predicate
...
def test_load_predicate_approved(tmp_path: Path):
    ...
    predicate_file.write_text(
        "from airlock.predicates import Approved\n"
...
def test_load_predicate_denied(tmp_path: Path):
    predicate_file = tmp_path / "predicate.py"
    predicate_file.write_text(
        "from airlock.predicates import Denied\n"
...
def test_load_predicate_conditional(tmp_path: Path):
    predicate_file = tmp_path / "predicate.py"
    predicate_file.write_text(
        "from airlock.predicates import Approved, NeedsHumanDecision\n"
