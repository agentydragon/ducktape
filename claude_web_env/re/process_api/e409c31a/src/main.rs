...
//!
//! New in e409c31a:
//!   - --firecracker-init flag: full VM init system (mount, networking, FUSE, etc.)
//!   - --listen-uds: Unix domain socket WebSocket listener
//!   - --listen-vsock-port: vsock WebSocket listener (Firecracker)
//!   - --control-vsock-port: vsock control server (Firecracker)
//!   - Static-pie linking (was dynamically linked)
//!   - tokio-vsock dependency for vsock support
//!   - jsonwebtoken dependency for JWT auth
//!   - libc dependency for raw syscalls in init path
//!
//! String refs at binary offset 0x28ea5c (CLI argument definitions):
...
//!   Plus all original flags from b0e4b2f4.
//!
//! String refs at binary offset 0x28dde0:
...
/// CLI arguments — updated for e409c31a.
/// Decompiled from serde visitor at binary offset 0x28ea5c.
/// New fields: firecracker_init, listen_uds, control_vsock_port, listen_vsock_port.
#[derive(Parser, Debug)]
...
/// Decompiled from 0x2089f0..0x2091fe  (2062 bytes) — unchanged from b0e4b2f4
...
/// Main entry point — updated for e409c31a.
///
/// Changes from b0e4b2f4:
/// - Runs firecracker_init::run_firecracker_init() before tokio if --firecracker-init
/// - addr is now Optional (can use --listen-uds or --listen-vsock-port instead)
/// - Supports UDS and vsock listeners for WebSocket connections
/// - Supports vsock control server via --control-vsock-port
/// - Control server gets mount_root_enabled flag when --firecracker-init is set
#[tokio::main]
async fn main() {
    env_logger::init();
