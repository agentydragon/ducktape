...
// antspaceDeployConfig holds Antspace deploy credentials parsed from an
// "antspace_deploy" auth config Token field.
// Binary: NewAuthContextWithSessionID, anonymous struct with 2 fields.
// JSON tags: url, auth_token.
type antspaceDeployConfig struct {
	URL       string `json:"url"`
...
//
// Binary struct layout (binary a6f96673):
//
...
//	offset 0x40: antspaceControlPlaneURL string (ptr+len)  ← new in a6f96673
//	offset 0x50: antspaceAuthToken string (ptr+len)        ← new in a6f96673
//	offset 0x60: supabaseProjectRef string (ptr+len)
//	offset 0x70: supabaseAnonKey string (ptr+len)
//	offset 0x80: supabaseDBPass string (ptr+len)
//	offset 0x90: supabasePAT string (ptr+len)
//	offset 0xa0: sessionID string (ptr+len)
...
type AuthContext struct {
	...
	antspaceControlPlaneURL string
	antspaceAuthToken       string
...
func NewAuthContextWithSessionID(
	...
	for _, config := range configs {
		...
		case "antspace_deploy":
			// Binary: NewAuthContextWithSessionID antspace_deploy case (new in a6f96673).
			// Token field contains a JSON-encoded struct with url and auth_token fields.
			// Logs "Configured Antspace deploy credentials" with type and token.
			if config.Token == "" {
				return nil, fmt.Errorf("antspace_deploy auth configuration missing required 'token' field")
			}
			var asCfg antspaceDeployConfig
			if err := json.Unmarshal([]byte(config.Token), &asCfg); err != nil {
				return nil, fmt.Errorf("failed to parse antspace_deploy auth configuration: %w", err)
			}
			ctx.antspaceControlPlaneURL = asCfg.URL
			ctx.antspaceAuthToken = asCfg.AuthToken
			if ctx.logger != nil {
				ctx.logger.Info("Configured Antspace deploy credentials",
					"type", config.Type,
					"token", config.Token,
				)
			}
...
// GetAntspaceControlPlaneURL returns the Antspace control plane URL.
// Binary: 0x8319c0 - context.go:197, reads offset 0x40/0x48.
func (a *AuthContext) GetAntspaceControlPlaneURL() string {
	return a.antspaceControlPlaneURL
}

// GetAntspaceAuthToken returns the Antspace auth token.
// Binary: 0x8319e0 - context.go:202, reads offset 0x50/0x58.
func (a *AuthContext) GetAntspaceAuthToken() string {
	return a.antspaceAuthToken
}
...
// Binary: context.go offset 0x80/0x88.
...
// Binary: context.go offset 0x90/0x98.
...
// Binary: checks supabaseProjectRef length (offset 0x68) != 0.
