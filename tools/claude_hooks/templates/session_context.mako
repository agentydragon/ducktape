...
Docker: ${container.status}, DOCKER_HOST=${container.socket_url}
...
Podman: ${container.status}, DOCKER_HOST=${container.socket_url}. Use fully qualified image names (docker.io/library/...)
...
pre-commit: hook environments installing in background (pid ${precommit.pid}). First `git commit` may block briefly on pre-commit's flock until done. Log: ${precommit.log_path}
% endif
