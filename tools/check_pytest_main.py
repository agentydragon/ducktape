...
def parse_build_file_for_target(build_file: Path, test_file: Path) -> dict | None:
    ...
    for match in re.finditer(py_test_pattern, content, re.DOTALL):
        ...
        if f'"{test_filename}"' in block or f'"{rel_path_str}"' in block:
            ...
            main_match = re.search(r'main\s*=\s*"([^"]+)"', block)
            if main_match:
...
def should_skip_file(file_path: Path) -> tuple[bool, str]:
    ...
    helper_patterns = [
        r"test_helpers?\.py$",
        r"test_utils?\.py$",
        r"testing/.*\.py$",  # testing utilities
    ]

    for pattern in helper_patterns:
        if re.search(pattern, str(file_path)):
            return True, f"test helper (matches {pattern})"
...
def check_file(file_path: Path) -> CheckResult:
    ...
    build_file = find_build_file(file_path)
    if build_file:
