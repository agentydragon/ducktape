...
- **Written:** 2026-07-13
- **Repository baseline:** `origin/devel` at `355921a37`; includes the FastMCP compatibility characterization in PR #3174
...
The implementation is now split into `RetryableRefreshOIDCProxy`, `DownstreamClientIdentityOIDCProxy`, and `ClientAuthorizationHookOIDCProxy`. The generic builder still composes all three so this preparatory refactor changes no runtime behavior or introduces a parallel state. The Haku adapter phase removes the raw-token hook and its callers atomically; downstream DCR identity restoration remains a focused shim while both Haku and Airlock consume that compatibility behavior.
...
4. **P2 — contain FastMCP customization in Haku without changing product UX.** Move the Haku authorization hook and all Haku-specific private FastMCP model/store access out of generic construction into one version-pinned Haku adapter wired to the current DCR-to-Operator mapping. It receives P1's verified principal, preserves FastMCP's validated client/redirect/resource/PKCE transaction, exposes the post-IdP checkpoint and opaque context lifecycle, and initially continues authorization automatically. Keep retryable refresh and downstream DCR-client-ID restoration as independently named shared compatibility shims because Airlock also consumes the latter; each product selects only the behavior it needs. Contract-test code persistence, token exchange, refresh, revocation, response loss, and retryable store failures against exactly FastMCP `3.4.4`.
...
4. **Complete:** PR #3174 pins the three former `ResilientOIDCProxy` compatibility facts; the follow-up behavior-neutral split gives refresh retry, downstream identity restoration, and the current Haku hook independent names and deletion paths.
...
Do not rebase or merge PR [#3122](https://github.com/agentydragon/ducktape/pull/3122). Its conflict-heavy rebase would mostly rewrite a monolithic branch whose central identity and lifecycle model is being discarded. Leave it open only while this plan is under review. Once this plan is accepted, close #3122 as superseded and link its retained commit as source material; build the phased successors from current `devel`. Closing the PR does not delete the commit. This plan PR itself does not modify or close #3122.
