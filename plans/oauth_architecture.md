...
PR [#3176](https://github.com/agentydragon/ducktape/pull/3176) split the implementation into `RetryableRefreshOIDCProxy`, `DownstreamClientIdentityOIDCProxy`, and `ClientAuthorizationHookOIDCProxy`. The generic builder still composes all three, so that preparatory refactor changed no runtime behavior and introduced no parallel state. P5 removes the raw-token hook and its callers when it activates the Haku adapter; downstream DCR identity restoration remains a focused shim while Airlock still consumes that compatibility behavior.
