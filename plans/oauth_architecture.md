...
haku/console/
  tool_call_service.py        # current actor-scoped tool-call application boundary
...
- PR [#3152](https://github.com/agentydragon/ducktape/pull/3152) made Haku MCP auth a `StaticMcpAuth | OAuthMcpAuth` union whose OAuth variant carries a required shared Postgres/Valkey store; P3 subsequently narrows Haku to the Postgres database it owns while generic consumers retain their existing storage choices.
...
5. **P3 — complete: live canonical Operator cutover.** Forward-only migration 0008 adds and activates `Operator`, `IdentityAnchor`, and `OidcIdentity`; preserves only downstream backend-token associations whose controller-fed Authentik user-id seed matches exactly; invalidates the complete old incoming agent-facing FastMCP registration/code/token/JTI/refresh store and every derived DCR-to-Operator link; drops ambiguous in-flight OAuth/ledger state; and replaces every remaining live subject/username authority with `operator_id` across browser sessions, Operator OAuth associations/flows, tool-call/event ownership, static-Agent resolution, and event routing. OAuth agents must authorize again after the cutover, with fresh local registration where applicable, so an old credential family cannot inherit a newly linked canonical Operator. Browser and MCP adapters construct issuer-scoped identities only after protocol verification, two configured issuers converge through one transaction-locked trust-domain anchor, and untrusted equal `sub` values fail closed. Active checks require the current trust domain, browser sessions additionally require a currently trusted issuer and an absolute one-hour reauthentication deadline, and disabled Operators are rejected at the static/OAuth MCP authentication boundary: before and after initial family issuance, access-token validation or transparent refresh, and explicit refresh rotation; outbound backend-token paths likewise check before persistence/return. Haku narrows its required OAuth persistence to the Postgres store it already owns, and validates the FastMCP URL names the exact same Postgres authority, allowing the database migration to invalidate the authority atomically; generic `mcp_infra` consumers retain Valkey support. The external deployment labels `operator_subject_env` / `operator_subject` remain stable only as startup seed names so old-image rollout ordering stays safe; they are never live request authority. P5 still replaces the temporary live DCR-client mapping and string Agent principal with canonical Agent/grant/binding records.
6. **P4 — complete: consolidate the existing tool-call authorization service without changing persistence.** `ToolCallApplicationService` now owns submission, Agent/Operator reads, policy evaluation, decisions, execution orchestration, polling, and event publication. HTTP and MCP are actor-resolving adapters; repository reads and locked transitions apply one exhaustive actor predicate. The two-Operator × two-Agent cross-product covers service, HTTP, MCP, event, decision, execution-token, and result paths. P4 deliberately changes no schema, DCR mapping, or persisted tool-call row format; P5 replaces those temporary identities once.
...
2. **Complete current-state hardening:** PR #3140 closed same-Operator cross-Agent reads; PR #3145 then replaced the duplicate caller/scope DTOs with a typed `OperatorActor | AgentActor`; P3 replaced every live Operator persistence key with canonical UUIDs without introducing a second authority. P5 replaces the remaining temporary string Agent principal/DCR mapping with canonical binding provenance.
...
6. **Complete:** treat Postgres/Valkey/Kubernetes Secrets as the accepted private storage boundary. PR #3143 removes lifecycle duck typing without adding encryption. PR #3152 made Haku's OAuth composition carry a non-optional shared store and nested it under the OAuth config; P3 narrows Haku itself to Postgres so migration 0008 can atomically invalidate the complete pre-cutover credential authority, while generic infrastructure keeps Valkey. Optionally evaluate FastMCP's standard storage wrapper in an independent later change; do not block Agent identity work on encryption.
...
1. P3 has added and activated local `Operator`, `IdentityAnchor`, and `OidcIdentity`. P5 atomically adds and activates `EnrollmentInteraction`, `Agent`, `AgentNameReservation`, `ClientSoftware`, `AuthorizationGrant`, `CredentialBinding`, `StaticCredential`, and `ToolCallPrincipal`, all with UUIDs and database invariants.
