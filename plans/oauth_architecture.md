...
The P2 off-production spike showed that Haku can compose the enrollment ceremony around FastMCP's public authorization path without patching or replacing its callback, transaction store, redirect validation, PKCE handling, or code/token issuance. A Haku-local adapter calls public `OAuthProxy.authorize()` with `AuthorizationParams`, temporally reserves the exact validated `(client_id, redirect_uri, S256 code_challenge)` tuple, and sends the browser through a Haku-owned enrollment interaction before following FastMCP's opaque upstream authorization URL. At downstream code exchange, public `AuthorizationCode` data provides the same tuple and P1's verified-principal resolver establishes the MCP-side Operator.

The sole accepted private compatibility seam remains P1's read/delete access to FastMCP's `_code_store`. The P2 adapter does not parse FastMCP state, access `_transaction_store`, intercept or rewrite the IdP callback, copy token issuance, or override route construction. It version-pins two protected compatibility hooks, `_extract_upstream_claims` and `_translate_scopes_from_idp`, to carry only opaque `grant_id` context and prevent scope broadening. A future public authorization-code inspection/consumption API would delete the remaining private seam; public typed token-context and scope-preservation hooks would delete the protected overrides. Ducktape will not currently make an upstream FastMCP contribution or wait for one.

FastMCP's current consent point remains pre-IdP. P2 uses that ordering deliberately: the Haku page authenticates its browser through Haku's own Operator session, collects the Agent name and explicit consent, and reserves the decision before forwarding the browser to Authentik. No Agent is created at that point. After FastMCP's untouched callback creates the downstream authorization code, token exchange resolves the verified MCP principal and requires its canonical Operator UUID to equal the browser session's canonical Operator UUID before creating the Agent and issuing grant.
...
MCP client            Browser               Haku/FastMCP             Authentik          Haku domain
    ...
    |                    |                         | reserve exact public tuple; create I ------>|
...
P2 passed the containment gate off production. The mounted full-flow contract target is `//haku/console:test_mcp_agent_enrollment_integration`; it passed a forced-fresh BuildBuddy RBE run (`34646f6d-25f0-4b75-aeb3-9568f893abed`, outer `f2111abb-2bc5-4e36-9f31-c8a7f91d5ceb`). The spike proves public authorize/`AuthorizationCode` tuple correlation, Haku session and interaction ownership, canonical browser/MCP Operator equality, opaque `grant_id` preservation through issue and refresh, first-`tools/call` activation, scope integrity, single-winner concurrent code exchange, and failure/reconciliation behavior. It intentionally adds no production wiring or live schema. P5's terminal contract still requires idempotent duplicate browser-POST results; the spike does not claim to prove that product behavior.

The production decision is **GO for the same Haku-local composition contract after P3's canonical Operator identity cutover is present**. It is not approval to transplant the spike wholesale or introduce an intermediate production authority. P5 must implement the aggregate and persistence model against the then-current code. In particular, `GrantCore.allowed_scopes` comes from the same provider-translated, client-facing effective scope set that FastMCP issues; raw upstream OIDC scope strings never become Haku authority. The implementation follows these containment rules:
...
- exercise authorize, deny, expiry, duplicate exact-tuple authorization, concurrent single-winner code exchange, refresh, response loss, retryable persistence failure, revocation, scope narrowing, and first-call activation; and
...
| `EnrollmentInteraction` | random `interaction_id UUID`, immutable verified browser `identity_id`, client-software reference, validated downstream transaction reference, exact requested scopes and redirect/client presentation snapshot, expiry, one-time browser-binding hash, `pending/allowed/exchanging/completed/denied/expired/failed`, and optional resulting `grant_id`; denial creates no grant                                                           |
...
Treat an OAuth Agent enrollment as one authorization aggregate and one locked Haku transaction, not as one denormalized database row. Browser approval creates only an interaction-owned name reservation. After the MCP principal resolves to the same canonical Operator, creating the Agent atomically promotes that reservation to the Agent and creates the `AuthorizationGrant` and `CredentialBinding`. The Agent owns its required name and, when introduced, its per-Agent auto-approval policy; the grant and binding own credential and token-family lifetime. Do not copy the name, owner, or policy onto OAuth records, and do not make a credential-lifetime row the canonical Agent.
...
EnrollmentInteraction
  ...
  pending -> denied
  pending -> expired
  pending/allowed/exchanging -> failed/expired
...
2. The interaction is bound server-side to the validated downstream transaction, exact client/scopes/redirect presentation, explicitly verified browser `OidcIdentity`, resolved Operator, expiry, and one-time browser/form binding.
...
4. allow remaining FastMCP records to expire by their existing TTLs or a later supported cleanup API instead of adding a private token-store seam; and
...
| Haku-local public composition adapter        | FastMCP plus one P1 private seam    | Desired synchronous flow                       | Medium per FastMCP upgrade   | Public tuple correlation; pinned `_code_store` and protected-hook contract tests                | P2 passed; implement after P3 under the containment gate           |
...
Now pause. Splitting route trust, authenticating provider-connect initiation, introducing the canonical Haku schema, and implementing the bounded Haku composition adapter are principled next phases, but they encode material design and rollout choices.
...
4. **P2 — complete off production: pass the Haku FastMCP adapter feasibility and contract gate.** The isolated mounted-flow spike proved that public `authorize()`/`AuthorizationParams` and public `AuthorizationCode` tuple correlation preserve FastMCP's validated client/redirect/resource/PKCE machinery without state parsing, callback interception, `_transaction_store`, copied issuance, or route overrides. P1 `_code_store` read/delete is the sole private seam; two protected hooks carry opaque `grant_id` context and enforce scope integrity. `//haku/console:test_mcp_agent_enrollment_integration` covers browser session/nonce/CSRF/Origin, canonical Operator equality, safe redirect-host presentation, naming, single-winner concurrent code exchange, code/token/refresh, response loss, retryable persistence failure, revocation, and first-call activation. The spike remains test-only: it does not wire production auth or live schema, and P5 still owns idempotent duplicate browser-POST behavior. Production is GO for this contract after P3, in P5's atomic cutover. Retryable refresh and downstream DCR-client-ID restoration remain independently named shared compatibility shims because Airlock also consumes the latter.
...
1. P2 proved the off-production composition contract. FastMCP validates and stores the downstream transaction through public `authorize()`; Haku temporally reserves the exact returned `(client_id, redirect_uri, S256 code_challenge)` tuple and runs pre-IdP consent in its own authenticated browser session; FastMCP's callback and issuance stay untouched; token exchange correlates public `AuthorizationCode` data and requires canonical Operator equality before Agent/grant creation. Do not install an auto-continue production adapter or write live Agent/grant/binding state before P5.
...
9. The decision is GO for the exact bounded adapter contract after P3, with quarantine as the fallback if a repin crosses the containment threshold. Do not proceed on the assumption that issue #4299 will solve it.
...
4. Key pending interactions by a random interaction ID and immutable verified browser identity plus public client/transaction tuple context; never by `client_id` and never by parsed FastMCP state.
...
| Grant lifecycle        | Interaction `pending -> allowed -> exchanging -> completed`, or denied/expired/failed; grant may be observed `issuing -> issued`, while `issuing/issued -> active` occurs only on first verified tool use; denial creates no grant; lost token response never activates; abandoned pre-exchange approval cleans its pending name, abandoned post-exchange issue cleans the draft Agent/name; reconnect uses predecessor CAS; refresh cannot activate/resurrect |
...
| Revocation             | Haku denial blocks the very next access and refresh use; another grant remains active; absent or failed individual-token cleanup does not weaken denial; remaining FastMCP records expire by existing TTL or a later supported cleanup API                                                                                                                                                                                                                     |
