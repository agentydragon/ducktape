...
| E    | DECIDED AGAINST               | The four-way split (`eager_reads` / `lazy_reads` / `eager_rebinds` / `lazy_rebinds`) reads more cleanly at use sites than a unified `BTreeSet<(BindingName, DepKind)>`, and the collectors are separate by design (lazy descends into function bodies, eager doesn't). The implicit "what if a name is in both" invariant is acceptably small.                                                                                                                                                                                        |
...
| G    | OBVIATED                      | Original motivation was to free F's wire-affecting renames. With the user accepting JSON wire-shape changes in F, the report-schema split is no longer on the critical path. Could still happen if the IR moves more — defer until then.                                                                                                                                                                                                                                                                                              |
