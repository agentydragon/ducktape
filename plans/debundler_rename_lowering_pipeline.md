...
| 3b   | DEFERRED | Realizability already runs over the spec-fixed partition before lowering; adding a second seal-time check on `plan.move_index` would duplicate the validation. Revisit if `MoveBinding` ever submits moves the spec partition didn't already cover                                    |
| 4a   | DONE     | `PlanRenameVisitor` in `lowering/lowering_execute.rs` is the unified Id-based executor for chunk-scope renames (renames + import-specifier `as` aliasing + `export { local }` public-name preservation + object-shorthand collapse, all one pass)                                     |
| 4b   | DEFERRED | `apply_moves` body-splitter — current `remaining_item_after_selection` driven by `binding_assignment` is semantically equivalent to consuming `plan.move_index` (Phase 8a tracking submits every entry). Substitution is pure plumbing                                                |
| 4c   | DEFERRED | Side-table rebuilds — tests pass; no observable issue with the current ad-hoc updates. Revisit if a new contributor needs to read a side table mid-pipeline                                                                                                                           |
| 4d   | DEFERRED | `LoweringOp { RewriteImportSpecifier, AddExport, ReorderHoists }` — no current contributor needs them; add on first use                                                                                                                                                               |
| 5    | DONE     | `submit_chunk_renames` replaces the inline validation loop in `lower.rs:167-238`; chunk-rename AST mutation flows through the plan-aware visitor                                                                                                                                      |
| 6    | DONE     | `submit_naturalize_renames` emits `(Id, Atom)` spec-driven + heuristic renames into the per-module plan with proper `SyntaxContext`; legacy `drop_target_collisions` + `RenameAndShorthandNaturalizer` retired                                                                        |
...
| 8b   | DEFERRED | `remaining_item_after_selection` (binding_assignment-driven body splitting) is semantically equivalent to consuming `plan.move_index` since 8a tracks every assignment. Substitution is pure plumbing; revisit if a contributor synthesizes a move that isn't in `binding_assignment` |
| 9    | DONE     | `plan_module_reference_needs` reverse-lookup retired; deferred naturalize application makes `body_facts` walk pre-rename bodies so the direct Id lookup hits                                                                                                                          |
| 10   | DONE     | `source_chunk_imports_for_moved_body` strips leading `./` from `info.src` before concatenation, producing canonical paths directly; defensive `normalize_relative_module_specifier` call dropped                                                                                      |
