...
## Stop mirroring a live Google access token into haku-sandbox

`cluster/k8s/agents/airlock/google-access-token-eso.yaml` puts a working Google
access token into `haku-sandbox` on a 1-minute refresh, where
...
rather than adding another agent namespace to it.
