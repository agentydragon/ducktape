...
| **OpenShell as the exec backend**                         | Not adopted here — but still in production elsewhere | Runs (**F13**), and the main `openclaw` gateway is on it today (`sandbox.mode: all`, `backend: openshell`, `mode: mirror`, three live sandboxes). Not adopted for `public-coder-agent` because it wedges silently and permanently from something ordinary we never identified, and its egress policy is per-process rather than per-pod (**F2**) — a `kubectl exec`-launched process bypasses it. Read this row as "we chose not to extend it", not "it was removed".                                                                     |
...
Note what `public-coder-agent` actually runs: `sandbox.mode: "off"`. There is no
execution split at all — exec runs in the harness container. So W2 is not blocked
by a bad synchronisation mechanism here; it was never attempted. The mirror below
is what the **main `openclaw` gateway** runs, and it is the comparison that makes
the cloud-worker path look worth costing.
...
the main gateway runs. Outbound it ships a git pack and the worker reconstructs a pinned shallow
