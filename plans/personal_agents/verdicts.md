...
| **OpenShell as the exec backend**                         | Ruled out for unattended use              | Runs (**F13**), but wedges silently and permanently from something ordinary that we never identified, and its egress policy is per-process rather than per-pod (**F2**) — so a `kubectl exec`-launched process bypasses it.                                                                                                                                                                                                                                                                                                               |
...
we use. Outbound it ships a git pack and the worker reconstructs a pinned shallow
