...
**The container is the trust boundary** (`haku/PLAN.md` → _Durable doctrine_): enforcement at
the credential / RBAC / network / proxy perimeter, never by in-agent permission rules — the
scanner runs `--dangerously-skip-permissions` *because* the perimeter is what limits it.
...
| 7 | `gmail-labeling` closure invariant | The **one sanctioned world-write**: labels under `haku/` only, enforced server-side before any Gmail call; the only `gmail.modify` credential stays server-side | <haku/gmail_labeling/SPEC.md> |
| 8 | base (image, read-only) vs. state (`haku-state`, Haku's only write) | Self-modification of Haku's contract is structurally a ducktape PR | `haku/PLAN.md` → _Durable doctrine_ |
...
| 10 | Console iframe containment: cross-origin isolation, `sandbox` without `allow-popups`/`allow-top-navigation`, console CSP `frame-src`, `openLink` scheme gate + operator-owned host whitelist + off-whitelist confirm | Agent UI cannot reach the launch bearer, open windows, or navigate the operator anywhere unvetted | <haku/console/plans/free_form_ui_iframe.md> |
| 11 | Capability tier: CSRF double-submit + server-side bearer + **top-layer `<dialog>` confirm**, audited in `haku-console` (a namespace Haku cannot read) | Privileged actions (`launch-routine`) require a genuine operator gesture on trusted chrome; agent UI can only *request* | <haku/console/README.md> |
...
| 13 | Audit trails | Every Haku proposal/decision is a git commit; LLM calls route through LiteLLM (attribution, budget, kill-switch) with Langfuse traces; capability invocations log in `haku-console` | `haku/PLAN.md` → _Durable doctrine_ |
...
| Silent subresource beacons (`fetch`/`<img>`/`sendBeacon`/WebSocket) to a third party | **Known gap (2026-07-01, tracked as a Haku item):** governed by the *haku-ui document's own* CSP, which Haku serves. Fix: `ResponseHeaderModifier` on the haku-ui `HTTPRoute` injecting a strict CSP (`connect-src 'self'`, `img-src 'self' data:`, …) — the same mechanism <cluster/k8s/authentik/app/httproute.yaml> already uses; the route is the only public door, so the header is operator-enforced. |
...
3. The console renders **no** Haku-authored content and holds no haku-state credential; the
   litmus test for console code: *does it hold a secret, perform a privileged action, or
   define the trust boundary?* If not, it belongs to Haku.
...
- **haku-ui response CSP not yet injected** (silent-beacon channel above) — Haku item
  `haku-ui-browser-exfil-csp-gap-2026`; fix is a one-filter route change.
