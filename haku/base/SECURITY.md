...
| 11  | Capability tier: CSRF double-submit + server-side bearer + **top-layer `<dialog>` confirm**, audited in `haku-console` (a namespace Haku cannot read)                                                                | Privileged actions (`launch-routine`) require a genuine operator gesture on trusted chrome; agent UI can only _request_                                                             | <haku/console/README.md>                                                                                             |
...
| Silent subresource beacons (`fetch`/`<img>`/`sendBeacon`/WebSocket) to a third party | **Known gap (2026-07-01, tracked as a Haku item):** governed by the _haku-ui document's own_ CSP, which Haku serves. Fix: `ResponseHeaderModifier` on the haku-ui `HTTPRoute` injecting a strict CSP (`connect-src 'self'`, `img-src 'self' data:`, …) — the same mechanism <cluster/k8s/authentik/app/httproute.yaml> already uses; the route is the only public door, so the header is operator-enforced.                                                                                                |
