...
the <claude_projection.py> `ClaudeProjector`. The runner (<runner.py>) hands it a launch and a
...
from haku.runtime.x.bridge.claude_options import EXECUTABLE_VARIABLE
from haku.runtime.x.bridge.claude_projection import ClaudeProjector
from haku.runtime.x.bridge.protocol import HarnessLaunch, Interrupt, PromptDispatch
from haku.runtime.x.bridge.session_api import ConsoleCommand, SessionApi
...
class ClaudeHarness:
    ...
    async def _initialize(self, session: SessionApi, stdin: StdinWriter) -> None:
        ...
        Bare: the launch argv already carries the system prompt and MCP wiring (<claude_options.py>),
