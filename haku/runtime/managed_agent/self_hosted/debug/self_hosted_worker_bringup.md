...
| 5   | git clone `CONNECT tunnel failed, 502`                                       | GitHub is blocked by the `haku-egress-proxy` egress allowlist                                                                                                                  | clone ducktape from the in-cluster Forgejo mirror instead                                                                                                                                                                                     |
...
| 9   | sessions stall at `idle` (claim works, results never post)                   | `haku-egress-proxy` TLS-intercepts and breaks the worker's long-lived HTTP/2 session stream to `api.anthropic.com` (claim = short poll, works; session-drive = stream, breaks) | mitmproxy `ignore_hosts api\.anthropic\.com` (raw TCP passthrough; egress still flows through the proxy). Worker trusts the real cert — its CA bundle is system roots + the mitmproxy CA. `cluster/k8s/agents/haku-egress-proxy/deployment.yaml` |
