...
harness-invariant, so Claude's projector (<claude/projection.py>) and Codex's
(<codex/projection.py>) share them rather than each minting its own — as do the frame-span and
...
from haku.runner.neutral_operations import FrameRange, Operation
