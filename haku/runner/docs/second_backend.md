...
`ClaudeHarness` (<../claude/harness.py>) and `CodexHarness` (<../codex/harness.py>) are parallel at
...
- each projects its own native frames to the neutral operations of <../neutral_operations.py>
  (<../claude/projection.py>, <../codex/projection.py>) and journals them through `SessionApi`,
...
- **`thread/start` params travel in the launch environment.** The runner owns `thread/start` now, so
  ...
  `CodexThread` ride the launch environment under `codex.options`' keys (<../codex/options.py>);
