...
| Principal axis       | `GrantPrincipalKind` {`AGENT`, `SESSION`, `ACCESS_PROFILE`}                                          | `create_grant` lets an Agent request an Agent, exact-session, or any access-profile principal; profile targets are not caller-scoped, and the Operator may approve or deny the request                                                                    |
