...
| Principal axis       | `GrantPrincipalKind` {`AGENT`, `SESSION`, `ACCESS_PROFILE`}                                          | `create_grant` is Agent-requestable for an Agent, exact session, or the caller's own access profile; `require_applicable_grant_principal` enforces that profile scope, and Operator approval remains required                                             |
