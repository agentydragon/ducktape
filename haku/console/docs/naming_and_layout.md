...
| Grant principal           | `GrantPrincipal` = `AgentGrantPrincipal \| SessionGrantPrincipal \| AccessProfileGrantPrincipal`                                                   | `grants/principal.py`                                           | Pydantic + `principal_*` columns | stays — durable stored selector                                                                              |
...
| Principal axis       | `GrantPrincipalKind` {`AGENT`, `SESSION`, `ACCESS_PROFILE`}                                          | `create_grant` accepts `self` (the current session, or current Agent when no session is active) or any explicit valid Agent, live-session, or access-profile principal; the Operator may approve or deny the request                                      |
