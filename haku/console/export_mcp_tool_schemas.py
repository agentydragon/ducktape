...
from haku.console.tools.http import HttpToolsService
...
def build_schema_servers() -> dict[str, FastMCP]:
    ...
    # launcher/broker respectively, haku_index needs an inert searcher, and kubernetes/http need inert
    ...
    servers = {
        ...
        for server_id, registration in build_in_process_servers(
            InProcessServerDependencies(
                ...
                http=HttpToolsService(grants=dependency),
            )
        ).items()
    }
