...
| `mcp_approval.py`                  | Operator-browser FastAPI adapter plus the current Postgres tool-call repository and `McpServerClient` — the one client for the configured MCP servers, which both executes tool calls and reflects catalogs. It does not own agent admission or lifecycle orchestration. |
...
| `mcp_config.py`                    | Connected-MCP-server catalog plus in-process/remote transport and static bearer resolution, shared by the application service, `McpServerClient`, and operator OAuth linkage.                                                                                            |
