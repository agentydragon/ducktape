...
_K8S_SPEC = kubernetes_models.GrantSpec(
    scope=kubernetes_models.NamespacesGrantScope(namespaces=frozenset({"demo"})),
    rules=(
        kubernetes_models.Rule(api_groups=frozenset({""}), resources=frozenset({"pods"}), verbs=frozenset({"get"})),
    ),
)
_K8S_OTHER_SPEC = kubernetes_models.GrantSpec(
    scope=kubernetes_models.NamespacesGrantScope(namespaces=frozenset({"other"})),
    rules=(
        kubernetes_models.Rule(
            api_groups=frozenset({"apps"}), resources=frozenset({"deployments"}), verbs=frozenset({"patch"})
        ),
    ),
)
...
def test_list_grants_resolves_self_and_named_subjects(console: _Console) -> None:
    ...
    async def exercise() -> None:
        ...
        assert [item.source.id for item in named if isinstance(item.source, DatabaseGrantSource)] == [
            view.grant.grant_id
        ]
