...
class PostgresMcpOperatorOAuthStore:
    ...
    async def complete_callback(
        ...
        async with self._sessions.begin() as session:
            ...
            if (row := await session.get(McpOperatorOAuthFlow, state)) is None:
...
async def _operator_username(request: Request) -> str:
    ...
    return await operator_auth.operator_username(request) or "operator"
...
async def _refresh_operator_oauth_token(token_client: _OperatorOAuthTokenClient, refresh_token: str) -> OAuthToken:
    ...
    if response.status_code >= 500:
        ...
        raise RefreshError(
            ...
            kind=RefreshFailureKind.UPSTREAM,
            action=RefreshFailureAction.RETRYING,
        )
    try:
        ...
        raise RefreshError(
            str(error),
            kind=(
                RefreshFailureKind.OAUTH_REJECTED if not error.invalid_response else RefreshFailureKind.INVALID_RESPONSE
            ),
            action=(
                RefreshFailureAction.RECONNECT
                if error.oauth_error == "invalid_grant"
                else RefreshFailureAction.OPERATOR_ACTION
            ),
        ) from error
    ...
    *,
    ...
    try:
        status = await oauth_store.complete_callback(
            state=state, code=code, operator_id=operator_id, username=await _operator_username(request)
        )
