...
async def _one_failed_turn(session_store, session_wakes, operator_id, *, unusable: bool) -> UUID:
    ...
    runtimes = RuntimeRegistry({HarnessKind.CLAUDE_CODE: _GivingUpRuntimeAdapter(unusable=unusable)})
...
async def test_generic_turn_loop_is_opaque_to_a_discriminator_free_harness(
    ...
    runtimes = RuntimeRegistry({HarnessKind.CLAUDE_CODE: _UnrelatedRuntimeAdapter()})
    service = SessionService(runtimes, session_store, session_wakes)
    client = _FakeCli(
...
async def _folded(session_store: Store, session_id: UUID, turn_id: UUID, *payloads: dict[str, Any]) -> None:
