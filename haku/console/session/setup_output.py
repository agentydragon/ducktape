"""The one row the console authors into the `session_frames` log out of a runner protocol frame."""
...
SETUP_OUTPUT_KIND = SessionFrameKind.SETUP_OUTPUT


def setup_output_frame(text: str) -> dict[str, Any]:
    """One line the sandbox printed, as a rollout row.

    **Console-authored, and it says so with its discriminator.** The runner protocol's own frame is
