...
describe("hostexecResultPreviews", () => {
  it("renders a successful exit, in both variants", () => {
    ...
    for (const variant of ["compact", "detailed"] as const) {
      expect(renderResultPreview(hostexecResultPreviews.hostexec_run, result, variant)).not.toBeNull();
    }
  });
  ...
  it("renders a nonzero exit with stderr", () => {
    ...
    expect(renderResultPreview(hostexecResultPreviews.hostexec_run, result, "detailed")).not.toBeNull();
  });
  ...
  it("returns null when the payload doesn't match the result schema", () => {
    expect(
      renderResultPreview(hostexecResultPreviews.hostexec_run, { exit: { kind: "bogus" } }, "detailed")
    ).toBeNull();
  });
});
