...
// and `hostexec/requests.tsx`'s rendering of `cmd`, which hostexecd runs verbatim as `bash -c
// cmd`). `@codemirror/legacy-modes` ships CodeMirror 5's classic shell mode as a `StreamParser`;
// `StreamLanguage.define` (from `@codemirror/language`, already a project dependency) wraps it
// into a real CodeMirror 6 `Language` extension.
