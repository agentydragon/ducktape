 ...
 * that is what the operator came for, so the narration collapses out of its way. The transcript is
 * the conversation's rather than the session's, so its emptiness is the caller's to say.
 */
export function bootstrapNarration(
  ...
  return {
    ...
    startsExpanded: session.status === "provisioning" || transcriptEmpty,
  };
}
