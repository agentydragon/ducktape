...
- `main.tsx` → `app.tsx` (the page: tiered open items + global feedback box) →
  `task.tsx` (one item card with its action toggles and a per-item feedback box).
- `feedback.tsx` — shared feedback form (global note + per-item, the latter tagged
  with the item id); shows an in-flight spinner while the commit-push lands.
