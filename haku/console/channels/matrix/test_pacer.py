...
from haku.console.channels.matrix.pacer import MAX_QUEUED_SENDS, RoomPacer, Send
...
async def test_a_required_send_reports_failure_without_stopping_the_queue() -> None:
    ...
    async with pacer.run():
        with pytest.raises(Error, match="M_FORBIDDEN"):
...
async def test_a_429_is_believed_over_our_own_accounting() -> None:
    ...
    async def limited() -> None:
        raise Error("M_LIMIT_EXCEEDED: slow down", retry_after_ms=40)
