...
`assistant` frame; the rest stayed NULL, and NULL there means both "nobody has looked" and "looked
and could not tell". This adds the column that separates them, so the recovery's outcome is stored
beside each row.
...
knows. Startup applies migrations before serving and the Deployment rolls at `maxUnavailable: 0`, so
a fold over the frame log inside `upgrade()` would hold every replacement replica out of Ready for
its duration. The scan is an operator-invoked path instead.

Additive and safe for the length of a roll: a replica on the previous image neither selects nor
writes this column, and both constraints are satisfied by every existing row.
