...
that `chunks.chunk_no` was before 0037 dropped it.
...
search and chat sweep error out. Accepted because the cost is bounded and small: the tables are
derived state rebuilt from `claude_chat_messages` on the next sweep, which retries on its own
tick.
