...
is `authored` for the same reason a lease change is — it has crossed no wire when it is accepted,
and `next_prompt` hands it to the CLI later — so its kind is an `AuthoredEventKind` (`chat_models`).

**Additive, and safe for the length of a roll.** A widened CHECK forbids nothing the previous image
writes, and that image never *reads* one of these rows: the two queries it makes against this table
are the transcript's tool-call view, which filters to
`tool_call_started`/`tool_call_completed` in SQL, and `reprojection`'s per-turn read, which selects
by `turn_id` — and a prompt row names no turn. Both matter, because `TextBackedStrEnumUnionColumn`
