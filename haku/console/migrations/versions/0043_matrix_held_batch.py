...
inserts this table and keeps acknowledging at enqueue, and only one replica syncs at a time (the
`MXSY` advisory lock), so the two behaviours never interleave within a batch. The one skew — an old
leader running past a row the new image wrote — costs re-delivery, which the system is built to be
safe against where a skip is the one it cannot survive, and it needs leadership to move
new → old → new, which a `maxUnavailable: 0` roll does not do.
