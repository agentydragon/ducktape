"""0073 deletes the prompt rows that name no origin, then makes new ones impossible.
...
def _session(conn: Connection) -> UUID:
    ...
    # `sessions.conversation_id` is NOT NULL from `0072`, which `_BEFORE` already is.
    conn.execute(
        text(
            """
