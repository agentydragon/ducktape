...
@pytest.fixture
def following(
    session_store: Store, chat_service: SessionService, conversation_wakes: ConversationWakes
) -> ConversationFollow:
...
async def _exchange(session_store: Store, operator_id: UUID, session_id: UUID, prompt: str, answer: str) -> None:
...
async def test_a_follow_opens_with_the_conversation_whole(
    following: ConversationFollow, session_store: Store, operator_id: UUID
) -> None:
...
async def test_a_change_landing_during_the_snapshot_is_carried_by_the_update_after_it(
    following: ConversationFollow, session_store: Store, operator_id: UUID, monkeypatch: pytest.MonkeyPatch
) -> None:
