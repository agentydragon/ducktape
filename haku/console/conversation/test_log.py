"""What a conversation event becomes as a row: which columns carry it, and what is left in the body."""
...
def stored(event: ConversationEvent) -> ConversationEventRow:
    ...
    together.
    """
    said = conversation_events.stored(event)
...
def test_the_provenance_arm_follows_from_whether_frames_were_named() -> None:
    ...
    authored = log.item_row(
        ConversationEventKind.ITEM_OPENED,
        conversation_event.PromptOpened(origin=SPA_ORIGIN),
        conversation_id=CONVERSATION_ID,
        event_seq=1,
        item_id=ITEM_ID,
        session_id=SESSION_ID,
...
def test_a_prompt_is_conversation_on_the_authored_arm() -> None:
    ...
    assert (asked.kind, asked.provenance) == (ConversationEventKind.ITEM_OPENED, EventProvenance.AUTHORED)
...
def test_a_room_prompt_names_the_room_as_well_as_the_events() -> None:
    ...
    assert conversation_event.PromptOpened(origin=origin).model_dump(mode="json")["origin"] == {
        "kind": "matrix",
...
def test_a_prompt_body_without_an_origin_is_rejected() -> None:
    ...
    with pytest.raises(ValidationError):
        conversation_event.PromptOpened.model_validate({"item_type": "prompt"})
...
def test_a_fact_the_console_authored_names_no_item_and_no_frames() -> None:
    ...
    taken = log.authored(
        conversation_event.SessionAdopted(previous_holder="haku-console-a", holder="haku-console-b"),
        conversation_id=CONVERSATION_ID,
        event_seq=3,
        session_id=SESSION_ID,
        turn_id=None,
...
def test_the_kind_of_an_authored_row_follows_from_its_body() -> None:
    ...
    lapsed = log.authored(
        conversation_event.LeaseExpired(reason=LeaseExpiryReason.UNADOPTED, last_holder=None),
        conversation_id=CONVERSATION_ID,
        event_seq=4,
        session_id=SESSION_ID,
        turn_id=None,
...
def test_every_kind_the_column_may_hold_can_be_read_back() -> None:
    ...
    unreadable = [
        ...
        if log.body_of(ConversationEventRow(kind=kind, body=body, created_at=NOW, provenance=EventProvenance.AUTHORED))
        is None
    ]
...
def test_a_row_of_a_kind_this_release_has_no_words_for_reads_as_one() -> None:
    ...
    assert log.body_of(row) == conversation_event.UnknownEventBody(
...
def test_a_body_carrying_a_field_this_release_does_not_know_still_reads() -> None:
    ...
    assert log.body_of(row) == conversation_event.LeaseExpired(reason=LeaseExpiryReason.UNADOPTED, last_holder=None)
...
def test_the_one_event_with_a_durable_home_elsewhere_gets_no_row() -> None:
    ...
    assert conversation_events.stored(TurnCompleted(end=TurnAnswered(), provenance=WHERE)) is None
