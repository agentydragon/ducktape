...
| `x/claude_code/`, `x/codex_app_server/` | **One CLI harness each.** Named for the product whose binary they launch, not for the model. Codex is launch-only — its native client + projection moved runner-ward (#4667); Claude's console client + projection remain, deletion-scheduled with the v3 turn loop. |
