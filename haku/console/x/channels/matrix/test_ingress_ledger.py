...
async def test_an_event_is_carried_from_the_moment_its_prompt_commits(
    ...
    await session_store.enqueue_prompt(operator_id, session_id, "hi", _from_room("$a"), ledger.carrying(("$a",)))
...
async def test_a_refused_prompt_carries_nothing(
    ...
    with pytest.raises(PromptRefusedError):
        await session_store.enqueue_prompt(
            operator_id, session_id, "and this", _from_room("$b"), ledger.carrying(("$b",))
        )
...
async def test_a_prompt_its_session_never_claimed_is_taken_by_the_replacement(
    ...
    await session_store.enqueue_prompt(operator_id, first, "answer me", _from_room("$a"), ledger.carrying(("$a",)))
...
async def test_re_recording_an_event_moves_it_to_the_prompt_now_answering_for_it(
    ...
    await session_store.enqueue_prompt(operator_id, session_id, "hi again", _from_room("$a"), ledger.carrying(("$a",)))
