...
from haku.console.x.conftest import age_lease, lease_of, provisioned, queued_for_the_room
...
async def test_a_created_session_holds_no_sandbox_and_no_credential_for_one(
    chat_store, operator_id, migrated_sessions
) -> None:
    """`create` writes the row and stops, which is the whole of what an idle room costs.

    Its lease is already spent rather than granted far into the future: nothing holds this session,
    and `LEASED_SESSION_STATUSES` is what keeps the sweep off it (below).
    """
    ...
    assert record.lease_expires_at <= datetime.now(UTC), "an idle session holds a lease nobody is keeping"


async def test_an_idle_session_admits_no_runner_at_all(chat_store, operator_id, migrated_sessions) -> None:
    """No token was minted for it, so the verifier on the row matches no bearer that exists.

    That is what makes an unallocated session unreachable without a second guard in
    `authenticate_bridge` saying so — including against the token its predecessor allocation held,
    since `allocate` overwrites rather than reuses.
    """
    view = await chat_store.create(operator_id, SpaSession())
    async with migrated_sessions() as db:
        record = await db.get(Session, view.session_id)
    ...
    assert await chat_store.authenticate_bridge(view.session_id, "any-bearer") == BridgeAuthentication.REJECTED
    assert len(record.bridge_token_fingerprint) == len(SessionStore._fingerprint("shaped like a digest"))


async def test_allocation_mints_the_credential_and_only_one_caller_wins(chat_store, operator_id) -> None:
    """The transition is what decides who creates the claim, so a race cannot make two sandboxes."""
    view = await chat_store.create(operator_id, SpaSession())
    ...
    assert await chat_store.allocate(view.session_id) is None, "a second allocation would be a second sandbox"


async def test_allocation_clears_the_marker_that_says_this_session_s_claim_is_gone(
    chat_store, operator_id, migrated_sessions
) -> None:
    """`claim_cleaned_at` is a statement about a claim, and allocation is what gives it a new one.

    A stamped idle row is what a replica on an image that reads `idle` as ended leaves behind for
    the length of a roll; carrying that forward would take the session out of the cleanup sweep
    that has to delete the sandbox this allocation is about to create.
    """
    view = await chat_store.create(operator_id, SpaSession())
    ...
    async with migrated_sessions() as db:
        record = await db.get(Session, view.session_id)
    assert record is not None
    assert record.claim_cleaned_at is None


async def test_the_sweep_leaves_an_idle_session_alone_however_old_its_lease_is(
    chat_store, operator_id, migrated_sessions
) -> None:
    """What the open/leased split exists for: worth keeping is not the same as owing a beat.

    An idle session is created with a lease already in the past, so a sweep reading the open set
    would fail every one of them `ADOPTION_GRACE` later — a room would lose its session for the
    crime of nobody having spoken in it.
    """
    idle = await chat_store.create(operator_id, SpaSession())
    leased, _ = await provisioned(chat_store, operator_id, SpaSession())
    for session in (idle, leased):
        await age_lease(migrated_sessions, session.session_id, seconds_ago=int(ADOPTION_GRACE.total_seconds()) + 60)

    assert await chat_store.expire_stale_leases() == 1

    assert await chat_store.status(idle.session_id) == SessionStatus.IDLE
    assert await chat_store.status(leased.session_id) == SessionStatus.FAILED


async def test_a_prompt_is_admitted_by_a_session_with_no_sandbox(chat_store, operator_id) -> None:
    """Admission on `idle` is what turns a message into the request for a sandbox.

    Refusing would reject the room's first message after quiet, leaving it with nothing that could
    ever ask for one (<README.md> § An idle session).
    """
    view = await chat_store.create(operator_id, SpaSession())
...
async def test_the_cleanup_sweep_offers_ended_sessions_until_their_claim_is_recorded_gone(
    ...
    live, _ = await provisioned(chat_store, operator_id, SpaSession())
    swept, _ = await provisioned(chat_store, operator_id, SpaSession())
    cleaned, _ = await provisioned(chat_store, operator_id, SpaSession())
...
async def test_the_rollout_reads_back_in_wire_order_with_a_keyset_cursor(chat_store, operator_id) -> None:
    ...
    session, _ = await provisioned(chat_store, operator_id, SpaSession())
...
async def test_the_resume_cursor_is_the_highest_number_a_runner_gave_this_session(chat_store, operator_id) -> None:
    ...
    """
    session, _ = await provisioned(chat_store, operator_id, SpaSession())
    other, _ = await provisioned(chat_store, operator_id, SpaSession())
...
async def test_two_sessions_may_hold_the_same_agent_id(chat_store, operator_id) -> None:
    ...
    mine, _ = await provisioned(chat_store, operator_id, SpaSession())
    theirs, _ = await provisioned(chat_store, operator_id, SpaSession())
    frame = assistant(message_id="msg_01abc")
...
async def test_sessions_come_back_newest_first_with_the_channels_holding_their_thread(
    ...
    await provisioned(chat_store, operator_id, SpaSession())
    matrix, _ = await provisioned(chat_store, operator_id, MatrixSession(room_id="!room:example.org"))
...
async def test_a_session_created_between_two_pages_cannot_shift_what_the_second_one_holds(
    ...
    older, _ = await provisioned(chat_store, operator_id, SpaSession())
    newer, _ = await provisioned(chat_store, operator_id, SpaSession())
...
async def test_two_sessions_created_in_one_instant_are_paged_exactly_once_each(
    ...
    first, _ = await provisioned(chat_store, operator_id, SpaSession())
    second, _ = await provisioned(chat_store, operator_id, SpaSession())
...
async def test_operator_conversation_read_surface_keeps_inventory_and_transcript_separate(
    ...
    """
    await provisioned(chat_store, operator_id, SpaSession())
    matrix, matrix_token = await provisioned(chat_store, operator_id, MatrixSession(room_id=ROOM))
...
async def test_a_session_records_the_surface_it_was_created_for(chat_store, migrated_sessions, operator_id) -> None:
    ...
    spa, _ = await provisioned(chat_store, operator_id, SpaSession())
    matrix, _ = await provisioned(chat_store, operator_id, MatrixSession(room_id="!room:allegedly.works"))
...
async def test_a_session_opens_its_own_conversation_unless_it_is_given_one(
    ...
    first = await chat_store.create(operator_id, SpaSession())
    second = await chat_store.create(operator_id, SpaSession())
    ...
    continued = await chat_store.create(operator_id, SpaSession(), conversation_id=opened)
...
async def test_an_idle_session_is_kept_rather_than_cleaned_up(chat_store, operator_id) -> None:
    """Holding no sandbox is not an ending: the session is still the room's, so the claim sweep
    leaves it alone rather than tidying up after a sandbox it never had."""
    view = await chat_store.create(operator_id, SpaSession())
...
async def test_shutdown_hands_back_every_lease_this_replica_holds(chat_store, migrated_sessions, operator_id) -> None:
    ...
    held = [await provisioned(chat_store, operator_id, SpaSession()) for _ in range(2)]
