...
3. Across a roll neither is recovered. The frame is in `claude_chat_frames`, so the runner's replay
...
from haku.console.x.claude_chat import ASSISTANT_FRAME_KIND, ClaudeChatStore
...
class Deployment:
    ...
    async def _ready(self, session_id: UUID) -> bool:
        return await self._store.status(session_id) == ChatSessionStatus.READY
