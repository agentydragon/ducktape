...
class SessionService:
    ...
    async def handle_journal_runner(self, websocket: WebSocket, session_id: UUID, bearer: str) -> None:
        ...
        # The belt-and-braces generation read at admission (the peering itself is image-carried).
        # `None` means the switch's migration has not run on the database this replica sees — a
        # transient deploy-ordering state the runner should retry through. A generation other than
        # this build's is a peer that must not serve this session; the runner finds a matching
        # replica or releases the sandbox.
        active = await self._store.active_generation()
        if active is None:
            await websocket.send_denial_response(
                Response(status_code=503, content=b"runtime transport generation is not yet readable on this replica")
            )
            return
