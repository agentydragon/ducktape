...
async def test_a_real_runner_finishes_a_turn_the_console_that_started_it_never_saw_the_end_of(
    ...
    async def finished_turns() -> list[str | None]:
        turns = await chat_store.list_turns(str(session_id), limit=10)
    ...
    try:
        ...
        [in_flight] = [turn for turn in await chat_store.list_turns(str(session_id), limit=10) if turn.ended_at is None]
    ...
    turns = sorted(await chat_store.list_turns(str(session_id), limit=10), key=lambda turn: turn.started_at)
    ...
    narration = await chat_store.read_frames(str(session_id), after_seq=None, limit=10, kinds=[SETUP_OUTPUT_KIND])
