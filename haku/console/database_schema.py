...
class KubernetesGrantRow(GrantEnvelopeColumns, Base):
    ...
    __table_args__ = (
        *grant_envelope_table_args("kubernetes_grants"),
        CheckConstraint(
...
class HttpGrantRow(GrantEnvelopeColumns, Base):
    ...
    invariants. Status is derived, never stored (root STYLE.adoc § SQLAlchemy): the row records the
...
class Session(Base):
    ...
    **`status` is derived, never stored** (root STYLE.adoc § SQLAlchemy): the row records the facts —
...
class SubmittedPrompt(Base):
    ...
    **State is derived** (root STYLE.adoc § SQLAlchemy): pending is neither stamp, and the stamps —
