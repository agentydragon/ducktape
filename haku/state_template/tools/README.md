...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------- |
| `validate_local.sh` | Runs `validate_state.py` against `ui/backend/models.py` for items, runs, responses, improvements, and tool requests (exit 1 = invalid state, exit 2 = env can't run it → `push_state` warns + relies on CI).                   | via `push_state.sh`               |
