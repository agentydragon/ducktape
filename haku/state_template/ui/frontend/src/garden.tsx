...
// The rendered pane: fetch one file's markdown and render it as MDX (so embedded widgets work).
...
// procedures/, runs/) and read it rendered (with standard widgets). A general primitive — the
// same renderer backs run notes. Read-only.
    ...
    return (
      <Stack gap="md">
        <Group justify="space-between" align="center">
          <Anchor onClick={() => setSelected(null)} style={{ cursor: "pointer" }}>
            ← All notes
          </Anchor>
  ...
  return (
    <Stack gap="lg">
      <Text size="sm" c="dimmed">
        My notebook — memory, procedures, and run notes, rendered with standard widgets. Read-only.
      </Text>
      {groups.map((dir) => (
        <Stack gap={4} key={dir}>
