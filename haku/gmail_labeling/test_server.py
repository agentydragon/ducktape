...
async def test_tool_surface(make_client):
...
async def test_apply_label_tool_succeeds(make_client):
...
async def test_apply_label_tool_rejects_outside_prefix(make_client):
    client, _ = make_client()
    async with Client(build_mcp(client)) as mcp_client:
