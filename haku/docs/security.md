...
3. The console renders **no** Haku-authored content. The one haku-state credential in its
   namespace (Haku's own, for the `haku_index` git corpus) is mounted only by the haku-indexer
   worker Deployment — not the console API pod — and is only ever fetched with; the
