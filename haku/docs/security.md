...
| 5   | `haku-mitmproxy` egress fence on `haku-sandbox` **and `haku-ci`**                                                                                                                                                     | All pod egress allowlisted at L3/L4 + L7. haku-ci (the Bazel image-build runner) egresses through the same proxy; its caching addon caches GETs only from a strict immutable-build-dep host allowlist, so no credentialed/API response ever enters the cache | <../../cluster/k8s/agents/haku-mitmproxy/>                                                                                 |
