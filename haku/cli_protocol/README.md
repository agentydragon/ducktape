...
The console drove that protocol directly rather than through the Agent SDK (decided 2026-08), so
the protocol became something this repo had to understand rather than depend on. Nothing here is published by
...
`bbr test //haku/cli_protocol:...` runs the captured-wire regression check.
...
frame, that `PreCompact` runs in time to matter. It does not catch changed behaviour, because it
reads a recording. For behaviour, re-run the probes and reconcile <protocol.md> — the fields it
