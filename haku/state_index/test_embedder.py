...
async def test_reports_its_identity(embedder: OnnxEmbedder) -> None:
    assert (embedder.model_key, embedder.dims) == (BGE_SMALL_EN_V15, 384)


async def test_embeds_to_unit_vectors_of_the_declared_width(embedder: OnnxEmbedder) -> None:
    vectors = await embedder.embed_documents(["a short note", "another one entirely"])
...
async def test_a_query_is_closer_to_its_topic_than_to_an_unrelated_one(embedder: OnnxEmbedder) -> None:
    """Retrieval, not just arithmetic: the whole index rests on this ordering holding."""
    relevant, unrelated = await embedder.embed_documents(
        [
    ...
    query = await embedder.embed_query("how do I get bread to rise")

    assert _similarity(query, relevant) > _similarity(query, unrelated)


async def test_empty_input_costs_no_inference(embedder: OnnxEmbedder) -> None:
