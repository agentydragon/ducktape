...
persist_agent_shell_init() {
  ...
  local shell_startup_files=("$bashrc" "$bash_profile")
  local shell_file

  for shell_file in "${shell_startup_files[@]}"; do
    touch "$shell_file"
  done
  ...
  if [ -f "$NIX_DAEMON_PROFILE_SH" ]; then
    log "persisting nix-daemon profile sourcing into shell startup files"
    for shell_file in "${shell_startup_files[@]}"; do
  ...
  if [ -n "${SOPS_AGE_KEY:-}" ]; then
    log "persisting SOPS_AGE_KEY into shell startup files for agent shells"
    for shell_file in "${shell_startup_files[@]}"; do
  ...
  log "persisting web_env.sh sourcing into shell startup files for runtime secret hydration"
  for shell_file in "${shell_startup_files[@]}"; do
