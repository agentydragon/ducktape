...
+----------------------------------+-----------------------------+--------------+---------------------+
| **RST (rstcheck)**               | ``rstcheck`` hook           | N/A          | Pre-commit          |
+----------------------------------+-----------------------------+--------------+---------------------+
...
+------------+-----------------------------------+--------------------------------+
| rstcheck   | Nix devshell                      | Pre-commit ``rstcheck`` hook   |
+------------+-----------------------------------+--------------------------------+
...
- **rstcheck** - reStructuredText syntax and directive validation
...
+------------------------+-----------------------------+-----------------------------------+
| ``rstcheck``           | local (system)              | RST syntax and directive checks   |
+------------------------+-----------------------------+-----------------------------------+
...
Cluster structural validation (kustomize builds, CRD layering, orphaned files, Flux dependencies, health checks, blueprint completeness) runs as the ``//cluster/validation:test_*`` Bazel test suite, not as a pre-commit hook.
...
- buildifier: ``@buildifier_prebuilt//buildifier`` (used by ``//devinfra/lint``)
...
1. **For Python/JS/Rust**: Add aspect to ``devinfra/lint/linters.bzl``
2. **For other languages**: Add to ``.pre-commit-config.yaml``
3. **Update this document**

RST documents use ``rstcheck`` for syntax and directive validation. The hook checks each changed document without publishing generated HTML artifacts.
