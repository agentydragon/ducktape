...
|AsciiDoc |`asciidoctor` hook |— |Pre-commit
|===
...
|asciidoctor |Nix devshell |Pre-commit `asciidoctor` hook
|===
...
* *asciidoctor* - AsciiDoc syntax and include validation
...
|`asciidoctor` |local (system) |AsciiDoc syntax and include validation
|===
...
[arabic]
. *Version drift risk*: Pre-commit uses external ruff/buildifier versions that may differ from Bazel-managed versions. TODO in `.pre-commit-config.yaml` tracks the buildifier version sync issue.
. *ESLint not in pre-commit*: JS/TS linting only runs in CI via Bazel aspects, not locally during commit.
. *mypy not in pre-commit*: Type checking only runs in CI via Bazel aspects, not locally during commit.

== Adding New Linters

[arabic]
. *For Python/JS/Rust*: Add aspect to `devinfra/lint/linters.bzl`
. *For other languages*: Add to `.pre-commit-config.yaml`
. *Update this document*

AsciiDoc documents use `asciidoctor` for syntax and include validation. The hook
renders each changed document to `/dev/null`; it does not publish HTML artifacts.
