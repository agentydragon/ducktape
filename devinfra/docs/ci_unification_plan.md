...
Python linting (ruff, mypy), JS/TS (eslint, TypeScript type checks), Rust (clippy, rustfmt), Terraform validation (rules*tf with hermetic provider mirror), k8s structural validation (kustomize builds, Flux dependency checks, etc. as the `//cluster/validation:test*\*`py_test suite). Current tool wiring: see`devinfra/docs/linting.md`.
