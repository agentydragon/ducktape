...
Python linting (ruff, mypy), JS/TS (eslint, svelte-check), Rust (clippy, rustfmt), Terraform validation (rules*tf with hermetic provider mirror), k8s structural validation (kustomize builds, Flux dependency checks, etc. as the `//cluster/validation:test*\*`py_test suite). Current tool wiring: see`devinfra/docs/linting.rst`.
...
## K8s validation (Phase 4 — done; superseded by `devinfra/docs/linting.rst`)

`kubeconform` is not a Bazel target — it runs only as a pre-commit hook (see
`devinfra/docs/linting.rst`). Structural validation (kustomize builds, CRD
...
`devinfra/docs/linting.rst` is the current source of truth for this wiring;
...
- [x] K8s structural validation — implemented as `//cluster/validation:test_*` py_test suite (see `devinfra/docs/linting.rst`)
