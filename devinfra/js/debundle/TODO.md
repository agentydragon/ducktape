...
- **Selector debt reporting.** Shipped as `debundle spec selector-debt`:
  ...
  indices. Remaining work: rank multi-statement windows, consider owner-graph
  context where useful, and produce richer edit-distance diagnostics than the
  current first-mismatch score/reason.
