...
- **Declarator-hole miss diagnostics.** When a `source_match` selector uses
  `DECLARATORS` holes inside a large multi-declarator declaration, failed
  matches should rank nearest candidates by pinned declarator anchors
  (target initializer callee shape, literal values, and pinned declarator
  order) rather than reporting an unrelated declaration with a similar raw
  AST size. The diagnostic should point at the first pinned declarator or
  segment that failed, especially for `binding_groups[].source_match` and
  `members[].selector.source_match.target_binding`.
