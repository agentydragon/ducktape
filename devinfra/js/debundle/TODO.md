...
- **Selector debt reporting.** Add a CLI report that ranks name-only
  selectors over autogenerated/minified sources, repeated copied
  `source_match` bodies, near-ambiguous structural matches, and selectors
  whose minified binding changed across two spec versions.
- **Selector linting.** Once the report is useful, add an opt-in warning
  or gate for name-only selectors over autogenerated/minified sources,
