...
5. Apply the assignment using the shared guide's CLI workflows. Prefer
   ...
   prefer structural `source_match` / `binding_groups` with `EXPR`, `STMT`,
   `ARGS`, `STMT_LIST`, `CLASS_REST`, or `DECLARATORS` holes for volatile
   subtrees. For one selected declarator inside a wider comma-list declaration,
   ...
   `comment:` naming the concrete blocker and desired feature. Use specific,
   copyable blocker text such as:
   - `blocked on Ducktape support for matching one declarator inside a multi-declarator declaration / arbitrary sibling declarator hole`
   - `blocked on Ducktape support for an object-literal arbitrary key/value property wildcard in source_match selectors`
