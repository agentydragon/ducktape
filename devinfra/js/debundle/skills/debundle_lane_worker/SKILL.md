...
5. Apply the assignment using the shared guide's CLI workflows. Prefer
   ...
   `source_match` / `binding_groups` with `EXPR`, `STMT`, `STMT_LIST`, or
   `CLASS_REST` holes for volatile subtrees, and keep ambiguous structural
   matches rejecting rather than source-order selected. Do not modify the
   ...
   structural AST-shaped `source_match` instead.
