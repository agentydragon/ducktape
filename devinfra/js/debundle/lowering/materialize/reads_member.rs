...
//! `member_reads` (the projection rides the shared
//! `owner_graph_projection::solve_projected`, like `cross_ref`).
//!
...
//! (`object: @Anchor`, naming another member by its readable `name:`); the kernel
//! needs the object's *minified* binding to match the AST member-read. As with a
//! cross-ref anchor, that binding comes from the already-resolved members of the
//! same chunk, not the owner graph's `export_name` — see `materialize::cross_ref`
//! for why that anchor-first ordering is forced.
...
/// `reads_member` EDB rows the owner graph alone cannot supply. The
/// `member_of_module` use-site EDB is unused by the `reads_member` resolvers, so
/// it stays empty. The shared skeleton lives in
/// `owner_graph_projection::solve_projected`.
pub(super) fn build_resolution(
    ...
    solve_projected(graph, Vec::new(), Vec::new(), Vec::new(), |node| {
        let member_reads = reads_by_ordinal
