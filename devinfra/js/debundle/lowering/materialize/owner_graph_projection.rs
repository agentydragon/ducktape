//! Shared owner-graph → `selector_solve` projection for the five
//! lowering bridges (`cross_ref`, `reads_member`, `member_of_module`,
//! `passed_to_call`, `makes_decorate_call`, `intrinsic_alias`).
//!
//! All project the in-memory `analysis::OwnerGraph` onto the lean owner
...
//! node's `(member_reads, module_member_uses)` EDB rows plus the chunk-level
//! binding-keyed rows (`call_arguments` / `decorate_calls` / `intrinsic_aliases`,
//! keyed by a binding, not a node, so passed separately). The kernel is
...
/// `call_arguments` are the chunk-level `passed_to_call` rows, `decorate_calls`
/// the chunk-level `makes_decorate_call` rows, and `intrinsic_aliases` the
/// chunk-level `intrinsic_alias` rows (all keyed by a binding, not a node).
...
pub(super) fn solve_projected(
    ...
    call_arguments: Vec<selector_solve::CallArgumentUse>,
    decorate_calls: Vec<selector_solve::DecorateCallUse>,
    intrinsic_aliases: Vec<selector_solve::IntrinsicAliasUse>,
    ...
    let edges = graph
        ...
        .collect();
    selector_solve::solve(&selector_solve::OwnerGraph {
        nodes,
        edges,
        call_arguments,
        decorate_calls,
