...
Over-restriction is an acceptable failure mode; under-restriction is a
soundness violation. When an analysis is only conditionally correct, check
the precondition and fall back to the conservative path — never widen a
rule because the common case is fine.

The contract, the realizability theorem it serves, and the
conditionally-correct pattern: <docs/design.md> § "Soundness over
completeness". Admission rules for the purity whitelists
(`PURE_STATIC_CALLS`, `PURE_STATIC_FUNCTION_REFS`,
`PURE_OBJECT_CALLS_ON_PLAIN_DATA`), the spec's declared-purity trust
contracts (`purity:`, `pure_new`, `pure_members`), and the paired tests each
new entry must land with: <docs/purity_soundness.md>. Read that before
touching <purity/whitelists.rs> or adding a purity annotation to a spec.
...
`note:` field on `LogicalModule` (module top) / `Member` /
`AnonymousStatement`, plus per-binding annotation notes under `annotations:`,
is a ratified exemption, because the rewriters drop `#` comments — so a
round-tripped `note:` value is the only debt-rationale annotation that survives
a rewrite, and its reader is the human spec author. The emit/no-emit semantics
and the `modules merge` → `note:` provenance behavior are documented in
<README.md> → "Comments".
