//! The selector resolve every command calls: a chunk and the spec entities
//! aimed at it in, one [`SelectorOutcome`] per entity out.
//!
...
//! the name pins and relational selectors, and CP-SAT assigns every entity at
//! once under `all_different`. Semantics: <docs/selector_resolution.md>.
...
impl<'m> Chunk<'m> {
    ...
    /// Resolve every entity of `modules` jointly. A name pin on an import
    /// specifier gets no outcome; every other entity gets exactly one.
    pub fn resolve(&self, modules: &[SpecModule]) -> Result<Resolution> {
        Resolve::new(self, modules).run()
    }
}

/// One call of [`Chunk::resolve`].
struct Resolve<'c, 'm> {
    chunk: &'c Chunk<'m>,
...
impl<'c, 'm> Resolve<'c, 'm> {
    ...
    fn run(mut self) -> Result<Resolution> {
            ...
            for (member_index, member) in module.members.iter().enumerate() {
                match &member.selector {
                    ...
                    selector => {
                        ...
                        self.constrained.push((module_index, member_index));
                    }
                }
            }
        }
        ...
        for &(module_index, member_index) in &self.constrained {
        ...
        let presolve_no_match = self.name_pins_without_candidates();
        for &target in &presolve_no_match {
            let (module_index, member_index) = self.members[&target];
            self.push_member(module_index, member_index, Outcome::NoMatch);
        }
        let result = match self.decide_without_solver() {
            Some(result) => result,
            None => solve_global_selector_program(&program, &self.fact_store(&program))?,
        };
        self.record_solution(&program, &result, &presolve_no_match)?;
        Ok(Resolution {
            outcomes: self.outcomes,
        })
    }
    ...
    fn push_member(&mut self, module_index: usize, member_index: usize, outcome: Outcome) {
        ...
        self.outcomes.push(EntityOutcome {
            module: module_index,
    ...
    /// Records the outcome of `module.anonymous_statements[position]`.
    fn push_anonymous(&mut self, module_index: usize, position: usize, outcome: Outcome) {
        ...
        let statement_index = statement.index;
        self.outcomes.push(EntityOutcome {
            module: module_index,
            entity: EntityIndex::AnonymousStatement(statement_index),
    ...
    /// Name-pinned targets no declaration in the chunk can satisfy: no
    /// top-level statement declares the name, or none of the pinned kind.
    fn name_pins_without_candidates(&self) -> BTreeSet<SelectorTargetId> {
        let mut kinds_by_binding = BTreeMap::<&str, Vec<StatementKind>>::new();
        ...
        self.members
            .iter()
            .filter(|(_, (module_index, member_index))| {
                let MemberSelector::Binding(pin) =
                    &self.modules[*module_index].members[*member_index].selector
                else {
                    return false;
                };
                kinds_by_binding.get(pin.name.as_str()).is_none_or(|kinds| {
                    pin.kind
                        .is_some_and(|kind| !kinds.contains(&statement_kind_for_spec(kind)))
                })
            })
            .map(|(target, _)| *target)
            .collect()
    ...
    /// The solution when no two entities can interact: every entity is a
    /// `source_match` or an anonymous statement, and no place is a candidate
    /// of two `source_match` entities, so each entity's candidates are exactly
    /// its own rows. `None` when the solver has to decide.
    fn decide_without_solver(&self) -> Option<SolverResult> {
        ...
        for entity in &self.projected {
            // Anonymous statements are not kept on distinct owners.
            if entity
                .targets
                .iter()
                .any(|target| anonymous.contains(target))
            {
                continue;
            }
            let entity_owners = entity
                .rows
                .iter()
                .flatten()
                .map(|place| place.owner)
                .collect::<BTreeSet<_>>();
            let entity_bindings = entity
                .rows
                .iter()
                .flatten()
                .filter_map(|place| place.binding.clone())
                .collect::<BTreeSet<_>>();
            if !owners.is_disjoint(&entity_owners) || !bindings.is_disjoint(&entity_bindings) {
                return None;
            }
            owners.extend(entity_owners);
            bindings.extend(entity_bindings);
        }
    ...
    fn fact_store(&self, program: &SelectorProgram) -> SelectorFactStore {
        selector_fact_store(program, self.chunk)
    }
    ...
    fn record_solution(
        ...
        result: &SolverResult,
        presolve_no_match: &BTreeSet<SelectorTargetId>,
    ) -> Result<()> {
        let eliminated = self.eliminations(program, result);
        let module = self.chunk.module;
        for (target, module_index, position) in std::mem::take(&mut self.anonymous) {
            ...
            self.push_anonymous(module_index, position, outcome);
        }
        for (target, (module_index, member_index)) in std::mem::take(&mut self.members) {
            ...
            self.push_member(module_index, member_index, outcome);
        }
        Ok(())
    }
    ...
    fn eliminations(
        ...
        program: &SelectorProgram,
        result: &SolverResult,
    ) -> BTreeMap<SelectorTargetId, Vec<EntityRef>> {
