...
impl Bindings {
    ...
    /// Match a wildcard string literal: it matches any string-literal subject
    /// value, but one wildcard token binds to one value consistently across the
    /// match (the deleted matcher's `bind_string`).
...
fn shorthand_property_view(index: &Index, node: NodeId) -> Option<ShorthandProperty<'_>> {
    let same_name_key_value = |is_binding: bool| {
        ...
        let value_ident = index.ident_of(*value)?;
...
/// committed bindings) once every remaining segment is placed.
...
/// a single-declarator var-decl. For each body window of length
