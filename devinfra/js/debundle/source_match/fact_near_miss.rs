...
/// The candidate-list near-miss **hint** appended to a member/binding-group
/// no-match bail, reproducing the deleted matcher's `source_match_no_match_hint`
/// framing on the fact model. For a class needle it renders a "Nearest class
/// candidates:" block (per candidate: declared bindings, member labels, the
/// matched/total pinned-member-name-in-order count, and the first-divergence
/// reason); for a declarator-hole var-decl needle a "Nearest variable declaration
/// candidates:" block plus the `DECLARATORS_*`/`target_binding` guidance.
/// `None` when the needle is not one of those anchored shapes (the caller then
/// emits only the generic no-match message + selector echo).
pub(crate) fn fact_source_match_no_match_hint(
    runtime_module: &Module,
    selector: &AnonymousStatementSelector,
) -> Option<String> {
    let parsed =
    ...
    let needle_index = item_index(needle)?;
    let mode = fact_mode(selector);
    class_no_match_hint(runtime_module, &needle_index, mode)
        .transpose()
        .or_else(|| {
            var_declarator_no_match_hint(runtime_module, needle, &needle_index, selector, mode)
                .transpose()
        })
        .transpose()
        .ok()
...
/// The "Nearest class candidates:" block for a class needle (`None` when the
/// needle is not a class declaration or pins no member names). Mirrors the deleted
/// `class_source_match_no_match_hint`, fact-based.
fn class_no_match_hint(
    runtime_module: &Module,
    needle_index: &Index,
    mode: Mode,
) -> Result<Option<String>> {
    let Some((_, nclass)) = fact_class_decl_of_item(needle_index) else {
        return Ok(None);
    };
    let pinned_label_count = needle_index
        .children(nclass)
        .iter()
        .filter(|&&member| {
            !selector_match::is_class_rest_member(needle_index, member)
                && fact_class_member_label(needle_index, member).is_some()
        })
        .count();
    ...
    for (body_idx, candidate) in runtime_module.body.iter().enumerate() {
        ...
        let Some((_, cclass)) = fact_class_decl_of_item(&candidate_index) else {
            continue;
        };
        // Only non-matching candidates contribute a "first mismatch" reason.
        ...
        let reason = first_class_divergence(needle_index, nclass, &candidate_index, cclass, mode)?;
        let declared = declared_bindings(candidate)
            .into_iter()
            .map(|binding| binding.binding_name)
            .collect::<Vec<_>>();
        let member_labels = candidate_index
            .children(cclass)
            .iter()
            .filter_map(|&member| fact_class_member_label(&candidate_index, member))
            .collect::<Vec<_>>();
        ...
        candidates.push(ClassCandidateHint {
            body_idx,
            declared,
            member_labels,
...
/// The "Nearest variable declaration candidates:" block for a declarator-hole
/// var-decl needle (`None` when the needle is not a var-decl with at least one
/// hole and one pinned declarator). Mirrors the deleted
/// `var_declarator_source_match_no_match_hint`, fact-based.
fn var_declarator_no_match_hint(
    runtime_module: &Module,
    needle: &ModuleItem,
    needle_index: &Index,
    selector: &AnonymousStatementSelector,
    ...
    let Some(needle_var) = item_var_decl(needle) else {
        return Ok(None);
    };
    if !needle_var
        .decls
    ...
    let Some(nvd) = fact_var_decl_node(needle_index) else {
        return Ok(None);
    };
    let mut candidates = Vec::new();
    for (body_idx, candidate) in runtime_module.body.iter().enumerate() {
        let Some(candidate_var) = item_var_decl(candidate) else {
            continue;
        };
        ...
        let Some(cvd) = fact_var_decl_node(&candidate_index) else {
            continue;
        };
        let reason = first_pinned_var_declarator_divergence(
            needle_var,
            needle_index,
            needle_index.children(nvd),
            candidate_var,
            &candidate_index,
            candidate_index.children(cvd),
            mode,
        )?;
        let declared = declared_bindings(candidate)
            .into_iter()
        ...
        let declarator_labels = candidate_var
            ...
            .map(render_var_declarator_label)
            .collect::<Vec<_>>();
        let matched_pinned_declarators = selector_match::pinned_declarator_matches_in_order(
            needle_index,
            needle_index.children(nvd),
            &candidate_index,
            candidate_index.children(cvd),
        ...
        .map_err(|unsupported| anyhow::anyhow!("fact near-miss: {}", unsupported.reason))?
        .len();
        candidates.push(VarDeclCandidateHint {
            body_idx,
            declared,
            declarator_labels,
            ...
            reason: MismatchReason { score: 0, reason },
        });
    }
    if candidates.is_empty() {
        return Ok(None);
...
fn render_class_candidate_hint(candidate: &ClassCandidateHint) -> String {
    let declared_hint = render_declared_hint(&candidate.declared);
...
fn render_declared_hint(declared: &[String]) -> String {
    if declared.is_empty() {
        return "declares no bindings".to_string();
    }
    format!(
        "declares {}",
        declared
...
/// In-order pinned class-member-name match count over facts (the matched/total
/// numerator the candidate hint renders). Mirrors the deleted
/// `count_pinned_class_member_labels_in_order`.
fn count_pinned_class_member_labels_in_order(
    needle_index: &Index,
    nclass: NodeId,
    ...
    let mut matched = 0;
    for &nmember in needle_index.children(nclass) {
        ...
        let Some(candidate_idx) = cmembers.iter().enumerate().skip(candidate_start).find_map(
            |(candidate_idx, &cmember)| {
                (fact_class_member_label(candidate_index, cmember).as_deref()
...
/// The `(name_ident, Class)` fact nodes of a (possibly exported) class declaration
/// top-level item, or `None`. A plain `class C {}` projects to a `ClassDecl` root;
/// `export class C {}` wraps it in `ExportDecl`.
fn fact_class_decl_of_item(index: &Index) -> Option<(NodeId, NodeId)> {
    let root = index.root()?;
    let classdecl = match index.kind(root) {
        "ClassDecl" => root,
        "ExportDecl" => {
            let &inner = index.children(root).first()?;
            (index.kind(inner) == "ClassDecl").then_some(inner)?
        }
        _ => return None,
    };
    let kids = index.children(classdecl);
    // ClassDecl children: [Ident(name), Class].
    Some((*kids.first()?, *kids.get(1)?))
}

/// The `VarDecl` fact node of a (possibly exported) var-decl top-level item.
fn fact_var_decl_node(index: &Index) -> Option<NodeId> {
    let root = index.root()?;
    match index.kind(root) {
        "VarDecl" => Some(root),
        "ExportDecl" => {
            let &inner = index.children(root).first()?;
            (index.kind(inner) == "VarDecl").then_some(inner)
        }
        _ => None,
    }
}
...
mod tests {
    ...
    /// the decorator caveat in `debug/f5_body_debt_feasibility.md`), not a
    /// near-miss-specific regression. This test pins that boundary: the fact path
    /// does not invent a divergent row for these inputs.
    #[test]
