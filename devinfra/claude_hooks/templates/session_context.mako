...
- `docker run` works. `docker build --network=host` works (BuildKit handles large output gracefully).
  See `devinfra/claude_hooks/docs/docker_evaluation_results.md` for details. Note: Use `--network=host` for builds; Alpine apk may need `--no-check-certificate` for TLS proxy.
- Storage: \
...
- `podman run` works (with `--network=host`). `podman build` works (gVisor workarounds are pre-configured).
  See `devinfra/claude_hooks/docs/gvisor_dockerfile_build.md` for details. Note: RUN steps producing >~3MB stdout may hit a buildah SIGPIPE bug — redirect output if needed.
- Storage: \
% if container.storage_driver == "overlay":
