...
from devinfra.github_proxy.host.relay_config import render
from util.bazel import runfiles
from util.oci import OciImage, load_oci_image
from util.testing.undeclared_outputs import undeclared_outputs_dir

SQUID = OciImage("_main/devinfra/github_proxy/host/testing/squid_image.rloc", "github-api-relay-squid:test")
...
def test_missing_credentials_fail_closed(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(runfiles.get_required_path("_main/devinfra/github_proxy/host/relay_config.py")),
            "--host",
            "proxy.test",
            "--port",
            "443",
...
def test_pinned_ca_replaces_old_bundle_and_nss(tmp_path: Path, squid_image: str) -> None:
    ...
    nss = state / "nss"
    script = runfiles.get_required_path("_main/devinfra/github_proxy/host/prepare_trust.sh")
    with closing(docker.from_env()) as client:
