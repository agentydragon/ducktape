...
| Binary size          | 51.9 MB (was 27 MB -- garble inlines and pads code)                      |
...
### Files to REMOVE from `src/` (dead code in 495ea204)
...
All 12 files identified from old binary DWARF paths have been created:

- `internal/api/ccr_backend.go`
- `internal/api/noop_backend.go`
- `internal/api/session_ingress_backend.go`
- `internal/claude/session_urls.go`
- `internal/envtype/anthropic/config.go`
- `internal/input/parser.go`
- `internal/manager/skill_extraction.go`
- `internal/mcp/servers/codesign/types.go`
- `internal/session/noop_activity_recorder.go`
...
- Binary size nearly doubled: 27 MB -> 51.9 MB (garble inlines and pads code)
- Version string `release-d84d76b7-ext` visible as a literal constant (not obfuscated)
