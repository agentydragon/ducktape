...
func (sc *StartupContext) UnmarshalJSON(data []byte) error {
	...
	sc.UseCodeSessions = raw.UseCodeSessions
	sc.Entrypoint = raw.Entrypoint
	sc.EnvironmentVariables = raw.EnvironmentVariables
	sc.EnvironmentSubType = raw.EnvironmentSubType
	sc.FilesystemID = raw.FilesystemID
	sc.FilestoreURL = raw.FilestoreURL
	sc.WorkerID = raw.WorkerID
	sc.WorkerEpoch = raw.WorkerEpoch
...
// FilesystemConfig holds filestore deployment configuration.
// Binary: type name "FilesystemConfig" preserved by garble in 495ea204.
// JSON tags: json:"filestore_url", json:"filesystem_id" found in binary string table.
// Referenced via json:"filesystem" tag in a parent struct.
