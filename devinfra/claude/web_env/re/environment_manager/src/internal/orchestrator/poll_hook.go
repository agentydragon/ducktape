...
// PollHookStdinContext holds the context data marshaled as JSON and piped
// to the poll hook's stdin. Contains session and environment metadata needed
// by the hook to determine what work is available.
//
// Binary: offset 0x30 of PollHook struct, marshaled via json.Marshal at 0xa8e2ae.
...
type PollHook struct {
	...
	// Offset 0x30: StdinContext *PollHookStdinContext (marshaled as JSON for hook stdin)
	// Offset 0x38: Logger *slog.Logger
	HookCommand  string
	HookArgs     []string
	PollTimeout  time.Duration
	StdinContext *PollHookStdinContext // Marshaled as JSON and piped to hook's stdin
	Logger       *slog.Logger
}
