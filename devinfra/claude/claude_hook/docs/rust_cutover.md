...
- `release.yml` builds `//devinfra/claude/claude_hook:claude_hook` and
  publishes it to GitHub Releases with tag `claude-hook-<12hex>`.
- `sync-pins.yml` auto-updates `nix/artifact-pins.json` every 30 minutes.
- `nix/packages/default.nix` exposes `claude-hook`, the static Rust binary
  installed as `$out/bin/claude-hook`.
- The `#devtools` flake output includes `claude-hook` and `claude-statusline`.
  The statusline wheel, pin, and release all use the `claude-statusline` name.
