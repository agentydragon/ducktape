terraform {
  ...
  required_providers {
    ...
    # Reads the MCP bearer Secrets in-cluster for the static_bearer credentials.
    # In-cluster auth via the tf-runner SA (cluster-wide secret read).
    kubernetes = {
  ...
  # CR's backendConfig.customConfiguration (see cluster/k8s/haku/self-hosted-agent-tf).
  backend "kubernetes" {
    secret_suffix = "haku-self-hosted-agent"
