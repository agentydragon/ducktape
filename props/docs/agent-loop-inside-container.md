...
1. On each LLM request, proxy queries `run_costs` view to get current USD cost
...
The `run_costs` view joins `llm_requests` with `model_metadata` pricing table.
...
- Cost tracking via `llm_request_costs` and `run_costs` views
...
- Proxy sums costs up the parent chain on each request via `run_costs` view
