...
[IMPORTANT]
====
`SPEC.adoc` is append-only. `TODO.md` files in each component track implementation progress.
====
...
[arabic]
. *Pull* existing definition images from the registry (`crane export`/`crane config`)
. *Edit* definitions by overlaying files at the correct runfiles path
. *Push* new definitions by digest (`crane mutate --append` + `crane push`)
. *Run* critics via `run_critic` tool (blocks until exit)
. *Wait* for grading via `wait_until_graded_tool` (polls until complete)
. *Read* grading data from SQL views (`recall_by_definition_split_kind`, `recall_by_definition_example`, `tp_occurrence_credits`)
. *Read* snapshot source code and ground truth to understand what critics should find
