...
import { AirlockConnection, parseSessionKeyFromHwmUri } from "./airlock-connection.js";
...
export default async function register(api: OpenClawPluginApi): Promise<void> {
  const log = scopedLogger(api, "airlock");
  ...
  async function catchUpSession(sessionKey: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      requestHeartbeatNow({ reason: "airlock:notification", sessionKey });
  ...
  // ── Discover and re-register airlock tools ──────────────────────────
  ...
  // events, but does not expose airlock-wrapped tools to agents.
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text" as const,
                  text: `Airlock server is currently unavailable. Please retry shortly. (${String(err)})`,
                },
              ],
            };
          }
