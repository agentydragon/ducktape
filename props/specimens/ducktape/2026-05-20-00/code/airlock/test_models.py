"""Tests for airlock discriminated union models."""
...
def test_action_key_roundtrip():
    key = ActionKey(session_key=_SESSION_A, action_seq=42)
    parsed = ActionKey.model_validate_json(key.model_dump_json())
    assert parsed.session_key == _SESSION_A
    assert parsed.action_seq == 42


def test_action_key_extra_fields_rejected():
    with pytest.raises(ValidationError):
        ActionKey(session_key=_SESSION_A, action_seq=1, extra="oops")  # type: ignore[call-arg]
