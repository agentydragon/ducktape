...
readonly AGENT_ENV_FILE="$(cd "$HOME" && pwd)/.codex/agent_env.sh"
...
write_agent_env_file() {
  mkdir -p "$(dirname "$AGENT_ENV_FILE")"
  {
    ...
    echo "# Auto-generated by devinfra/codex_cloud/setup.sh"
    echo ". \"\$HOME/.cargo/env\""
    if [ -f "$NIX_DAEMON_PROFILE_SH" ]; then
      echo ". ${NIX_DAEMON_PROFILE_SH}"
      echo 'export PATH="$HOME/.nix-profile/bin:/nix/var/nix/profiles/default/bin:$PATH"'
    fi
    if [ -n "${SOPS_AGE_KEY:-}" ]; then
      printf "export SOPS_AGE_KEY='%s'\n" "$SOPS_AGE_KEY"
    fi
    echo "[ -n \"\${SOPS_AGE_KEY:-}\" ] && [ -f \"${REPO_ROOT}/devinfra/secrets/web_env.sh\" ] && source \"${REPO_ROOT}/devinfra/secrets/web_env.sh\" >/dev/null 2>&1 || true"
    if [ -n "${BUILDBUDDY_API_KEY:-}" ]; then
      printf "export BUILDBUDDY_API_KEY='%s'\n" "$BUILDBUDDY_API_KEY"
    fi
    printf "export BBR_BAZELRC='%s'\n" "$BBR_BAZELRC_PATH"
    printf "export SESSION_BAZELRC='%s'\n" "$CODEX_BAZELRC_PATH"
  } >"$AGENT_ENV_FILE"
...
persist_agent_shell_init() {
  ...
  log "writing dedicated non-interactive agent env file at ${AGENT_ENV_FILE}"
  write_agent_env_file
  ensure_line "[ -f \"${AGENT_ENV_FILE}\" ] && . \"${AGENT_ENV_FILE}\"" "$bash_profile"
  ensure_line "export BASH_ENV='${AGENT_ENV_FILE}'" "$bash_profile"
  ensure_line "[ -f \"${AGENT_ENV_FILE}\" ] && . \"${AGENT_ENV_FILE}\"" "$bashrc"
  log "for Codex Web env vars, set BASH_ENV=${AGENT_ENV_FILE}"
