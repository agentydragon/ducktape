...
from x.agent_server.mcp_bridge.auth import TokensConfig
from x.agent_server.mcp_bridge.compositor_factory import create_global_compositor
from x.agent_server.mcp_bridge.registry import InfrastructureRegistry
from x.agent_server.persist.sqlite import SQLitePersistence
from x.agent_server.runtime.registry import AgentRegistry
from x.agent_server.server.mcp_routing import TOKEN_TABLE, MCPRoutingMiddleware
...
def run_uvicorn(host: str = "127.0.0.1", port: int = 8765) -> None:
    uvicorn.run("x.agent_server.server.app:create_app", host=host, port=port, log_level="info", factory=True)
