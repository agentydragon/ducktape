...
from props.agents.critic.conftest import insert_issue, insert_occurrence
...
async def test_critic_sql_rls_isolation(test_critic_run, test_snapshot, temp_engine, db: Database):
    ...
    with db.session() as session:
        ...
        other_run = make_fake_critic_run(
            session=session, example=other_example.to_example_spec(), status=AgentRunStatus.IN_PROGRESS
        )
