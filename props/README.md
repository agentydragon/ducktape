...
The standalone registry proxy authenticates OCI clients against PostgreSQL and proxies `/v2/*` to Forgejo. CI pushes agent images via `docker login props-registry.allegedly.works`, and Kubernetes agent pods pull through the same proxy host using the `props-registry-pull` imagePullSecret rendered from the CNPG `props-db-app` credentials.

Keep the credential directions separate:

- `props-forgejo-upstream-creds`: proxy-to-Forgejo auth, consumed only by `props-registry-proxy`.
- `props-registry-pull`: kubelet/client-to-proxy auth, generated by ESO from `props-db-app`.

The registry is configured with `REGISTRY_HTTP_RELATIVEURLS=true` so blob upload `Location` headers use relative URLs (required for external clients going through the Gateway API proxy).
