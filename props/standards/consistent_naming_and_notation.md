...
- Avoid parallel synonyms for the same concept in names (e.g., `interface` vs `protocol` vs `facade`) unless distinctions are intentional and documented; prefer one obvious name. See also: [Renames must pay rent](./no_random_renames.md).
...
- Related properties: [Renames must pay rent](./no_random_renames.md), [Self‑describing names](./self_describing_names.md).
