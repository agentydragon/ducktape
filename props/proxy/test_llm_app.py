...
- Streaming and stateful mode rejection
"""
...
class TestLLMAuthentication:
    """Tests for LLM proxy authentication validation."""
    ...
    def test_non_agent_username_returns_401(self, client: TestClient, synced_test_db: DatabaseConfig):
        ...
        response = client.post(
            "/v1/responses",
            json={"model": "gpt-4o", "input": []},
            headers={"Authorization": basic_auth_header("postgres", "password")},
        )
        assert response.status_code == 401
        assert "Invalid agent token format" in response.json()["detail"]


class TestLLMModelEnforcement:
    """Tests for model restriction enforcement."""

    @patch("props.proxy.llm_app.httpx.AsyncClient")
    async def test_wrong_model_returns_403(
        ...
        response = client.post(
            ...
            headers={"Authorization": basic_auth_header(creds.username, creds.password)},
        )
...
class TestLLMStreamingRejection:
...
class TestLLMStatefulModeRejection:
...
class TestLLMRequestLogging:
    ...
    async def test_successful_request_is_logged(
        ...
        response = client.post(
            ...
            headers={"Authorization": basic_auth_header(agent_creds.username, agent_creds.password)},
        )
        assert response.status_code == 200
...
class TestLLMCompletedAgentRejection:
...
class TestLLMRLSRecursiveAccess:
    ...
    async def test_parent_can_see_child_llm_requests(
        ...
        async with TempUserManager(synced_test_db.admin, child_run_id) as child_creds:
            response = client.post(
                "/v1/responses",
                json={"model": "gpt-4o", "input": []},
                headers={"Authorization": basic_auth_header(child_creds.username, child_creds.password)},
            )
            assert response.status_code == 200
    ...
    async def test_unrelated_agent_cannot_see_other_requests(
        ...
        async with TempUserManager(synced_test_db.admin, agent1_run_id) as agent1_creds:
            response = client.post(
                "/v1/responses",
                json={"model": "gpt-4o", "input": []},
                headers={"Authorization": basic_auth_header(agent1_creds.username, agent1_creds.password)},
            )
            assert response.status_code == 200
