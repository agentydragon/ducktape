...
* Backend API: http://localhost:8000/api/ — see link:docs/backend_api.md[] for endpoint reference
...
Specimens live in `props/specimens/` (previously a https://github.com/agentydragon/specimens[separate repository]).
...
The `ADGN_PROPS_SPECIMENS_ROOT` environment variable is set automatically by direnv (`.envrc`) to `props/specimens/`. See link:specimens/docs/authoring_guide.md[] for the format spec.

== Evaluation Workflow

[arabic]
. *Specimens* — frozen code snapshots with human-labeled canonical issues
. *Critiques* — agent-generated issue reports per specimen
. *Grading* — comparison against canonical issues (recall/precision)
. *Selection* — agents ranked by fitness scores

Grading is automatic via snapshot graders. See link:docs/training_strategy.md[] for details.
