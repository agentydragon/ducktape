...
def agent_run_id(synced_test_db: DatabaseConfig) -> UUID:
    ...
    with get_session() as session:
        ...
        type_config = CriticTypeConfig(example=example.to_example_ref())
        agent_run = AgentRun(
            agent_run_id=run_id,
            image_digest=CRITIC_IMAGE_REF,
            model="gpt-4o",
...
class TestAuthentication:
    ...
    def test_invalid_credentials_returns_401(
        self, client: TestClient, synced_test_db: DatabaseConfig
    ):
    ...
    def test_non_agent_username_returns_401(
        self, client: TestClient, synced_test_db: DatabaseConfig
    ):
    ...
    async def test_completed_agent_returns_403(
        ...
        with get_session() as session:
            ...
            agent_run = AgentRun(
                ...
                image_digest=CRITIC_IMAGE_REF,
                model="gpt-4o",
                status=AgentRunStatus.COMPLETED,  # Already completed
                type_config=type_config.model_dump(),
            )
            session.add(agent_run)
            session.commit()
...
class TestRLSRecursiveAccess:
    ...
    async def test_unrelated_agent_cannot_see_other_requests(
        ...
        mock_response = AsyncMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"id": "resp", "output": [], "usage": {"input_tokens": 10, "output_tokens": 5}}
        mock_client = AsyncMock()
        mock_client.post.return_value = mock_response
        mock_client.__aenter__.return_value = mock_client
        mock_client.__aexit__.return_value = None
