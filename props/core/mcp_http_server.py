...
def _run_sse_server(server: FastMCP, host: str, port: int, stop_event: threading.Event) -> None:
    """Run FastMCP SSE server in a thread until stop_event is set.
    ...
    try:
        ...
        # FastMCP's run("sse") will block here
...
def serve_mcp_http(server: FastMCP, *, host: str = "127.0.0.1", port: int | None = None):
    ...
    server._auth = StaticTokenVerifier(token)

    stop_event = threading.Event()
    thread = threading.Thread(
        target=_run_sse_server, args=(server, host, port, stop_event), name="mcp-http-server", daemon=True
    )
    thread.start()
