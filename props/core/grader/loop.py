...
from agent_core.handler import AbortIf, BaseHandler, LoggingHandler, RedirectOnTextMessageHandler
...
from openai_utils.model import BoundOpenAIModel, SystemMessage, UserMessage
...
from props.core.grader.tools import (
    DeleteEdgesArgs,
    FillRemainingArgs,
    FPRef,
    GTDetails,
...
from props.core.loop_utils import ExitState
...
# =============================================================================
# Drift detection for daemon mode
# =============================================================================
...
class GraderDriftHandler(BaseHandler):
    ...
    def on_before_sample(self) -> Abort | InjectItems | NoAction:
            ...
            for edge_spec in args.edges:
                match edge_spec.gt_ref:
                    case TPRef(tp_id=tp_id, occurrence_id=tp_occ):
                        fp_id, fp_occ = None, None
                    case FPRef(fp_id=fp_id, occurrence_id=fp_occ):
        ...
        def submit(args: SubmitArgs) -> None:
            ...
            with get_session() as session:
                pending_count = session.scalar(
                    select(GradingPending).where(GradingPending.snapshot_slug == snapshot_slug).exists().select()
                )
                # For one-off mode, we only check our specific critic run
                # The actual validation happens in the agent_run status update
