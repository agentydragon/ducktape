...
from props.core.grader.drift_handler import check_grading_pending
from props.core.grader.loop import GraderMode, run_grader_loop
from props.core.grader.notifications import GRADING_PENDING_CHANNEL, GradingPendingNotification
...
class DaemonState:
    ...
    def notification_callback(
        ...
        logger.debug(f"Notification for {self.snapshot_slug}: {notification.event.operation} {notification.event.table}")
        self.notification_queue.append(notification)
        self.wake_event.set()


async def run_daemon(snapshot_slug: SnapshotSlug, system_prompt: str) -> int:
    ...
    try:
        while not state.shutdown:
            ...
            exit_code = await run_grader_loop(system_prompt, snapshot_slug, GraderMode.DAEMON)
...
async def main() -> int:
    ...
    with get_session() as session:
        ...
        logger.info("Agent run: %s, snapshot: %s", agent_run.agent_run_id, snapshot_slug)
    ...
    exit_code = await run_daemon(snapshot_slug, system_prompt)
