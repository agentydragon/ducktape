...
def show_execution_traces(limit: int = 5, parent_agent_run_id: UUID | None = None) -> None:
    ...
    with get_session() as session:
        ...
        for cr in critic_runs:
            ...
            llm_request_count = (
                session.query(LLMRequest).filter(LLMRequest.agent_run_id == cr.agent_run_id).count()
            )
            summaries.append(
                CriticRunSummary(cr.agent_run_id, snapshot_slug, cr.image_digest, str(cr.status.value), llm_request_count)
            )
