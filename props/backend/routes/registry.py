...
async def put_manifest(request: Request, repo: str, ref: str, admin_db: AdminDb, auth: Auth) -> Response:
    ...
    if not can_run_agent_type(auth.identity, ACL_CAN_PUSH_REGISTRY):
        raise _deny(auth.identity, "push to registry")

    if not is_digest(ref) and not can_run_agent_type(auth.identity, ACL_CAN_PUSH_TAGS):
        raise _deny(auth.identity, "push by tag")
...
async def registry_proxy(request: Request, path: str, auth: Auth, admin_db: AdminDb) -> Response:
    ...
    if request.method in ("GET", "HEAD"):
        if not can_run_agent_type(auth.identity, ACL_CAN_READ_REGISTRY):
            raise _deny(auth.identity, "read from registry")
    elif not can_run_agent_type(auth.identity, ACL_CAN_PUSH_REGISTRY):
        raise _deny(auth.identity, "push to registry")
