...
- [ ] Once `bazel-ci`'s PR-affected-target computation settles on one of `target-determinator` (#2679, #2683) or `bazel-diff` (#2706, #2705), drop the loser: remove its `nix/packages/*.nix`, its entry from `nix/packages/default.nix` and `flake.nix`'s `devToolsCommon`, and any references from `bazel-ci.yml` / docs. Both were packaged during the rollout while measuring the real memory + wall-time cost; only one should stay long-term.
