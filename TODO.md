...
- [ ] Drop `google-auth-oauthlib` and `google-auth-httplib2` from <pyproject.toml> if nothing needs them: no BUILD file references either, and unlike the wheels removed with `x/gatelet` this predates that deletion. Check first whether `google-api-python-client` needs them at runtime rather than as declared deps — that is the reason they were kept out of the gatelet dependency prune. Removal regenerates `requirements_bazel.txt` and the Gazelle manifest; follow <devinfra/docs/lockfiles.md>.
...
- [ ] Collapse `github_enabled` + `github_repo` in <devinfra/wt/shared/config_file.py> into one optional field. Today `github_enabled: bool = True` and `github_repo: str = ""` let a config say "GitHub on, repo unset", which `wt_server` has to reject at startup — an invalid state the types permit. `github_repo: str | None = None` makes the absent case the only way to express "no GitHub" and deletes the flag. Spotted during the footgun-defaults audit (#4694) and left out of it because this is a shape change, not a removed default.
- [ ] Give `CodexThread.approval_policy` and `.sandbox` in <haku/console/x/codex_app_server/client.py> enum types instead of bare `str`. Both carry a fixed vocabulary the app-server defines, and `sandbox` in particular selects the containment posture — `"danger-full-access"` is deliberate for the runtime pod, but a typo in it currently type-checks. STYLE.md § General: enums, not string literals.
- [ ] Delete the orphaned comment above the `_operator_auth_requires_canonical_public_origin` validator in <haku/console/config.py> ("Optional standing Kubernetes authorization policy…"). It documents a field that is not there — `kubernetes_authorization` lives on `ConsoleConfigFile` in <haku/console/mcp_config.py>, not on `Settings`.
