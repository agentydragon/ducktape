...
- [ ] **Gazelle cannot run at all** — it exits 1 before emitting any diff, so no BUILD file can be regenerated and new targets must be hand-written. The failure is ~15 packages where a `py_binary` already holds the name Gazelle wants for the `py_library` it generates from the same source: `//cpap:sync`, `//debug/litellm_probe:probe_models`, `//devinfra/claude/statusline`, `//devinfra/claude:scan_transcript_permissions`, `//devinfra/claude/web_env/tools:{build_and_diff,capture_manifest,capture_versions,diff_manifests,fetch_debs}`, `//devinfra/cpio:cpio_tool`, `//devinfra/firecracker/manager:service`, `//devinfra/firecracker/vm_pod:entrypoint`, `//devinfra/gc:workspace_gc`, `//devinfra/js:spa_fingerprint`, and `//devinfra/nixos_bazel_test/testdata/workspace:remote_smoke` (the last is the one that turns a warning into the fatal error). Gazelle names the fix in its own message: the `# gazelle:python_library_naming_convention` directive. Alternatively give those binaries `main_module` + a distinctly named library, which STYLE.md already prescribes for `py_binary` sharing a library's source. Also fix the unrelated parse warning on `agent_core/script_handler.py`.
