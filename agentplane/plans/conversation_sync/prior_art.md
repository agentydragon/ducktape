...
**P8 and W1 both point at row three.** A client-declared query is exactly "the client says what it
...
- **Zero (Rocicorp).** Client declares queries; the server maintains them incrementally and streams
  ...
  (P8), how deep history paging behaves, self-hosting.
- **Replicache (Rocicorp, earlier).** Client pulls a delta against a cookie; the **server** computes
  what changed for that client. Nothing to subscribe to, so W1 holds by construction, and the pull
...
- **PowerSync.** Postgres → SQLite, with server-side **sync rules** defining per-user buckets.
  Partition model like Electric's, so likely inherits W1; worth checking whether bucket membership
...
1. **Zero** — does a client query select fields, and can it page backwards cheaply? If yes it is the
   only option on this list that satisfies P8, W1 and W2 without us building the mechanism.
...
3. **Electric's `subset__where`** — our proxy pins it to `true = true`. If Electric can snapshot a
   ...
   snapshot may satisfy P1 and O1 together, and W1 with a single subscription. That would change the
   Electric option substantially, and it is the cheapest thing on this list to find out.
