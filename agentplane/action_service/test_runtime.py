...
from agentplane.action_service.policies.resources import BINDINGS_PLURAL, POLICY_SETS_PLURAL, SERVICE_ACCOUNTS_PLURAL
...
async def test_main_serves_real_stdio_execution_and_closes_in_order(db_url: str, tmp_path: Path) -> None:
    ...
    with (
        patch("agentplane.action_service.main.k8s_config.load_incluster_config"),
...
async def test_main_failure_disposes_engine_after_owned_resources(failure: str) -> None:
    ...
    settings = Settings(database_url="postgresql://unused", _cli_parse_args=False)
    with (
        patch("agentplane.action_service.main.make_engine", return_value=engine),
        patch("agentplane.action_service.main.verify_schema", verify),
        patch("agentplane.action_service.main.k8s_config.load_incluster_config"),
...
async def test_main_auto_approves_the_bound_service_account_from_watched_policy_objects(
    ...
    async with fake_apiserver(
        ...
        with (
            patch("agentplane.action_service.main.k8s_config.load_incluster_config", side_effect=in_cluster),
            patch("agentplane.action_service.main.PolicyIndex", return_value=index),
            patch.object(uvicorn.Server, "serve", serve),
        ):
            await async_main(settings)
