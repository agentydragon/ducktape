...
def test_image_contains_both_harnesses_and_the_runner_entry_point() -> None:
    ...
    layout = get_required_path(get_required_path("_main/agentplane/runner/image_rloc.rloc").read_text().strip())
    ...
    assert config["config"]["Entrypoint"] == ["/agentplane/runner/runner_image_bin"]
    assert members["agentplane/runner/runner_image_bin"].mode & 0o111
