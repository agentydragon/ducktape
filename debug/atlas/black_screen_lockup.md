...
3. **Memory pressure / ZFS ARC starvation (incident 15, recurrence Mar 18)**: wyrm2's memory was bumped to 112 GiB (balloon=0) ~1-2 days before this boot. With 128 GiB total, wyrm2 + Talos CP (8 GiB) = 120 GiB, leaving only ~8 GiB for the host, ZFS ARC, and kernel. ZFS ARC reports `memory_available_bytes = -1.9 GiB` (negative!), `arc_no_grow = 1`, and ARC size (3.6 GiB) is below `c_min` (3.9 GiB). ZFS zvol write threads blocked 122+ seconds on `dbuf_read → cv_wait` (waiting for ARC-evicted buffers to be re-read from disk). The `txg_quiesce` thread also blocked, preventing transaction group commits. `journald` reported "Under memory pressure, flushing caches" at 3836s and 3862s, immediately before the ZFS stall at ~3934s. Load average hit 101 as processes piled up waiting for ZFS I/O. **This failure mode is new with the recent memory increase** — earlier incidents (7-14) had different wyrm2 memory configs.

   **Mar 18 recurrence**: After wyrm2 was reduced to 96 GiB and ran stable for 7 days, memory pressure stalls returned — brief multi-second freezes affecting all host processes. Root cause: virtiofsd for `/tank/code` had grown to **6.6 GiB RSS** with `cache=auto` (the default). The `cache=never` fix from the wyrm (VM 100) OOM investigation (notes removed 2026-08; git history) was not carried over when wyrm2 (VM 110) replaced wyrm. Memory budget: wyrm2 96 GiB + Talos 8 GiB + virtiofsd 6.6 GiB + ZFS ARC 6.9 GiB = ~118 GiB of 123 GiB usable, with `memory_available_bytes` at 130 MB. PSI confirmed: `full avg60=0.99` (1% of time completely stalled on memory). Fix: set `cache=never` on both virtiofs mounts in terraform config.
