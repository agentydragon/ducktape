...
2. **One generated ConfigMap** in an otherwise hand-written directory
   (`agents/haku-openclaw-spike/app`, `agents/public-coder-agent/app`): a
   `<name>-config.k8s.yaml` the hand-written `kustomization.yaml` lists as a resource,
   replacing a `configMapGenerator` entry.
3. **Hand-written.**
