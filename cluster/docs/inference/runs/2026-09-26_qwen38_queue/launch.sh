...
if [[ $mode == downloads ]]; then unit=wyrm2-qwen38-downloads; fi
if systemctl --user is-active --quiet "$unit"; then
  echo "Queue already active" >&2
  exit 1
fi
# A previous manually started download must be stopped before resuming its files.
if [[ $mode == downloads ]] && pgrep -af 'curl.*--output /var/lib/llm-models-ssd/Qwen3.8-Flash-Next-GGUF/'; then
...
systemd-run --user --unit="$unit" --collect \
  ...
  --setenv="PATH=$PATH" \
  "$(command -v bash)" "$output/source/2026-09-26_qwen38_queue/queue.sh" "$output" "$mode"
