...
| `local-path-ovh-hdd` | local-path-provisioner | `hil-ovh` | OVH-HA CNPG DBs; SeaweedFS HDD volume servers; bulk node-pinned data. KS-5 HDD (`storage-tier=hdd`)       |
| `local-path-ovh-ssd` | local-path-provisioner | `hil-ovh` | Small fsync-sensitive OVH-local data (SeaweedFS SSD tier, hot DBs). KS-GAME NVMe (`storage-tier=ssd`)     |
| `local-path-ovh`     | local-path-provisioner | `hil-ovh` | **Deprecated** alias of `local-path-ovh-hdd` (existing consumers only). See <docs/ovh_storage_tiering.md> |
