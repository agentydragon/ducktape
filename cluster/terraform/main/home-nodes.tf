    ...
    name => yamlencode({
      machine = merge(local.worker_machine_base, {
        install = {
          ...
          image = "factory.talos.dev/metal-installer/${talos_image_factory_schematic.kimsufi.id}:${lookup(local.talos_installer_version_overrides, name, var.talos_version)}"
        }
