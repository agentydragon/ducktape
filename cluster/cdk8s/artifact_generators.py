...
_DUCKTAPE_ARTIFACTS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("atuin-namespace", ("cluster/k8s/atuin/namespace",)),
    ...
    ("atuin", ("cluster/k8s/atuin/app",)),
    ("atuin-db", ("cluster/k8s/atuin/db",)),
...
def generate_artifact_generators(root: Path) -> None:
    """Synthesize the ArtifactGenerator CRs and their Flux/Kustomize wrappers."""
    ...
    (out_dir / "flux-kustomization.yaml").write_text(
        Yaml.format_objects(
            [
                flux_kustomization(
                    "artifact-generators",
