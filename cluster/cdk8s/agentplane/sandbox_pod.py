...
a kubeconfig that reaches the API server through the proxy, and the proxy environment that points a
workload at the sidecar. The runner template (app.py) and the sandbox Actions' command box
...
def egress_mounts() -> list[SandboxTemplateSpecPodTemplateSpecContainersVolumeMounts]:
    ...
    path every client falls back to, plus the kubeconfig. A subPath mount does not follow ConfigMap
    updates: a CA rotation reaches a sandbox at its next Pod."""
    return [
        SandboxTemplateSpecPodTemplateSpecContainersVolumeMounts(
...
def add_tool_config(scope: Construct, env: Environment) -> None:
    """The kubeconfig: the API server through the proxy, verified against the bundle that carries the
    interception root, with the placeholder the proxy swaps for the box's own projected token. The
    box has no token of its own to put there; the sidecar holds it."""
    ConfigMap(
        scope,
        "sandbox-tool-config",
