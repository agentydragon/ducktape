...
- **Namespaces** several Kustomizations deploy into (`seaweedfs`, `monitoring`,
  `haku-sandbox`, `kube-system`) or whose contents are written at runtime (`budget`,
  `agentydragon`) keep their own Kustomization and convert standalone.
