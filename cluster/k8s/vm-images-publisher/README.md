...
`vm-images-publisher.k8s.yaml` is generated by
`cluster/cdk8s/vm_images_publisher/publisher.py`:

- the namespace, with privileged PodSecurity (the Job needs `/dev/kvm` for the
  qemu-efi image builder);
- the existing `vm-images` Bucket plus its SeaweedFS identities and
  publisher-local `S3Credentials`. The operator generates
  `vm-images-ci-writer-s3-credentials` and
  `vm-images-cdi-reader-s3-credentials` in this namespace;
- a suspended CronJob carrying the PodTemplate. Operators trigger one-off Jobs
  from it via `kubectl create job --from=cronjob/…`.
