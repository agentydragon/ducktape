...
- [ ] **Seed Langfuse API keys in Vault after first deploy**: Once Langfuse is
      running at `https://langfuse.allegedly.works`, sign in, create a project,
      and generate an API key pair. Then write them to Vault so LiteLLM can
      pick them up:
      `vault kv put kv/langfuse/api-keys public_key=pk-lf-... secret_key=sk-lf-....
`
      The `langfuse-api-keys` ExternalSecret in the `ollama` namespace will
      sync within 1h (or force with
      `kubectl annotate externalsecret -n ollama langfuse-api-keys force-sync=$(date +%s)`).
