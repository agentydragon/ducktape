...
- [x] **Langfuse API keys are fully automated** — generated by the
      `langfuse-secrets` Terraform module (`kv/langfuse/secrets`), bootstrapped
      into Langfuse via `LANGFUSE_INIT_PROJECT_*` env vars on first startup, and
      read by LiteLLM via the `langfuse-api-keys` ExternalSecret in the `ollama`
      namespace. No manual step required.
