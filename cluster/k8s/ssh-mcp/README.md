...
`cluster/cdk8s/ssh_mcp_constructs.py`; the three SOPS-encrypted key files remain
hand-written siblings. The generator reads the target roster from
`cluster/cdk8s/ssh_mcp_config.py`, the devbox Service, its canonical public host key,
and `nebula-mesh.json`.
...
or captured host keys in <../../cdk8s/ssh_mcp_config.py>, then run
`bb run //cluster/cdk8s:generate_manifests`. The devbox hostname comes from its Service and
the key from `ssh_keys/public-coder-devbox-host.pub`; the same run generates sshpiper's typed
Pipe and matching host-key pins.
...
is <../../cdk8s/ssh_mcp_constructs.py>.
