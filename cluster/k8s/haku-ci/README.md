...
- **Not in `haku-sandbox`.** It lives in its own namespace where Haku has **no RBAC**, so Haku
  ...
  through the **same `haku-egress-proxy`** that fences haku-sandbox: `ccnp-force-proxy-egress.yaml`
  permits only DNS, cluster-internal (the in-cluster Forgejo git + registry), and
  `haku-egress-proxy:3128`. All external egress (base images, npm/pypi, Bazel/toolchain,
  Forgejo actions) flows through the proxy's allowlist
  (`agents/haku-egress-proxy/cnp-haku-cloud-api-egress.yaml`), where Squid caches immutable
...
| `ccnp-force-proxy-egress.yaml` | egress fence: DNS + in-cluster + `haku-egress-proxy:3128` only (all external egress via the proxy)               |
| `config.yaml`                  | the act_runner config (labels, dind `DOCKER_HOST`, capacity, job-container proxy/CA), via a `configMapGenerator` |
| `deployment.yaml`              | the act_runner + rootless `dind` sidecar (proxy env + haku-egress-proxy CA mount)                                |
