...
`kube-api-proxy.k8s.yaml` (the HTTPRoute and the nginx ConfigMap, Deployment and Service)
and `kustomization.yaml` are generated by <../../cdk8s/kube_api_proxy.py>, which also holds
the Flux Kustomization with its health checks.
...
`kube_api_proxy.py`); otherwise the apiserver returns `400 "Upgrade request required"`.
