...
- **Logs/configmaps in explicitly opted-in namespaces**. A GitOps-owned Namespace label,
  `rbac.ducktape.io/agent-logs-configmaps-reader: "true"`, is the source of truth. The
  `generate-agent-diagnostics-readers` Kyverno policy generates one namespaced
  `RoleBinding` containing the Haku group, both Haku ServiceAccounts, and the
  `kubectl-sandbox-users` group. These carry controller diagnostics, not user content.
  Namespace inventory diagnostics use the separate
  `rbac.ducktape.io/agent-namespace-diagnostics-reader: "true"` label and the same policy.
