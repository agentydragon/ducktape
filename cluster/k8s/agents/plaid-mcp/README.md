...
- `plaid-mcp-db-readonly` is generated by ESO for the read-only `plaid_ro` role,
  used by the Postgres MCP and copied by ESO into `haku-sandbox` for Haku's ad-hoc
  queries, so both share one revocable read-only credential. The copy reads through a
  store that reaches only this Secret, and refreshes every 10 minutes.
