...
# Cerebras public inference models. Key from the CEREBRAS_API_KEY env var
# (litellm-cerebras-key secret). The roster is maintained in model_rosters.py.
...
def _cerebras_entries() -> Iterator[dict]:
    for model in CEREBRAS_MODELS:
        model_info = {"mode": "chat", "supports_function_calling": True}
        if model in CEREBRAS_VISION_MODELS:
            model_info["supports_vision"] = True
        yield {
            "model_name": exposed_name(Provider.CEREBRAS, ApiShape.OAI_CHAT, model),
            "litellm_params": {"model": f"cerebras/{model}", "api_key": "os.environ/CEREBRAS_API_KEY"},
...
def _expected_main_config() -> dict:
    ...
    model_list.extend(_cerebras_entries())
...
def test_terraform_cheap_experiments_allowlist_is_the_intended_low_cost_subset() -> None:
    ...
    expected_cerebras = {exposed_name(Provider.CEREBRAS, ApiShape.OAI_CHAT, model) for model in CEREBRAS_MODELS}
    expected = (
        expected_ollama
        | expected_google
        | expected_cerebras
