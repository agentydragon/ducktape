...
Five proxies bound what agent workloads reach. Four enforce a host list -- three Cilium
`toFQDNs` fences generated by `cluster/cdk8s/egress_fences.py` plus the openclaw spike's
app-level iron transform -- and one (`public-coder`) is a deliberate, documented waiver that
...
def allowlists(generated_fences: dict[str, dict[str, Any]], k8s_dir: Path) -> dict[str, frozenset[str]]:
    ...
    for iron, nothing for the waiver (`test_public_coder_waiver_is_unconfined` keeps that true)."""
    return {
