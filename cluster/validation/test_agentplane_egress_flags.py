...
A renamed or dropped setting is otherwise found by a CrashLoopBackOff. `allowed_service_account_namespaces` is
a set, and how pydantic-settings takes a set on the command line is not obvious from the field, so the
...
def test_a_second_workload_namespace_is_another_comma() -> None:
    ...
    without = [
        arg for arg in _proxy_args("agentplane-staging") if not arg.startswith("--allowed-service-account-namespaces=")
    ]
    settings = Settings(
        _cli_parse_args=[*without, "--allowed-service-account-namespaces=agentplane-staging,public-coder", DATABASE_URL]
    )
...
def test_a_deployment_naming_no_workload_namespace_is_refused(namespace: str) -> None:
    ...
    without = [arg for arg in _proxy_args(namespace) if not arg.startswith("--allowed-service-account-namespaces=")]
