...
func TestShouldChangeEndpoint(t *testing.T) {
	// DOWN → should change
	ps := &PeerStatusSpec{State: kubespan.PeerStateDown, LastUsedEndpoint: netip.MustParseAddrPort("1.2.3.4:51820")}
	if !ps.ShouldChangeEndpoint() {
		t.Error("expected ShouldChangeEndpoint=true for DOWN peer")
	}

	// No endpoint set → should change
	ps = &PeerStatusSpec{State: kubespan.PeerStateUnknown}
	if !ps.ShouldChangeEndpoint() {
		t.Error("expected ShouldChangeEndpoint=true with no endpoint")
	}

	// UP → should not change
	ps = &PeerStatusSpec{State: kubespan.PeerStateUp, LastUsedEndpoint: netip.MustParseAddrPort("1.2.3.4:51820")}
	if ps.ShouldChangeEndpoint() {
...
func TestPickNewEndpoint(t *testing.T) {
	...
	ps = &PeerStatusSpec{LastUsedEndpoint: ep1}
	got = ps.PickNewEndpoint(endpoints)
	if got != ep2 {
	...
	ps = &PeerStatusSpec{LastUsedEndpoint: ep3}
	got = ps.PickNewEndpoint(endpoints)
	if got != ep1 {
		t.Errorf("expected %s, got %s", ep1, got)
	}
