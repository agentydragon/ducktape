...
`bb run //devinfra:gazelle` owns Python `py_library`/`py_test` rules; CI fails on
drift (`--mode=diff`), so run it instead of hand-editing managed `srcs`/`deps`.
Mechanism, escape hatches, and limitations: <devinfra/docs/gazelle.md>.
