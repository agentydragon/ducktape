...
def _out(**kw) -> ProviderFetch:
    return ProviderFetch(fetched_at=_NOW, **kw)


def _pq(**kw) -> ProviderQuota:
    return ProviderQuota(provider="claude", last_output=_out(**kw))


def test_no_extra_usage_is_not_over_plan() -> None:
    assert not currently_over_plan(_out())
...
def test_feature_off_is_not_over_plan() -> None:
    ...
    assert not currently_over_plan(out)
...
def test_prepaid_exhausted_with_feature_on_is_over_plan() -> None:
    ...
    assert currently_over_plan(out)
...
def test_extra_status_transitions() -> None:
    quotas = AllQuotas(
        providers=[
            ...
            ProviderQuota(
                provider="zai",
                last_output=_out(long_window=QuotaWindow(used_percent=5, reset_seconds=86400, window_seconds=604800)),
            ),
