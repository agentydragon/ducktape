...
1. **Equity and rates are independent.** `rate_beta` fits to **+1.57 (R² = 0.0041)** over
...
2. **Equity history is 46 years.** VFINX 1980–2026 is the longest _total-return_ series
...
3. **No cyclical credit spread.** A muni's spread over the curve is a constant, so the model
...
4. **Mismatched inflation and equity windows.** Inflation reaches 1947, equity only 1993, so
...
5. **The rate means are barely identified.** OLS on a near-unit-root series biases mean
...
6. **A linear curve.** The yield at an instrument's duration interpolates between the short
...
7. **No equity distribution.** `IncomeCategory` has no qualified-dividend rate, so an equity
...
8. **No held-to-maturity instrument.** Every fixed-income holding here is a marked fund with
...
9. **Nothing is regime-switching.** Rates mean-revert around one level with one volatility.
   The 2009–2021 ZIRP era and 1981 are the same process here, differing only by draw.
10. **No housing, no private equity, no crypto.** By design — compose with another provider
