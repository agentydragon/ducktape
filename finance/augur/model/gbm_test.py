...
def test_shape_and_month_zero_is_initial_value() -> None:
...
def test_deterministic_when_sigma_zero() -> None:
    # With no volatility the sampler collapses to the analytic path initial * exp(cumsum(mu)).
...
def test_log_return_moments() -> None:
    mu, sigma = 0.003, 0.05
