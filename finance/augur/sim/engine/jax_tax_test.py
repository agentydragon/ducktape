...
def _cents(*usd: float) -> jnp.ndarray:
    return jnp.asarray([usd_to_cents(value) for value in usd], dtype=jnp.int64)
...
def test_apply_brackets_hand_computed_federal_single_50k() -> None:
    ...
    tax = _apply_brackets(_cents(50_000.0), **_brackets(fed.ordinary_income_brackets["single"]))
...
def test_apply_brackets_hand_computed_federal_single_200k() -> None:
    ...
    fed = load_jurisdiction("federal_us")
    tax = _apply_brackets(_cents(200_000.0), **_brackets(fed.ordinary_income_brackets["single"]))
...
def test_apply_brackets_vectorized_handles_multiple_rollouts() -> None:
    ...
    fed = load_jurisdiction("federal_us")
    tax = _apply_brackets(
        _cents(0.0, 11_600.0, 50_000.0, 200_000.0), **_brackets(fed.ordinary_income_brackets["single"])
    )
...
def test_apply_brackets_negative_input_zeroes_out() -> None:
    ...
    produce zero tax."""
    schedule = [TaxBracket(upper_usd=10.0, rate=0.1), TaxBracket(upper_usd=math.inf, rate=0.2)]
    tax = _apply_brackets(_cents(-50.0, 0.0, 5.0, 15.0), **_brackets(schedule))
...
def test_apply_brackets_california_single_100k() -> None:
    ...
    tax = _apply_brackets(_cents(100_000.0), **_brackets(ca.ordinary_income_brackets["single"]))
...
def test_ltcg_brackets_zero_when_total_taxable_below_threshold() -> None:
    ...
    tax = _apply_ltcg_brackets(_cents(10_000.0), _cents(30_000.0), **_brackets(fed.ltcg_brackets["single"]))
    assert tax.tolist() == [0]
...
def test_ltcg_brackets_split_across_zero_and_fifteen_percent() -> None:
    ...
    fed = load_jurisdiction("federal_us")
    assert fed.ltcg_brackets is not None
    tax = _apply_ltcg_brackets(_cents(20_000.0), _cents(30_000.0), **_brackets(fed.ltcg_brackets["single"]))
...
def test_ltcg_brackets_pure_fifteen_when_ordinary_already_in_15pct_band() -> None:
    ...
    fed = load_jurisdiction("federal_us")
    assert fed.ltcg_brackets is not None
    tax = _apply_ltcg_brackets(_cents(50_000.0), _cents(100_000.0), **_brackets(fed.ltcg_brackets["single"]))
...
def test_ltcg_brackets_vectorized() -> None:
    ...
    tax = _apply_ltcg_brackets(
        _cents(10_000.0, 20_000.0, 50_000.0, 0.0),
        _cents(30_000.0, 30_000.0, 100_000.0, 0.0),
...
def test_net_capital_gains_pure_gains_pass_through() -> None:
    ...
    net_st, net_lt, offset, carry_out = _net_capital_gains_jnp(_cents(4_000.0), _cents(10_000.0), _cents(0.0))
...
def test_net_capital_gains_short_term_loss_offsets_long_term_gain() -> None:
    ...
    net_st, net_lt, offset, carry_out = _net_capital_gains_jnp(_cents(-3_000.0), _cents(10_000.0), _cents(0.0))
...
def test_net_capital_gains_net_loss_caps_ordinary_offset_and_carries_remainder() -> None:
    ...
    net_st, net_lt, offset, carry_out = _net_capital_gains_jnp(_cents(-5_000.0), _cents(-7_000.0), _cents(0.0))
    assert net_st.tolist() == [0]
...
def test_net_capital_gains_small_loss_fully_offsets_ordinary() -> None:
    ...
    _, _, offset, carry_out = _net_capital_gains_jnp(_cents(-1_200.0), _cents(0.0), _cents(0.0))
...
def test_net_capital_gains_carryforward_consumes_following_year_gain() -> None:
    ...
    net_st, net_lt, offset, carry_out = _net_capital_gains_jnp(_cents(2_000.0), _cents(5_000.0), _cents(9_000.0))
...
def test_net_capital_gains_vectorized_independent_rollouts() -> None:
    ...
    net_st, net_lt, offset, carry_out = _net_capital_gains_jnp(
        _cents(4_000.0, 1_000.0, -2_000.0), _cents(10_000.0, 1_000.0, -6_000.0), _cents(0.0, 0.0, 0.0)
    )
...
def test_tax_math_is_traceable() -> None:
    ...
    ordinary_tax = jax.jit(lambda amount: _apply_brackets(amount, **ordinary_table))(_cents(50_000.0))
    ltcg_tax = jax.jit(lambda gain, ordinary: _apply_ltcg_brackets(gain, ordinary, **ltcg_table))(
        _cents(20_000.0), _cents(30_000.0)
    )
    _, _, offset, _ = jax.jit(_net_capital_gains_jnp)(_cents(-5_000.0), _cents(-7_000.0), _cents(0.0))
