...
# `sim/data/jurisdictions/federal_us.yaml`, single filer, in currency quanta. An engine reaches
...
def _long_term_lot(*, cost_basis_per_unit: Decimal) -> InitialLot:
    return InitialLot(
        lot_id="alice-vti",
        agent_id="alice",
        account_id="checking",
...
def _sell_it_all() -> ScheduledAssetSale:
...
def gain_below_the_deduction_case(*, wages: Decimal = Decimal(0)) -> Case:
    ...
    return Case(
        scenario=scenario(
            ...
            initial_lots=[_long_term_lot(cost_basis_per_unit=LOT_BASIS)],
            scheduled_asset_sales=[_sell_it_all()],
...
def realized_loss_case(*, loss: Decimal) -> Case:
    ...
    return Case(
        scenario=scenario(
            ...
            horizon_months=HORIZON_MONTHS,
            initial_lots=[_long_term_lot(cost_basis_per_unit=LOSS_SALE_PRICE + loss)],
            scheduled_asset_sales=[_sell_it_all()],
            tax_profiles=[taxed("alice", "federal_us")],
        ),
        rollout_count=1,
        series={VTI: levels([[LOSS_SALE_PRICE] * (HORIZON_MONTHS + 1)])},
    )


class TaxStatuteAcceptance:
    ...
    def test_the_case_really_is_a_bare_gain_against_a_full_deduction(self, engine: Engine) -> None:
        """The premise of the test below.

        Without this, a tax amount could be right or wrong for a reason having nothing to do
        with the rule under test — a gain that came out short-term, say, or ordinary income
        leaking in from somewhere.
        """

        breakdown = engine.events(gain_below_the_deduction_case().compiled_run).tax_breakdowns
    ...
    def test_an_unused_standard_deduction_shelters_a_long_term_gain(self, engine: Engine) -> None:
        ...
        accruals = engine.events(gain_below_the_deduction_case().compiled_run).tax_accruals
    ...
    def test_a_capital_loss_offsets_ordinary_income_only_up_to_the_1211_cap(
        self, engine: Engine, loss: Decimal, offset_quanta: int
    ) -> None:
        ...
        breakdown = engine.events(realized_loss_case(loss=loss).compiled_run).tax_breakdowns
        assert [row["ordinary_income_quanta"] for row in breakdown.to_dicts()] == [-offset_quanta]

    def test_the_stacking_case_really_is_wages_beside_a_gain(self, engine: Engine) -> None:
        """The premise of the test below: a tax figure can be right for an unrelated reason."""

        breakdown = engine.events(gain_below_the_deduction_case(wages=WAGES).compiled_run).tax_breakdowns
        assert breakdown.height == 1, "one jurisdiction, one tax year"
        row = breakdown.to_dicts()[0]
        assert row["standard_deduction_quanta"] == FEDERAL_STANDARD_DEDUCTION_QUANTA
    ...
    def test_a_long_term_gain_is_rated_from_where_ordinary_income_leaves_off(self, engine: Engine) -> None:
        ...
        accruals = engine.events(gain_below_the_deduction_case(wages=WAGES).compiled_run).tax_accruals
