"""What a simulator does with a scenario, stated so any engine can be held to it.

These are the behavioural cases the simulator grew up with: transfers and their windows,
FIFO sales and the lots they consume, indexed amounts, year-end assessment and payment,
obligations and the band that funds them, a financed property and what holding it costs, and
what a rollout does when it runs out of money.

They read the channels every engine answers in rather than one engine's buffers, so a second
implementation is held to the same answers instead of to a second description of them.
"""
...
def _lots(result: SimulationResult) -> pl.DataFrame:
    """The lot channel with units restored from quanta.

    The channel carries the integer the engines actually hold plus the scale that divides it;
    these cases were written in units, and the division is the same one either engine's
    reader would do.
    """

    return result.lots.with_columns(
        (pl.col("remaining_quantity_quanta") / pl.col("quantity_scale")).alias("remaining_quantity")
    )
...
class TransferAcceptance:
    """Money moves only where a scenario says it does, and every rollout sees the same moves."""

    def test_alice_gives_bob_five_dollars_one_rollout(self, backend: Backend, alice_bob_scenario: Scenario) -> None:
        ...
        result = backend(sampled(alice_bob_scenario, rollout_count=1, locations={}))

        initial = result.cash.filter(pl.col("month_index") == 0).sort("agent_id")
        assert initial.get_column("balance_quanta").map_elements(quanta_to_usd, return_dtype=pl.Float64).to_list() == [
            10.0,
            20.0,
        ]

        post = result.cash.filter(pl.col("month_index") == 1).sort("agent_id")
        ...
        totals = (
            result.cash.group_by("month_index")
        ...
        assert result.events.transfers.height == 1
        txn = result.events.transfers.row(0, named=True)
        assert txn["from_agent_id"] == "bob"
        assert txn["to_agent_id"] == "alice"
        assert txn["amount_quanta"] / 100 == 5.0
        assert txn["month_index"] == 0

    def test_no_scheduled_transfers_leaves_balances_unchanged(self, backend: Backend) -> None:
        ...
        result = backend(sampled(scenario, rollout_count=1, locations={}))

        # Six rows: initial month 0 through end-of-horizon month 5.
        assert result.cash.height == 6
        assert (
            result.cash.get_column("balance_quanta").map_elements(quanta_to_usd, return_dtype=pl.Float64).to_list()
            == [100.0] * 6
        )
        assert result.events.transfers.is_empty()

    def test_rejects_zero_rollout_count(self, backend: Backend, alice_bob_scenario: Scenario) -> None:
        with pytest.raises(ValueError, match="rollout_count must be positive"):
            backend(sampled(alice_bob_scenario, rollout_count=0, locations={}))

    def test_recurring_paycheck_accrues_monthly(self, backend: Backend) -> None:
        ...
        alice_final = (
            result.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 12))
        ...
        payroll_final = (
            result.cash.filter((pl.col("agent_id") == "payroll") & (pl.col("month_index") == 12))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
        ...
        assert result.events.transfers.height == 12
        assert set(result.events.transfers.get_column("month_index").to_list()) == set(range(12))
        assert set(result.events.transfers.get_column("cause_id").unique().to_list()) == {"alice_paycheck"}

    def test_recurring_transfer_bounded_by_end_month(self, backend: Backend) -> None:
        ...
        result = backend(sampled(scenario, rollout_count=1, locations={}))
        assert result.events.transfers.height == 5  # months 0..4

        # Alice's balance plateaus at 500.0 from month 5 onward.
        balances = (
            result.cash.filter(pl.col("agent_id") == "alice")
            .sort("month_index")
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .to_list()
    ...
    def test_one_thousand_rollouts_identical_when_inputs_are(self, backend: Backend) -> None:
        ...
        result = backend(sampled(scenario, rollout_count=rollout_count, locations={}))
        ...
        assert result.events.transfers.height == rollout_count * 24
    ...
    def test_combined_one_off_and_recurring(self, backend: Backend) -> None:
        ...
        assert result.events.transfers.height == 11

        # Alice at end-of-horizon: 10 × $1000 paychecks + $5000 bonus = $15000.
        alice_final = (
            result.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 10))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
...
class AssetSaleAcceptance:
    """A sale consumes the lots it names, in the order FIFO fixes, at the price its rollout saw."""

    def test_initial_lot_partial_sale_consumes_units_credits_proceeds(
        self, backend: Backend, constant_price_bundle
    ) -> None:
        ...
        lots_at_m3 = _lots(result).filter(pl.col("month_index") == 3)
        assert lots_at_m3.get_column("remaining_quantity").to_list() == [100.0]

        # Post-sale: month 4 onward, 70 units remain.
        for month in (4, 5, 6):
            snapshot = _lots(result).filter(pl.col("month_index") == month)
        ...
        assert result.events.lot_dispositions.height == 1
        disp = result.events.lot_dispositions.row(0, named=True)
    ...
    def test_initial_lot_full_sale_zeros_remaining_quantity(self, backend: Backend, constant_price_bundle) -> None:
        ...
        remaining_after = _lots(result).filter(pl.col("month_index") == 3).get_column("remaining_quantity").item()
    ...
    def test_asset_sale_scales_across_rollouts(self, backend: Backend, constant_price_bundle) -> None:
        ...
        assert result.events.lot_dispositions.height == rollout_count
        # Every rollout's lot row at end-of-horizon has 30 units remaining.
        end_state = _lots(result).filter(pl.col("month_index") == 2)
        assert end_state.height == rollout_count
        assert end_state.get_column("remaining_quantity").unique().to_list() == [30.0]

    def test_fifo_sale_crossing_two_lots(self, backend: Backend, constant_price_bundle) -> None:
        ...
        assert result.events.lot_dispositions.height == 2
        by_lot = {
            row["lot_id"]: row
            for row in result.events.lot_dispositions.sort("purchase_month_index").iter_rows(named=True)
        }
        ...
        assert (
            result.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 9))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
            == 24000.0
    ...
    def test_same_month_scheduled_sales_consume_lots_sequentially(
        self, backend: Backend, constant_price_bundle
    ) -> None:
        scenario = Scenario(
            agents=[Agent(agent_id="alice")],
        ...
        dispositions = result.events.lot_dispositions.sort(["cause_id", "purchase_month_index"])
        ...
        end_lots = _lots(result).filter(pl.col("month_index") == 2).sort("lot_id")
        ...
        final_cash = (
            result.cash.filter(pl.col("month_index") == 2)
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
    ...
    def test_fifo_holding_period_classification_per_disposition(self, backend: Backend, constant_price_bundle) -> None:
        ...
        result = backend(sampled(scenario, rollout_count=1, locations={}))
        dispositions = result.events.lot_dispositions.with_columns(
    ...
    def test_sales_of_two_different_assets_are_independent(self, backend: Backend, constant_price_bundle) -> None:
        ...
        end_lots = _lots(result).filter(pl.col("month_index") == 6).sort("lot_id")
        ...
        assert (
            result.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 6))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
            == 1350.0
    ...
    def test_scheduled_sale_consumes_only_source_account_fifo_pool(
        self, backend: Backend, constant_price_bundle
    ) -> None:
        scenario = Scenario(
            agents=[Agent(agent_id="alice")],
        ...
        disposition = result.events.lot_dispositions.row(0, named=True)
    ...
    def test_scheduled_sale_oversell_raises_without_partial_disposition(
        self, backend: Backend, constant_price_bundle
    ) -> None:
        scenario = Scenario(
            agents=[Agent(agent_id="alice")],
    ...
    def test_series_driven_sale_uses_deterministic_price_curve(
        self, backend: Backend, deterministic_series_bundle
    ) -> None:
    ...
    def test_gbm_series_diverges_across_rollouts_same_seed_is_reproducible(self, backend: Backend) -> None:
        ...
        result_a = backend(sampled(scenario, rollout_count=200, locations={}))
        result_b = backend(sampled(scenario, rollout_count=200, locations={}))

        # Reproducibility: the same seed vector produces the same run, end to end. Stated on
        # the cash the sales produced rather than on the sampled paths, because what has to be
        # reproducible is the answer and not an intermediate an engine need not expose.
        assert result_a.cash.equals(result_b.cash)
        ...
        cash_at_end = (
            result_a.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 6))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
        )
...
class YearEndTaxAcceptance:
    ...
    def test_year_end_tax_accrual_federal_and_california_single_filer(self, backend: Backend) -> None:
        ...
        assert result.events.transfers.filter(pl.col("income_category") == "ordinary").height == 12

        # Two tax accruals at month 11 — federal + CA — for one rollout.
        accruals = result.events.tax_accruals.sort("jurisdiction_id")
        ...
        # paychecks.
        ytd_alice = result.income.filter(pl.col("agent_id") == "alice").sort("month_index")
        ytd_values = (
            ytd_alice.get_column("income_quanta").map_elements(quanta_to_usd, return_dtype=pl.Float64).to_list()
        )
        assert ytd_values[11] == pytest.approx(11 * _engine_usd(200_000.0 / 12.0), abs=0.02)
        assert ytd_values[12] == 0.0

    def test_year_end_tax_includes_long_term_capital_gain_under_federal_ltcg_schedule(
        self, backend: Backend, constant_price_bundle
    ) -> None:
        ...
        accruals = {row["jurisdiction_id"]: row for row in result.events.tax_accruals.iter_rows(named=True)}
        ...
        cg_at_month_11 = result.capital_gains.filter((pl.col("month_index") == 11) & (pl.col("agent_id") == "alice"))
    ...
    def test_e2e_pinned_ltcg_tax_safe_harbor_and_cash_numerics(self, backend: Backend, constant_price_bundle) -> None:
        ...
        tax_payments = result.events.transfers.filter(pl.col("cause_id").str.contains("tax")).sort(
        ...
        tax_settlement = result.events.tax_settlements.row(0, named=True)
        ...
        final_cash = (
            result.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 13))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
        ...
        final_lot = _lots(result).filter((pl.col("lot_id") == "alice_long_vti") & (pl.col("month_index") == 13))
        assert final_lot.get_column("remaining_quantity").item() == 0.0

    def test_e2e_pinned_multi_asset_ltcg_stcg_tax_breakdown_numerics(
        self, backend: Backend, constant_price_bundle
    ) -> None:
        ...
        accrual = result.events.tax_accruals.row(0, named=True)
        assert accrual["amount_quanta"] / 100 == pytest.approx(4_196.0, abs=0.01)
        breakdown = result.events.tax_breakdowns.row(0, named=True)
        ...
        gains = {
            ...
            for row in result.capital_gains.filter((pl.col("month_index") == 11) & (pl.col("agent_id") == "alice"))
            .sort("classification")
            .iter_rows(named=True)
        }
        assert gains == {"ltcg": pytest.approx(10_000.0), "stcg": pytest.approx(1_500.0)}

    def test_e2e_pinned_tax_payments_force_asset_liquidation_and_settle_liability(
        self, backend: Backend, deterministic_series_bundle
    ) -> None:
        ...
        assert result.events.tax_settlements.get_column("amount_quanta").map_elements(
            quanta_to_usd, return_dtype=pl.Float64
        ).sum() == pytest.approx(4_016.0, abs=0.01)

        policy_sales = result.events.lot_dispositions.filter(pl.col("cause_id").str.starts_with("allocation_sale"))
    ...
    def test_explicit_empty_tax_profiles_means_no_year_end_accrual(self, backend: Backend) -> None:
        ...
        result = backend(sampled(scenario, rollout_count=1, locations={}))
        assert result.events.tax_accruals.is_empty()
        assert result.tax_liabilities.is_empty()

    def test_year_end_tax_payment_debits_agent_cash(self, backend: Backend) -> None:
        ...
        tax_payments = result.events.transfers.filter(pl.col("cause_id").str.contains("tax"))
        ...
        assert result.events.tax_settlements.height == 1
        settlement = result.events.tax_settlements.row(0, named=True)
        ...
        irs_end_cash = (
            result.cash.filter((pl.col("agent_id") == "irs") & (pl.col("month_index") == 13))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
    ...
    def test_tax_payment_can_trigger_rollout_failure_when_unfunded(self, backend: Backend) -> None:
        ...
        failures = result.events.rollout_failures
...
class ObligationAcceptance:
    """An obligation is settled from the account that owes it, or it is not settled at all."""

    def test_due_now_obligation_sells_assets_and_settles(self, backend: Backend, deterministic_series_bundle) -> None:
        ...
        accrual = result.events.obligation_accruals.row(0, named=True)
        assert accrual["obligation_id"] == "rent_due_m0"
        assert accrual["amount_due_quanta"] / 100 == pytest.approx(500.0)

        settlement = result.events.obligation_settlements.row(0, named=True)
        ...
        funding_sale = result.events.lot_dispositions.row(0, named=True)
        ...
        final_cash = (
            result.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 1))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
    ...
    def test_due_now_obligation_failure_aborts_payment(self, backend: Backend) -> None:
        ...
        failure = result.events.rollout_failures.row(0, named=True)
    ...
    def test_liquidity_policy_sale_uses_rollout_specific_prices(self, backend: Backend) -> None:
        scenario = Scenario(
            agents=[Agent(agent_id="alice"), Agent(agent_id="landlord")],
            initial_cash=[
                InitialAccountBalance(agent_id="alice", account_id="checking", balance=0),
    ...
    def test_liquidity_policy_consumes_only_policy_account_fifo_pool(
        self, backend: Backend, deterministic_series_bundle
    ) -> None:
        scenario = Scenario(
            agents=[Agent(agent_id="alice"), Agent(agent_id="landlord")],
        ...
        end_lots = _lots(result).filter(pl.col("month_index") == 1).sort("lot_id")
        assert end_lots.select("lot_id", "account_id", "remaining_quantity").to_dicts() == [
    ...
    def test_liquidity_policy_can_sell_from_source_account_into_cash_account(
        self, backend: Backend, deterministic_series_bundle
    ) -> None:
        scenario = Scenario(
            agents=[Agent(agent_id="alice"), Agent(agent_id="landlord")],
    ...
    def test_liquidity_policy_covers_monthly_spend_deficit(self, backend: Backend, deterministic_series_bundle) -> None:
        ...
        m0_dispositions = result.events.lot_dispositions.filter(pl.col("month_index") == 0)
        ...
        end_cash = (
            result.cash.filter((pl.col("agent_id") == "alice") & (pl.col("month_index") == 3))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
    ...
    def test_policy_without_sale_orders_fails_hard_demand_even_with_assets(
        self, backend: Backend, deterministic_series_bundle
    ) -> None:
        ...
        assert settlement["amount_paid_quanta"] / 100 == pytest.approx(0.0)
        assert settlement["shortfall_quanta"] / 100 == pytest.approx(500.0)
        assert result.events.rollout_failures.height == 1

    def test_the_band_is_measured_after_the_months_hard_demands(
        self, backend: Backend, deterministic_series_bundle
    ) -> None:
        ...
        sale = result.events.lot_dispositions.row(0, named=True)
    ...
    def test_the_band_does_not_fire_when_the_projected_balance_clears_the_floor(
        self, backend: Backend, deterministic_series_bundle
    ) -> None:
    ...
    def test_a_band_it_cannot_refill_does_not_fail_the_rollout(self, backend: Backend) -> None:
        ...
        scenario = Scenario(
            ...
            # The sleeve is priced; what makes the raise come back empty is that Alice holds no
            # units of it, not that the asset is unpriceable.
            external_series=SeriesModelBundle.independent(
                asset_prices=AssetPriceGroups(security={SecuritySymbol("vti"): Constant(value=100.0)})
            ),
            target_allocation_policies=[
                TargetAllocationPolicy(
                    agent_id="alice",
    ...
    def test_same_account_hard_demands_settle_all_or_none(self, backend: Backend) -> None:
        scenario = Scenario(
        ...
        settlements = result.events.obligation_settlements.sort("obligation_id")
        ...
        assert result.events.transfers.is_empty()
        assert result.events.rollout_failures.height == 2


class PropertyCarryingCostAcceptance:
    """A purchase creates the property, its financing, and the recurring costs of holding it."""

    def test_real_estate_purchase_mortgage_and_property_tax_numerics(
        self, backend: Backend, san_francisco_location: Location
    ) -> None:
        ...
        result = backend(sampled(scenario, rollout_count=1, locations={"san_francisco": san_francisco_location}))

        final_property = result.properties.filter(pl.col("month_index") == 2).row(0, named=True)
        ...
        final_cash = (
            result.cash.filter((pl.col("month_index") == 2) & (pl.col("agent_id") == "alice"))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
        ...
        assert result.events.property_purchases.height == 1
        assert result.events.mortgage_originations.height == 1
        assert result.events.mortgage_payments.height == 1
        assert result.events.transfers.filter(pl.col("cause_id") == "sf_home_property_tax_m1").height == 1

    def test_real_estate_purchase_requires_known_location(
        self, backend: Backend, san_francisco_location: Location
    ) -> None:
        scenario = Scenario(
        ...
        with pytest.raises(
            ...
            backend(sampled(scenario, rollout_count=1, locations={"san_francisco": san_francisco_location}))

    def test_property_tax_falls_back_to_location_rate_when_policy_rate_unset(
        self, backend: Backend, san_francisco_location: Location
    ) -> None:
        ...
        sf_tax = (
            result.cash.filter((pl.col("month_index") == 2) & (pl.col("agent_id") == "sf_tax_collector"))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
    ...
    def test_property_tax_routes_flat_usd_special_assessment_from_location(
        self, backend: Backend, vallejo_mare_island_location: Location
    ) -> None:
        ...
        result = backend(
            sampled(scenario, rollout_count=1, locations={"vallejo_mare_island": vallejo_mare_island_location})
        )
        ...
        tax_collected = (
            result.cash.filter((pl.col("month_index") == 2) & (pl.col("agent_id") == "vallejo_tax_collector"))
            .get_column("balance_quanta")
            .map_elements(quanta_to_usd, return_dtype=pl.Float64)
            .item()
        )
...
class RolloutFailureAcceptance:
    """A rollout that cannot pay stops, and says where."""

    def test_rollout_marked_failed_when_assets_exhausted(self, backend: Backend, deterministic_series_bundle) -> None:
