...
_SCALE = np.asarray([100, 100, 100], dtype=np.int64)
...
def _view(*, month: int = 6, cash_slots: tuple[int, ...] = (0, 1), lot_slots: tuple[int, ...] = (0, 1)):
    return build_actor_view(
        ...
        lot_unit_price_cents=jnp.asarray([[100, 200], [300, 400], [5, 5]], dtype=jnp.int64),
...
def test_lots_are_marked_not_held_at_cost() -> None:
    """A policy reasoning about allocation needs what a sleeve is worth now. Lot 0 holds
    500 quanta at scale 100 — 5 units — priced at 100 cents, so 500 cents. Basis is 50,
    so a view that reported cost would say 250 and the allocation would be wrong by half."""

    view = _view()

    assert [[int(x) for x in row] for row in view.lot_value_cents] == [[500, 2_000], [600, 1_600]]
...
def test_the_view_is_a_function_of_this_month_only() -> None:
    """No clairvoyance. The builder takes marks already resolved for `month`; nothing it
    computes reaches into a later index. Advancing the month with identical state changes
    only the holding period, never a value — if a price for a later month could leak in,
