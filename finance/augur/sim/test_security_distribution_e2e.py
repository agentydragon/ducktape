...
def _cash_by_month(*, tax_character: tuple[DistributionTaxSlice, ...], distributes: bool = True) -> dict[int, int]:
...
def test_the_payout_is_units_times_dollars_per_unit_every_month() -> None:
    ...
    assert deltas == dict.fromkeys(range(_HORIZON), 200_000)
...
def test_a_holding_with_no_declared_distribution_pays_nothing() -> None:
    ...
    assert set(_cash_by_month(tax_character=_TREASURY, distributes=False).values()) == {0}
