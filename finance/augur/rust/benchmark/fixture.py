"""The deterministic feature-rich scenario both engines benchmark and differ against.

Authored as a `Scenario` and its sampled paths, so the Rust fixture is the encoding of the
very plan the JAX engine runs — see `differential/case.py` for why that direction is the
...
def write_fixture(path: Path, *, rollout_count: int, horizon_months: int) -> None:
    """The integer document the Rust simulator reads, for the standalone benchmark binary."""

    with path.open("w") as file:
        json.dump(
            feature_rich_case(rollout_count=rollout_count, horizon_months=horizon_months).fixture,
            file,
            separators=(",", ":"),
        )
