...
from finance.evidence.loading import evidence_dir_from_env, read_monthly_levels
...
def monthly_levels_by_wire() -> dict[str, dict[str, float]]:
    ...
    return {
        wire: {
            level.month.strftime("%Y-%m"): level.value for level in read_monthly_levels(evidence_dir_from_env(), source)
        }
