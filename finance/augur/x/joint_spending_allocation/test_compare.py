...
from finance.augur.x.joint_spending_allocation.compare import measurements
...
def test_same_paths_and_reserves_fund_bills_and_chosen_consumption(results: Path) -> None:
    ...
    for cell in manifest["cells"]:
        output = json.loads((results / f"{cell['name']}.json").read_text())
        ...
        for rollout, report in zip(output["rollouts"], output["measurements"]["paths"], strict=True):
            ...
            assert (
                opening["intended_consumption"]
                == opening["consumption_requested"]
                == opening["consumption_paid"]
                == expected
            )
...
def test_both_policy_dimensions_change_the_joint_financial_result(results: Path) -> None:
    fixed = json.loads((results / "r800-fixed_real-constant.json").read_text())["measurements"]["paths"]
    bounded = json.loads((results / "r800-bounded-constant.json").read_text())["measurements"]["paths"]
    glide = json.loads((results / "r800-bounded-glide.json").read_text())["measurements"]["paths"]
    ...
    assert bounded[1]["months"][12]["intended_consumption"] > fixed[1]["months"][12]["intended_consumption"]
...
def test_selected_original_ids_replay_identical_intentions_and_financial_prefixes(results: Path) -> None:
    manifest = json.loads((results / "experiment.json").read_text())
    for cell in manifest["cells"]:
        compact = json.loads((results / f"{cell['name']}.json").read_text())
        traces = json.loads((results / f"{cell['name']}-traces.json").read_text())
...
def test_tax_free_control_keeps_tax_payments_separate_from_consumption() -> None:
    ...
    assert [row["consumption_paid"] for row in taxed["months"]] == [
        row["consumption_paid"] for row in untaxed["months"]
    ]
...
def test_exhaustion_distinguishes_rejected_consumption_from_unattempted_intention(bill: int) -> None:
    ...
    financial = output["rollouts"][0]["trace"]["financial"]
    assert sum(row["proceeds"] for row in financial["dispositions"]) == 10_000_000
    assert sum(row["basis"] for row in financial["dispositions"]) == 8_000_000
