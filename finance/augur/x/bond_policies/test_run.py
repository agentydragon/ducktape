...
from finance.augur.x.bond_policies.run import compile_construction, execute, measurements
...
def test_zero_withdrawals_preserve_units_and_pay_the_last_in_horizon_coupon(
    ...
    assert financial["dispositions"] == financial["obligations"] == financial["tax_accruals"] == []
    ...
    assert financial["months"][-1]["lots"][0]["units_remaining"] == 1_000_000_000
...
def test_current_coupon_funds_spending_before_units_are_sold(controlled_construction: ProxyConstruction) -> None:
    ...
    result = execute(run, rollout_ids=[0], capture="forensic")[0]
    financial = result["trace"]["financial"]
    sales = financial["dispositions"]
    assert [row["month"] for row in sales] == [12, 24, 36, 48, 60, 72]
    assert sales[0]["proceeds"] == sales[0]["basis"] == 300_000
    # After the first sale, 970 units earn $1,940; the second withdrawal needs $3,060.
    assert sales[1]["proceeds"] == sales[1]["basis"] == 306_000
    assert [next(iter(row["action"])) for row in result["trace"]["receipts"]] == ["Sell", "PayClaim"] * 6
    assert measurements(result)["spending_paid_quanta"] == 3_000_000
    assert result["stop"] is None
    assert financial["tax_accruals"] == []
...
def test_zero_yield_proxy_funds_withdrawals_from_principal_without_coupon_income() -> None:
    ...
    assert [row["proceeds"] for row in result["trace"]["financial"]["dispositions"]] == [500_000] * 6
    assert measurements(result)["spending_paid_quanta"] == 3_000_000
...
def test_internal_sale_or_redemption_stays_in_nav_not_household_income(
    ...
    summary = result["summary"]
    financial = result["trace"]["financial"]
    assert financial["dispositions"] == []  # Internal bond trades are not household unit sales.
...
def test_nondivisible_unit_sale_keeps_exact_proceeds_and_basis() -> None:
    ...
    assert (sale["proceeds"], sale["basis"], sale["realized_gain"]) == (100, 3333, -3233)
...
def test_cli_exports_inputs_compact_results_and_selected_original_timelines(tmp_path: Path) -> None:
    ...
    reports = json.loads((output / "summary.json").read_text())
    ...
    for cell in reports:
        ...
        results = json.loads((cell_dir / "rollouts.json").read_text())
        assert [row["rollout_id"] for row in results] == list(range(5))
        assert all(row["trace"] is None for row in results)
        original = results[cell["rollout_id"]]
        assert cell["path"] == config["path_names"][cell["rollout_id"]]
        assert measurements(original).items() <= cell.items()
        if cell["annual_spending_usd"] == "5000":
            assert cell["stop"] is None
            assert cell["spending_paid_quanta"] == 3_000_000
        else:
            assert cell["stop"] is not None
            assert cell["spending_paid_quanta"] == 0
            assert cell["ending_mark_month"] == 12
            assert cell["terminal_wealth_quanta"] is None
        assert original["summary"]["tax_accruals"] == []
        traces = json.loads((cell_dir / "traces.json").read_text())
        ...
        for replay in traces:
            ...
            for entry in replay["trace"]["financial"]["journal"]:
