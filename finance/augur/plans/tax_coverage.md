...
checked on devel 2026-09-26. This is not a certification of current tax fidelity
...
| Opening tax state                 | `TaxProfile` supplies an aggregate `prior_year_tax`; `TaxBook.enroll` (`sim/tax_year.py`) starts income, gain/carryover and payment state empty. No opening YTD/carryover import surface.                                                                | Same continuation started before versus after a synthetic taxable sale, including earlier payments and a brought-forward loss; no tax reset or duplicated sale.                                                                                  |
