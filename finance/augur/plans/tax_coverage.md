...
| Equity/fund distributions         | `SecurityDistribution` slices name their income source: interest by issuer, ordinary, or qualified dividends (federal long-term capital-gain rates, ordinary in California; `sim/qualified_dividends_test.py`). A qualified declaration is trusted: no holding-period test. The year-end breakdown does not itemize qualified dividends, and the app's deployment config declares interest slices only. Equity samplers emit total-return prices. Distinct fund capital-gain/return-of-capital processing is absent. | Compatible price-return plus payout amounts for the equity funds in scope, the qualified-dividend holding-period test (or its explicit exclusion per product), qualified dividends itemized in the year-end breakdown and declarable in deployment config, reinvestment basis and no double-counted return. Either implement other payout kinds these funds need or reject those products. |
...
| High-income taxes                 | `PreparedTaxRules` has no NIIT fields; the California YAML explicitly defers its additional high-income tax.                                                                                                                                                                                                                                                                                                                                                                                                         | Single-filer NIIT below/across/above the threshold, with muni interest excluded and qualified dividends included; California's 1% surcharge above $1M (current California law). These are acceptance blockers for affected reports, not optional “later refinements.”                                                                                                                      |
...
2. **Distribution slice:** supported price/payout construction, the
   qualified-dividend holding-period test and basis after reinvestment.
   Coordinate with BIND; the tax-free proxy remains useful only under its
   declared assumption.
3. **High-income slice:** independently test NIIT and California's 1%
   surcharge. It need not wait for the whole instrument refactor.
4. **Payment slice:** per-jurisdiction estimated-payment schedules and the
   ...
   an oracle.
5. **Mortgage-interest slice:** independently test the single-filer federal and
