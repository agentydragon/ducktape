...
example. LANG adopts the Python bridge with the scoped POL consumer if GL supports
it; a rejected probe leaves
...
remain decisions for BATCH/GL. GP now resolves scoped timing/execution choices,
...
| Consumption is converted into `ActiveObligation`; chosen spend and existing promises share an all-or-none funding group.                                                                             | Distinguish consumption requests, due claims, payment actions and actual receipts. Contracts generate claims; actors choose how to fund/pay them. Keep current grouping as a named control, not the target API. | OUT, POL, HOUSE; gate GP   |
...
flowchart TB
    ...
    POL -- actor-action consumer --> LANG
```
...
extension. LANG consumes a scoped POL actor-action example before calling the new
policy boundary supported; the STEP/BATCH parity probe does not. Joint callback
capture consumes POL only when introduced. Experiments
...
that boundary. POL moves strategy into optional policy-callable helpers; GP and
...
books stay in Rust. BATCH/GL compares scalar-policy adaptation with batch-native
functions; the public callable shape and batch representation remain open.

STEP extracts retained state and internal stepping from `simulate_rollout`;
the native full-horizon driver uses the same machinery. BATCH ports the existing
...
allocator to an opening callback would miss later cashflows and claims; POL must
...
rework. LANG promotes the bridge with a scoped POL actor-action consumer, extending
...
| POL    | After scoped GP/OBS choices, implement the actor observation/action/result loop for a public-security funding/payment consumer. Move cash-band, withdrawal, lot-selection and rebalance strategy into optional callable helpers; retain execution, tax and payment mechanics in the executor. Native helpers may be exposed to Python.                                                      | Bill → requested sale → available funds → actor payment reconciles. Two policies compose or omit a helper; no hidden engine funding/rebalance after their actions. Request/result identity drives actor/path-local memory. Explicit rejection/deadline behavior; no general recovery model. Joint spending/allocation varies without settlement edits. Full exits are EXIT; compact capture is CAP.                      |
...
| STEP   | Extract Rust-owned in-memory rollout state and internal stepping. Route the existing native full-horizon driver through it; do not promote its current phases into the public actor API.                                                                                                                                                                                                    | Native spending/allocation, housing, tax and stopped-book controls retain decisions, receipts, snapshots and timing. Cross-year/payment advances retain lots, pending contracts/liabilities and recorder state without reinitialization. No checkpoint format.                                                                                                                                                           |
| BATCH  | Through the existing extension, run the bounded-spending control over STEP; compare scalar-policy adaptation and batch-native functions. Preserve current observations, integer requests and capture; expose actual component receipts and path status. This probes language cost, not the completed actor interface.                                                                       | Native/Python parity for requests, paid consumption, trades, taxes, balances and failures. No future observations or cross-path information use; isolated memory/randomness; reordered/chunked and selected replay parity; no post-stop calls. Invalid inputs and policy exceptions fail explicitly. Publish workload, authoring/layout comparison and profiling for GL.                                                 |
...
| LANG   | If GL chooses adoption, promote the measured bridge with POL's scoped actor-action consumer; migrate affected callers and language/API docs atomically. Add BOND/HOUSE actions only when their semantics exist.                                                                                                                                                                             | Ordinary Python policies issue economic actions and may call sleeve helpers; one canonical executor supplies consequences. Extend parity/error/replay controls and agreed cost budgets to the action consumer. No engine-imposed investment strategy, policy registry, financial DSL or parallel engine.                                                                                                                 |
...
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GP — scoped information and action semantics       | The actor-observation/action boundary is settled in the interface plan. Pin the first consumer's information/review times, known contract/tax facts, same-time ordering, execution versus available funds, request/result memory and rejection/deadline/stop behavior. Broader partial-payment/default/recovery models require explicit scope. Opening review/all-or-none settlement are probe controls, not the destination.                | STEP/BATCH, OUT and EXIT proceed against current controls. Non-mutating previews require a consumer and canonical calculations; no universal scheduler or optimizing counterparty is required.                                      |
...
| GL — adopt or revise the Python batch boundary?    | After BATCH, require parity; compare scalar-adapted and batch-native authoring, data layout, profiled throughput, transfer/capture cost and peak memory against agreed workloads/budgets. Select a supported shape via LANG with the scoped POL consumer, or identify a measured bottleneck and bounded rework.                                                                                                                              | Native POL/STUDY/RUN, CAP and financial corrections proceed. The probe does not freeze action semantics. Rejection leaves the authoring gap open; neither native-only policies nor a whole-engine rewrite follows automatically.    |
...
1. Start **STEP**, then **BATCH**, and resolve **GL** for the policy-authoring
   ...
   promote via **LANG** with the scoped **POL** actor-action consumer, or replan
   from the probe's evidence. **EXIT** and CAP's compact-allocation slice remain
   independently ready against separate callbacks. These slices and the first
   STEP/BATCH probe need no new joint-policy semantics or tax coverage.
2. Scope **OUT**'s cause-identity remainder.
   Run **GP**'s remaining timing/action choices, **GT**, and **GS** alongside
   implementation; the actor/environment ownership boundary is already settled.
