...
| TAX     | GT's core scope is decided (federal + California, single filer). Land separate supported-case changes: distribution character/qualified dividends (ordinary in California); NIIT and California's 1% surcharge; current-year tables held flat plus opening year-to-date facts; payment timing once GT's payment-timing item is decided.                                                                                                                                                                                                                                                                                                                                                                                     | Independently sourced annual-liability examples plus integrated sale-to-fund-spend, reinvestment basis, year-crossing, exemption and tax-payment tests. Compare with a second calculation, not a copy of the engine formula. Reject or exclude unimplemented cases explicitly. No second simulator or universal tax-law DSL.                                                                                                                                      |
...
The [tax-coverage checklist](tax_coverage.md) records GT's decided scope, its
open items and the evidence behind them. GP extends the settled monthly contract (<../SPEC.md>) without
...
| GT — what must be financially faithful first?      | Core scope decided: federal + California, single filer, US residence; equity funds, munis, direct bonds and managed TLH; qualified dividends and NIIT; current law held flat; current-month start with year-to-date facts. Still open: account treatment, other income/deductions and payment timing; for BOND, clean/dirty price, coupon/accrual dates, sale/redemption ordering, premium/discount tax treatment and unsupported TIPS/credit cases. Commit a public supported-case matrix with source/independent-oracle cases. The owner selects scope; implementation must not guess it. | Tax-free studies, supplied-curve valuation and interface cleanup. NIIT/qualified dividends cannot stay silently absent from a personal comparison that needs them. Future tax law must be an explicit assumption, not a prediction. |
...
2. **MA3** remains a runnable paired TLH comparison on the existing Python
   ...
   **TAX**'s distribution, high-income and calendar/opening-state slices; its
   payment slice waits on GT's payment-timing item. Scope GS and continue independent BIND/SCORE work. **domain composition** selects further domain
