...
Committed and still open: retire the configured housing and PE strategies. Keep useful
...
| P12     | Retire the remaining configured strategy readers, HOUSING's and PE's.                                                                                                                                                                     | HOUSING (after GHOUSE) and PE (after GPE), both deferred.                                                                                         |

Every COMPOSE slice and P12 waits on a deferred gate or is postponed. Keep older
financial capability branches scoped as below. No dependency is introduced merely
because files overlap or a rebase will be needed.
...
package should correct existing behavior or simplify its interfaces. Financial components such as the Python-owned TLH portfolio
...
| A total-return equity proxy can look like a taxable security, and no equity path is split into price return plus a qualified-dividend payout. | Explicit product bindings and supported distribution character; separate price return from payouts for taxed holdings.                                           | BIND, TAX    |
...
| Tax surface is narrower than the chosen scope: no effective-year schedule in `Jurisdiction`; no opening year-to-date facts.                   | Declared supported-case matrix, dated rules and opening tax state; unsupported relevant cases reject. Existing loss netting/carryforward is not reimplemented.   | GT, TAX      |
...
flowchart TB
    ...
    GT --> MIDYEAR["MIDYEAR: dated mid-year start with opening tax state"]
    MIDYEAR --> DATES["DATES: calendar dates in results, wire and frontend"]
...
their implementations are not backlog. The current app has one decision-making
...
The remaining configured consumers, HOUSING's and PE's, move to a tracked agent or
the action session, with optional Python proposal helpers; GP and HOUSE define the
scoped action/execution contracts. Domain changes serve an actual consumer.

For each, preserve its financial cadence and identify changes caused by post-claims
timing or explicit ordered actions. Its Python policy owns proposal ordering and
spending/tax reserves.
...
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P12 — retire configured controls and adapters | Move HOUSING's and PE's remaining configured readers onto ordinary policies submitting common actions. Land supported slices independently; extend the common action path only for capabilities existing callers require. Remove each configured record with its last reader. | HOUSING, PE (their readers only) | Every production caller's decisions come from an ordinary policy over the common actions; ported Python tests exercise canonical steps. Preserve existing financial capabilities and explicitly resolve phase differences; no silent behavior change or compatibility runner. No new adaptive housing, tax or market capability is implied. |
...
required actions and timing are supported. Keep any independent study recurrence as
a labeled mathematical control, not a second financial executor.

Completion requires the app's housing and PE decisions, like its funding, to come from
its household's ordinary policy over the same `World` and actions as the Python
experiments, not a configured strategy.
...
| CAP     | Add consumer-required actor/account/component financial observations using canonical records. Account/component selection and further trade/contract capture each need a named consumer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Preserve source scope, exact units and observed/stopped validity. No second accounting store, posting-template framework or metrics framework; app-specific recording stays in `product/`.                                                                                                                                                                                                                                                                        |
| BIND    | One binding kind: each held or purchasable security and each managed portfolio binds to one price series tagged **total-return** or **price-return**, plus an optional **payout** series. Composition rejects a payout series on a total-return price, and a price-return price without one unless the product is declared non-paying. Then supply equity price-return and payout paths with explicit historical/fitted payout assumptions, coordinated with TAX's distribution-character slice. Opening levels, relative path anchors and fixed contractual amounts stay distinct, with their anchoring explicit at composition, not inferred from source names. Reuse typed conditioning records and existing typed keys. | A total-return series cannot back a distributing holding, and a price-return series cannot silently drop its payouts: both reject before execution, as does incompatible tax character, while a declared non-paying price-return holding runs with zero payouts. Price plus payouts reconcile before tax, with timing and provenance. Product terms, construction assumptions and investor strategy have distinct owners; no global registry or universal fitter. |
| TAX     | GT's core scope is decided (federal + California, single filer). Land separate supported-case changes: distribution payouts and the qualified-dividend holding period; current-year tables held flat; per-jurisdiction estimated-payment schedules and the true-up, paid on schedule with no penalty modeling; the mortgage-interest deduction with the SALT cap's income phase-out.                                                                                                                                                                                                                                                                                                                                        | Independently sourced annual-liability examples plus integrated sale-to-fund-spend, reinvestment basis, year-crossing, exemption and tax-payment tests. Compare with a second calculation, not a copy of the engine formula. Reject or exclude unimplemented cases explicitly. No second simulator or universal tax-law DSL.                                                                                                                                      |
| MIDYEAR | A world starts at a named year and month rather than month 0 being an implicit January. Tax-year boundaries, estimated-payment months and the year close move behind one calendar helper in place of the `month % 12` sites; opening year-to-date income, realized gains, carryover and payments enter the first tax year. Month indices stay the clock for paths, lots, claims and policies.                                                                                                                                                                                                                                                                                                                               | The same continuation started in January and mid-year agrees on the overlapping tax years; a mid-year start with opening year-to-date facts neither drops nor double-counts earlier income; the first year-end, true-up and installments land on their calendar months.                                                                                                                                                                                           |
| DATES   | Results and the product wire carry the start date once; reports and the frontend derive calendar dates from month indices (axes, tables, selected-path detail), keeping years-from-start as a view. No per-event absolute dates.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | A January-start study and a mid-year app run both label calendar dates correctly; visual baselines updated for the relabelled axes.                                                                                                                                                                                                                                                                                                                               |
...
The [tax-coverage checklist](tax_coverage.md) records GT's decided scope and the
evidence behind it. GP extends the settled monthly contract (<../SPEC.md>) without
...
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GP — expanded information and settlement semantics | The first household example observes after cashflows/claim assembly, executes one ordered list, and stops on an invalid action or a still-unpaid due claim. Pin additional product/housing/PE settlement and deadlines, and ordering between multiple decision-making actors before migrating P12's affected existing consumers or enabling new cases.                                                                                                                                                                                                                                                                                                                                             | Public-portfolio experiments and the app's funding proceed. Funding helpers return proposals; no implicit allocator, retry protocol or recovery model. Existing capabilities cannot be dropped to claim convergence.                |
| GT — what must be financially faithful first?      | Scope decided: federal + California, single filer, US residence; equity funds, munis, direct bonds and managed TLH; qualified dividends and NIIT; the mortgage-interest deduction; no charitable deductions; current law held flat in nominal dollars; current-month start with year-to-date facts; estimated payments and true-up paid on schedule. Taxable accounts, FIFO lots, fees deferred. Still open: for BOND, clean/dirty price, coupon/accrual dates, sale/redemption ordering, premium/discount tax treatment and unsupported TIPS/credit cases. Commit a public supported-case matrix with source/independent-oracle cases. The owner selects scope; implementation must not guess it. | Tax-free studies, supplied-curve valuation and interface cleanup. NIIT/qualified dividends cannot stay silently absent from a personal comparison that needs them. Future tax law must be an explicit assumption, not a prediction. |
...
1. **COMPOSE**: the rollout axis is postponed until a real large-N workload needs it;
   ...
   **P12**'s remaining readers wait on HOUSING and PE.
2. **MA3** remains a runnable paired TLH comparison on the existing Python
   component/session. Continue **STUDY** consumers alongside cleanup. Dispatch
   **TAX**'s distribution, calendar/opening-state, payment and
   mortgage-interest slices. Scope GS and continue independent BIND/SCORE work. **domain composition** selects further domain
