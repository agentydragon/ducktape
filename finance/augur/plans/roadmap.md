...
| P12     | Retire configured strategy orchestration: the sim suites' configured household and its policy schemas.                                                                                                                                    | Continue independently landable slices on settled contracts; full P12 still needs HOUSING and PE for their readers.                               |
...
| The sim suites still fund through `policy/configured_household.py` and the configured `TargetAllocationPolicy`/`_AllocationPolicy` records.                      | Migrate consumers to common actions; remove configured policy schemas, lowering and orchestration with their last caller, reusing the shared Python helpers.     | P12; GP      |
