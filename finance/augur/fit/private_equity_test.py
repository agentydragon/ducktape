...
def test_runtime_scale_prior_penalizes_paths_above_soft_cap() -> None:
    def model(*, soft_cap_market_cap_usd: float) -> TrainedPrivateEquityModel:
        return TrainedPrivateEquityModel(
            artifact=TrainedPrivateEquityModelArtifact(
                issuer_id=PRIVATE_COMPANY_A,
                as_of_date=date(2026, 5, 27),
                current_mark_usd=100.0,
    ...
    loose = model(soft_cap_market_cap_usd=1_000_000_000_000.0)
    tight = model(soft_cap_market_cap_usd=1_000_000_000.0)
    request = ExogenousSamplingRequest(
...
def test_runtime_sampling_fails_on_nonfinite_private_equity_prices(
    ...
    with pytest.raises(ValueError, match="non-finite prices"):
        model.sample(
            ExogenousSamplingRequest(
                horizon_months=1, rollout_seeds=(1,), required_private_equity_issuers=frozenset({PRIVATE_COMPANY_A})
            )
        )
