...
async def test_pat_accepted_true_on_200_and_terminates_session() -> None:
    ...
    cfg = ResignerConfig(
        api_key="k", secret_namespace="test", secret_name="refresh-token", secret_key="refresh_token", pat="pat-123"
    )
...
async def test_pat_accepted_false_on_401() -> None:
    ...
    cfg = ResignerConfig(
        api_key="k", secret_namespace="test", secret_name="refresh-token", secret_key="refresh_token", pat="stale"
    )
    async with _client(handler) as http:
...
async def test_ready_uses_health_only_when_no_pat_configured() -> None:
    ...
    cfg = ResignerConfig(api_key="k", secret_namespace="test", secret_name="refresh-token", secret_key="refresh_token")
    async with _client(handler) as http:
