...
| ZeroClaw  | Yes (long-polling Bot API, streaming, in-chat model switching, deny-by-default allowlist)                                 | Yes (sync-based via `matrix-sdk` Rust lib, E2EE, no public endpoint needed, crypto state in `~/.zeroclaw/state/matrix/`, compiled via Cargo feature flag)                                            | 17+ total: WhatsApp, Signal, Slack, Discord, iMessage, Mattermost, IRC, Lark, DingTalk, QQ, Nostr, Email, webhook |
| NanoClaw  | Via `/add-telegram` skill (not built-in, must be added)                                                                   | No (MicroClaw fork has it)                                                                                                                                                                           | WhatsApp (native), Slack, Discord, Gmail                                                                          |
| IronClaw  | Yes (native WASM channel, webhook mode, pairing codes)                                                                    | **No** — not natively supported. Available only via managed hosting (ironclaw.app) or OpenClaw bridges.                                                                                              | Slack (WASM channel), HTTP webhooks, REPL/TUI (Ratatui), web gateway                                              |
| PicoClaw  | Yes (long-polling, voice transcription via Groq Whisper)                                                                  | No                                                                                                                                                                                                   | Discord, QQ, DingTalk, LINE, WeCom                                                                                |
| Nanobot   | Yes — first-class, recommended. `python-telegram-bot` long polling. Media groups, typing indicators, voice transcription. | Yes (v0.1.4.post3, 2026-02-25). `matrix-nio` with E2EE, media uploads, typing keepalive, Markdown-to-HTML. Requires optional extras (`nio`, `nh3`, `mistune`). Sync loop as background asyncio task. | Discord, WhatsApp, Slack, Feishu, Email, 8+ total. Cross-channel messaging via `MessageTool`.                     |
| n8n       | Yes (trigger node, but one webhook per bot limit)                                                                         | Yes — built-in Matrix node (send messages, invite users, trigger on events)                                                                                                                          | Slack, Discord, email, SMS, 400+ service triggers                                                                 |
...
| Nanobot   | Yes — built-in cron tool, jobs in `~/.nanobot/data/cron/jobs.json`. Known bug: timezone handling may be ignored (#685).                  | **No webhooks** — open proposal for HTTP server (Flask/FastAPI) but not yet implemented.                                                          | Heartbeat every 30min reads `HEARTBEAT.md` (opt-in, gateway mode only). Subagent spawning.   |
...
| NanoClaw  | **Yes** — native via Agent SDK                     | Built on Anthropic Agent SDK with built-in MCP client. Config in `mcpServers` object in `src/index.ts` or `.mcp.json`. Supports stdio and HTTP transports. Can create custom in-process MCP servers via `createSdkMcpServer`. `/customize` skill for interactive MCP setup.                                                       |
...
| PicoClaw  | **No** — planned (issue #290)                      | Open GitHub issue #290 requests MCP support. Roadmap mentions it to transform PicoClaw into "AIEOS hub." Third-party `picoclaw-mcp-server` exists (LobeHub) but not native client support.                                                                                                                                        |
| Nanobot   | **Yes** — since v0.1.4                             | Native MCP client. Config in `tools.mcpServers` (compatible with Claude Desktop/Cursor format). Supports local (stdio) and remote (HTTP/SSE) servers. Remote supports custom auth headers, configurable `toolTimeout`, and OAuth 2.0 with PKCE (auto on 401). Tools auto-discovered and registered on startup.                    |
| n8n       | **Yes** — native MCP client node                   | MCP Client tool node connects to MCP servers via SSE/streamable HTTP. Can also expose n8n workflows as MCP servers (MCP Server Trigger node). **Limitation**: no stdio transport — stdio-based MCP servers (like Claude Desktop tools) need a proxy like `mcp-remote`.                                                            |
...
| n8n       | **Yes** — Wait node (offloads to DB for waits >65s, frees execution slot), async sub-workflows (Execute Workflow without waiting, callback via `$execution.resumeUrl`), "Async Portal" pattern (pause at checkpoints, external events resume via session ID). Human-in-the-loop approval for AI Agent tool calls. Chat node "send message and wait for response."                    | General — any workflow step                                           | Webhook callback, manual approval node, Wait node resume, polling                                                                                                              |
...
| ZeroClaw  | **Yes**           | Built-in React frontend at `127.0.0.1:3000` via `zeroclaw gateway`. Also: third-party desktop app (ZeroClaw UI) with streaming logs, doctor diagnostics, channel health, chat, config editor for all 29 sections. |
...
| Nanobot   | **No** (official) | CLI + messenger-based only. Third-party community project `nanobot-ui` (SunnyChopper) exists but not maintained by HKUDS.                                                                                         |
