    ...
    async with serve_app(app, port=free_port):
        ...
        async with GateClient(agent_transport(base_url, agent_jwt), message_handler=waiter) as agent:
            ...
            with anyio.fail_after(5.0):
                await waiter.wait_for(agent, key, ActionStatus.DONE)
...
async def test_reject_leaves_action_rejected_and_skips_backend(
    ...
    async with serve_app(app, port=free_port):
        ...
        async with GateClient(agent_transport(base_url, agent_jwt), message_handler=waiter) as agent:
            ...
            with anyio.fail_after(5.0):
                await waiter.wait_for(agent, key, ActionStatus.REJECTED)
...
async def test_auto_approve_predicate_skips_queue(make_gate_app: GateAppFactory, free_port: int, agent_jwt: str):
    ...
    backend, calls = _make_backend()
    app = make_gate_app(backend, predicate=lambda ns, tool, args: Approved())
    base_url = f"http://127.0.0.1:{free_port}"
...
async def test_multi_backend_namespace_isolation(make_gate_app: GateAppFactory, free_port: int, agent_jwt: str):
    ...
    async with serve_app(app, port=free_port):
        waiter = ResourceWaiter()
        async with GateClient(agent_transport(base_url, agent_jwt), message_handler=waiter) as client:
            tools = await client.list_tools()
