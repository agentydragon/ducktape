...
class ResponseRecordModel(BaseModel):
    ...
    Attributes:
        ...
        error: Simple error reason string (format: "{Context}: {ErrorType}").
            For structured error details, see snapshot.error.detail
    """
    ...
    error: str | None = None  # Simple reason like "Upstream HTTP 500"
    created_at: datetime
    updated_at: datetime
...
class FrameListModel(BaseModel):
    """Cursor-based paginated frame list."""
...
@admin_app.get("/api/responses", response_model=PaginatedList[ResponseRecordModel])
async def list_responses(
    limit: int = Query(DEFAULT_LIST_LIMIT, ge=1, le=MAX_LIST_LIMIT),
    offset: int = Query(0, ge=0),
    db: ResponsesDB = Depends(get_db),
) -> PaginatedList[ResponseRecordModel]:
