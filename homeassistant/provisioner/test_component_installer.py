...
def test_install_rejects_wrong_checksum(tmp_path: Path):
    with pytest.raises(ValueError, match="SHA256 mismatch"):
        install_component(tmp_path, b"not the release", _component(sha256="0" * 64))


def test_install_rejects_path_traversal(tmp_path: Path):
    payload = _archive({"../escaped": "bad"})
    component = _component(sha256=hashlib.sha256(payload).hexdigest())

    with pytest.raises(ValueError, match="unsafe archive member"):
        install_component(tmp_path, payload, component)

    assert not (tmp_path.parent / "escaped").exists()


def test_install_replaces_component_and_only_copies_component(tmp_path: Path):
...
def test_install_supports_archive_root(tmp_path: Path):
...
def test_install_rejects_unexpected_manifest(tmp_path: Path):
    payload = _archive(
...
def test_initialize_config_creates_only_missing_files(tmp_path: Path):
    ...
    initialize_component_config(tmp_path, component)
