...
from x.gatelet.server.endpoints.webhook_view import PayloadSummary
from x.gatelet.server.models import AuthCRSession, AuthKey, Base
from x.gatelet.server.security import hash_password
from x.gatelet.server.tests.utils import persist
...
def _patch_get_db_session(monkeypatch, db_session: AsyncSession) -> None:
    ...
    monkeypatch.setattr("x.gatelet.server.database.get_db_session", _override)
    monkeypatch.setattr("x.gatelet.server.app.get_db_session", _override)
    monkeypatch.setattr("x.gatelet.server.lifespan.get_db_session", _override)
...
def _patch_get_settings(monkeypatch, test_settings: Settings) -> None:
    ...
    """
    monkeypatch.setattr("x.gatelet.server.config.get_settings", lambda: test_settings)
    monkeypatch.setattr("x.gatelet.server.lifespan.get_settings", lambda: test_settings)
...
async def _stub_data(monkeypatch):
    ...
    monkeypatch.setattr("x.gatelet.server.lifespan.fetch_states", _states)
    monkeypatch.setattr("x.gatelet.server.lifespan.get_latest_payloads", _payloads)
