...
from x.agent_server.mcp.chat.server import attach_persisted_chat_servers
from x.agent_server.mcp.loop.server import LoopServer
from x.agent_server.mcp.runtime.server import RuntimeServer
from x.agent_server.mcp.ui.server import UiServer
...
from x.agent_server.persist.types import ApprovalOutcome
from x.agent_server.presets import discover_presets
from x.agent_server.runtime.handlers import build_handlers
from x.agent_server.runtime.images import resolve_runtime_image
from x.agent_server.server.bus import ServerBus
from x.agent_server.server.runtime import AgentSession, UiEventHandler
from x.agent_server.server.system_message import get_ui_system_message

if TYPE_CHECKING:
    from mcp_infra.mounted import Mounted
    from x.agent_server.mcp.approval_policy.engine import PolicyProposerServer, PolicyReaderServer
...
class AgentContainer:
    ...
    async def list_mcp_entries(self) -> dict[str, ServerEntry]:
        ...
        """
        if self.compositor_client is None:
            return {}
    ...
    async def _setup_mcp_infrastructure(
        ...
        self.compositor_client = mcp_client
    ...
    async def _handle_actor_msg(self, msg: _ActorMsg) -> ActorResult:
        ...
        match msg:
            case _StartMsg(mcp_config=mcp_cfg):
                ...
                (self.compositor, self.compositor_client, self._notif_buffer) = await self._setup_mcp_infrastructure(
                    self.approval_engine, mcp_cfg
                )

                # Phase 3: Agent runtime
                self.session, self.agent = await self._setup_agent_runtime(
                    self.compositor_client, self._notif_buffer, self.approval_engine
                )
