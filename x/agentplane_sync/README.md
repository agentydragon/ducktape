...
| API conversation authorization and caller shape overrides | Confirmed at Python proxy boundary | Tests reject an unscoped conversation, GET table override, POST table override, and attempt to add another conversation in subset `WHERE`. Real browser shape delivery remains in progress. |
| Real Electric subset snapshot and initial 30-row browser tail | Not demonstrated yet | Earlier RBE runs reached Chromium but exposed two spike wiring defects (a relative URL and forwarding to Electric's root instead of `/v1/shape`). The source fixes are in this draft; the current rerun must prove actual rows and bounded bytes. |
| Browser late update racing an older-page fetch; any-item updates and selective payloads | Not demonstrated yet | Assertions cover it, but the browser has not yet passed initial shape sync. Body chunks are stored separately as append/replace payload parts; shape rows contain per-field revisions and byte counts only. |
| Live head arrivals with older loaded rows retained; scroll anchor preservation | Not demonstrated yet | Browser assertions require the new head to appear without dropping loaded history, and preserve the visible row's pixel offset while adding a page. |
