...
| Electric standalone service  | 1.8.0, Linux amd64 OCI digest in `MODULE.bazel` | [HTTP API](https://electric.ax/docs/sync/api/http), [configuration](https://electric.ax/docs/sync/api/config) |
...
The capacity probe configures `ELECTRIC_MAX_SHAPES=2`, creates three shapes,
waits for the pinned service's periodic eviction cycle, requests the oldest
handle, then verifies that a fresh snapshot returns its complete history. The
current deployment pin is Electric 1.8.1; the repository's pinned test image is
1.8.0, so this is not deployment-version evidence. A successful 409/reset probe
would establish the configured handle lifecycle, not by itself a long-run
server-memory bound.
...
| Configured shape-cap eviction and fresh-handle reset                                | Pending RBE rerun                            | `shape_capacity_test` creates 1k/20k-row histories and a third shape at `ELECTRIC_MAX_SHAPES=2`; it requires the oldest handle to return 409 and a fresh initial snapshot to reload every row. It records cgroup usage and PID 1 RSS/high-water samples at startup, after each shape, after expiry, and after reset.                                         |
