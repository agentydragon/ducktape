...
because its shape parser called `binary_to_float` on that integer. The
parameterized path is the one measured below and is also exercised by the
existing exact-generation/cursor E2E assertions.
