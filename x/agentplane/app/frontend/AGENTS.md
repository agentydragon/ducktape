...
A passing `bbr test //x/agentplane/app/frontend:visual_<scenario>` only proves the scene mounted
...
- **Or run the scenario locally** — `bbr test //x/agentplane/app/frontend:visual_<scenario>
--noremote_accept_cached --nocache_test_results` — and download the PNG it writes to the test's
...
URL param on the harness page (`?raw=1`, `?reasoning=<item-id>`) rather than a simulated click —
check `harness.tsx`'s `PAGES` map before assuming a new state needs click-simulation infrastructure
this app's `visual-test-lib.mjs`-based tests don't have (unlike `haku/console`'s own `render.mjs`).
