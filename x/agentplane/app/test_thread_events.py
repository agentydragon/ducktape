"""What a follower is promised: the frames a Thread produces, and who is allowed to ask.

Driven against the service rather than over a socket. Every promise here is a property of the fold
over the archive -- what replays, what follows it live, where a resumed stream starts, how ingestion
ending differs from a stream breaking -- and none of it is a property of Connect's framing, which is
the library's to get right. The wire a browser actually reads is covered end to end by
//x/agentplane/app:test_thread_browser, in real Chromium against a real app.

What is left over HTTP is the one thing the mount adds and the service cannot answer for: that the
RPC surface is routed at all, and that it is guarded.
"""
...
from x.agentplane.app.thread_events import FOLLOW_EVENTS_PATH, ThreadEventsService
...
# is one the service is not going to produce.
FRAME_S = 20
...
@pytest.fixture
def service(store: TrajectoryStore) -> ThreadEventsService:
    return ThreadEventsService(store, Drain())


def _follow(service: ThreadEventsService, thread_id: UUID, after: int = 0) -> AsyncIterator[FollowEventsResponse]:
    return service.frames(FollowEventsRequest(thread_id=str(thread_id), after_cursor=after))
...
async def test_a_follower_is_told_the_stream_is_open_before_the_thread_produces_anything(
    service: ThreadEventsService, thread_id: UUID
) -> None:
    """Connect withholds response headers until the first message, so until the service says
    something, a stream that opened and a request that stalled look the same to a caller."""
    assert (await _next(_follow(service, thread_id))).WhichOneof("frame") == "heartbeat"


async def test_following_replays_the_archived_prefix_and_then_carries_what_arrives_live(
    service: ThreadEventsService, thread_id: UUID, store: TrajectoryStore, lease: IngestionLease
) -> None:
    stream = _follow(service, thread_id)
    assert (await _next(stream)).WhichOneof("frame") == "heartbeat"
    # Before the entries it bounds: a caller compares this cursor against its own prefix to tell
    # replay from live, which it cannot do if the promise arrives after the replay.
...
async def test_following_resumes_from_the_cursor_the_caller_already_holds(
    ...
    assert await _entries(_follow(service, thread_id, after=2), 1) == [3]
...
async def test_two_followers_of_one_thread_each_read_from_where_their_own_caller_is(
    ...
    first, second = _follow(service, thread_id), _follow(service, thread_id, after=2)
...
async def test_an_ended_feed_ends_the_stream_behind_every_entry_it_committed(
    ...
    frames = await _drained(_follow(service, thread_id))
...
async def test_a_cursor_beyond_the_archived_prefix_is_refused_rather_than_waited_out(
    ...
    with pytest.raises(ConnecpyException) as refused:
        await _entries(_follow(service, thread_id, after=4), 1)
    assert refused.value.code is Code.INVALID_ARGUMENT


async def test_an_unknown_thread_is_not_an_empty_one(service: ThreadEventsService) -> None:
    with pytest.raises(ConnecpyException) as refused:
        await _entries(_follow(service, uuid4()), 1)
...
async def test_the_rpc_mount_is_routed_and_refuses_a_caller_it_cannot_identify(app_url: str) -> None:
    """The two things the mount adds over the service: that this path reaches it at all, and that it
    is behind the check every route is behind. A mount inherits no route dependency, so without this
    the guard could be absent and every other test here would still pass.

    What the stream then carries is the library's framing and is not re-asserted; a browser reads it
    for real in //x/agentplane/app:test_thread_browser.
    """
