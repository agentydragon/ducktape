...
async def test_a_session_is_one_thread_and_its_events_read_back_in_order(
    ...
    events = await store.runner_events(thread, "s-1", limit=100)
    assert [event.sequence for event in events] == [1, 2, 3, 4]
    assert events[1].native.line == '{"type":"x"}'
    assert events[1].at.ToDatetime(tzinfo=UTC) == datetime(2026, 9, 2, 12, 0, 2, tzinfo=UTC)
    assert [event.sequence for event in await store.runner_events(thread, "s-1", after_sequence=2, limit=100)] == [3, 4]
    assert [event.sequence for event in await store.runner_events(thread, "s-1", after_sequence=1, limit=2)] == [2, 3]
    assert await store.last_sequence(thread, "s-1") == 4
    assert await store.last_sequence(other, "s-2") == 0
...
async def test_thread_commands_are_ordered_idempotent_and_reject_payload_reuse(store: TrajectoryStore) -> None:
    ...
    assert (first.thread_id, first.ordinal, first.command, first.replay_cursor) == (thread, 1, command, 1)
    assert await store.request_thread_command(thread, command) == first
    assert await store.thread_commands(thread) == [first]
    # A command is visible immediately after app persistence, before a runner receives it.
    records = await store.thread_records(thread, limit=100)
    assert records.last_replay_cursor == 1
    assert len(records.records) == 1
    assert records.records[0].WhichOneof("record") == "command"
    assert records.records[0].command.ordinal == 1
    assert records.records[0].command.command == command
...
async def test_thread_replay_records_keep_runner_receipt_as_exact_separate_evidence(
    store: TrajectoryStore, lease: IngestionLease
) -> None:
    thread = await store.thread("sb-1", "s-1", SPEC)
    command = pb.Command(command_id="input-1", submit_input=pb.SubmitInput(text="First."))
    accepted = await store.request_thread_command(thread, command)

    await store.record(
        thread, "s-1", [_event(1, command_received=pb.CommandReceived(command_id=command.command_id))], lease=lease
    )

    snapshot = await store.thread_records(thread, limit=100)
    assert [record.replay_cursor for record in snapshot.records] == [accepted.replay_cursor, 2]
    assert snapshot.records[0].command.command == command
    receipt = snapshot.records[1]
    assert receipt.WhichOneof("record") == "runner_event"
    assert receipt.runner_event.runner_session_id == "s-1"
    assert receipt.runner_event.event == _event(1, command_received=pb.CommandReceived(command_id="input-1"))
...
async def test_only_the_first_unreceived_command_is_ready_for_active_runner_delivery(
    ...
    first = await store.request_thread_command(
        thread, pb.Command(command_id="first", submit_input=pb.SubmitInput(text="First."))
    )
    second = await store.request_thread_command(
        thread, pb.Command(command_id="second", submit_input=pb.SubmitInput(text="Second."))
    )
    ...
    await store.record(thread, "s-1", [_event(1, command_received=pb.CommandReceived(command_id="first"))], lease=lease)
...
async def test_a_thread_is_unnamed_until_renamed_and_keeps_its_progress(
    ...
    thread = await store.thread("sb-1", "s-1", SPEC)
    await store.record(thread, "s-1", [_event(1, harness_started=pb.HarnessStarted(pid=1))], lease=lease)
...
async def test_commits_wake_another_replica_and_leave_durable_replay(
    ...
    with replica.changes.subscribe(changed):
        ...
        changed.clear()
        await store.record(thread, "s-1", [_event(1, harness_lost=pb.HarnessLost())], lease=lease)
        await asyncio.wait_for(changed.wait(), timeout=5)
        assert [event.sequence for event in await replica.runner_events(thread, "s-1", limit=10)] == [1]
...
async def test_command_receipts_wake_the_dedicated_cross_replica_outbox_invalidator(
    ...
    with replica.command_changes.subscribe(changed):
        await store.request_thread_command(
            thread, pb.Command(command_id="input-1", submit_input=pb.SubmitInput(text="deliver me"))
        )
        await asyncio.wait_for(changed.wait(), timeout=5)
        changed.clear()
        await store.record(
            thread, "s-1", [_event(1, command_received=pb.CommandReceived(command_id="input-1"))], lease=lease
        )
        await asyncio.wait_for(changed.wait(), timeout=5)
...
async def test_only_current_lease_can_write_or_renew(
    ...
    try:
        ...
        with pytest.raises(IngestionLeaseLostError):
            await store.record(thread, "s-1", [_event(1, harness_lost=pb.HarnessLost())], lease=lease)
        await replica.record(thread, "s-1", [_event(1, harness_lost=pb.HarnessLost())], lease=successor)
        wrong_thread = await store.thread("sb-2", "s-2", SPEC)
        with pytest.raises(IngestionLeaseLostError):
            await replica.record(wrong_thread, "s-2", [_event(2, harness_lost=pb.HarnessLost())], lease=successor)
        assert await store.last_sequence(wrong_thread, "s-2") == 0
...
async def test_ingested_events_project_the_durable_attachment_without_replay_regression(
    ...
    await store.record(
        ...
        [
            ...
            _event(5, command_rejected=pb.CommandRejected(command_id="test-rejected", reason="no")),
    ...
    await store.record(thread, "s-1", [_event(7, harness_started=pb.HarnessStarted())], lease=lease)
    assert await replica.feed_state(thread) == stopped
    with pytest.raises(ValueError, match="older"):
        await store.set_attached(thread, attached, lease=lease)
    assert await replica.feed_state(thread) == stopped
    await store.record(thread, "s-1", [_event(8, harness_started=pb.HarnessStarted(resumed=True))], lease=lease)
    ...
    await store.record(thread, "s-1", [_event(9, harness_lost=pb.HarnessLost())], lease=lease)
...
async def test_historical_catchup_does_not_rewind_an_attachment_snapshot(
    ...
    await store.set_attached(thread, attached, lease=lease)
    await store.end_feed(thread, lease=lease, error=None)
    await store.record(thread, "s-1", [_event(1, harness_started=pb.HarnessStarted())], lease=lease)
    ...
    assert await store.last_sequence(thread, "s-1") == 1
    ...
    try:
        async with engine.begin() as connection:
            ...
            write = asyncio.create_task(
                store.record(thread, "s-1", [_event(1, harness_lost=pb.HarnessLost())], lease=lease)
            )
        ...
        with pytest.raises(IngestionLeaseLostError):
            await write
        assert await store.last_sequence(thread, "s-1") == 0
    finally:
