...
async def test_a_session_is_one_thread_and_its_events_read_back_in_order(
    ...
    # A replay after a reconnect brings entries already stored under their cursors: they are not written twice.
    await store.record(
        thread,
        "s-1",
...
async def test_thread_record_replay_keeps_app_intent_distinct_from_runner_evidence(
    store: TrajectoryStore, lease: IngestionLease
) -> None:
    thread = await store.thread("sb-1", "s-1", SPEC)
    command = command_pb2.Command(command_id="input-1", submit_input=command_pb2.SubmitInput(text="remember this"))

    admitted = await store.request_thread_command(thread, command)
    # Idempotent app admission cannot synthesize a second browser fact.
    assert await store.request_thread_command(thread, command) == admitted
    runner_entry = _event(1, command_admitted=event_pb2.CommandAdmitted(command=command))
    await store.record(thread, "s-1", [runner_entry], lease=lease)
    # Replay after a runner reconnect likewise cannot duplicate copied evidence.
    await store.record(thread, "s-1", [runner_entry], lease=lease)

    records = await store.thread_records(thread, limit=100)

    assert [record.replay_cursor for record in records] == [1, 2]
    assert records[0].thread_id == str(thread)
    assert records[0].command.ordinal == admitted.ordinal
    assert records[0].command.command == command
    assert records[1].runner_event.runner_session_id == "s-1"
    assert records[1].runner_event.entry == runner_entry
    assert await store.thread_records(thread, after_cursor=1, limit=100) == [records[1]]
    assert await store.last_thread_record_cursor(thread) == 2
...
async def test_only_the_first_nonadmitted_command_is_ready_for_active_runner_delivery(
    ...
    await store.record(
        thread, "s-1", [_event(1, command_admitted=event_pb2.CommandAdmitted(command=first.command))], lease=lease
    )
...
async def test_a_thread_is_unnamed_until_renamed_and_keeps_its_progress(
    ...
    thread = await store.thread("sb-1", "s-1", SPEC)
    await store.record(thread, "s-1", [_event(1, harness_started=event_pb2.HarnessStarted(pid=1))], lease=lease)
...
async def test_commits_wake_another_replica_and_leave_durable_replay(
    ...
    with replica.changes.subscribe(changed):
        ...
        changed.clear()
        await store.record(thread, "s-1", [_event(1, harness_lost=event_pb2.HarnessLost())], lease=lease)
        await asyncio.wait_for(changed.wait(), timeout=5)
...
async def test_only_current_lease_can_write_or_renew(
    ...
    try:
        ...
        with pytest.raises(IngestionLeaseLostError):
            await store.record(thread, "s-1", [_event(1, harness_lost=event_pb2.HarnessLost())], lease=lease)
        await replica.record(thread, "s-1", [_event(1, harness_lost=event_pb2.HarnessLost())], lease=successor)
        wrong_thread = await store.thread("sb-2", "s-2", SPEC)
        with pytest.raises(IngestionLeaseLostError):
            await replica.record(
                wrong_thread, "s-2", [_event(2, harness_lost=event_pb2.HarnessLost())], lease=successor
            )
...
async def test_ingested_events_project_the_durable_attachment_without_replay_regression(
    ...
    await store.record(thread, "s-1", [_event(7, harness_started=event_pb2.HarnessStarted())], lease=lease)
    assert await replica.feed_state(thread) == stopped
    with pytest.raises(ValueError, match="older"):
        await store.set_attached(thread, attached, lease=lease)
    assert await replica.feed_state(thread) == stopped
    await store.record(thread, "s-1", [_event(8, harness_started=event_pb2.HarnessStarted(resumed=True))], lease=lease)
    ...
    await store.record(thread, "s-1", [_event(9, harness_lost=event_pb2.HarnessLost())], lease=lease)
...
async def test_historical_catchup_does_not_rewind_an_attachment_snapshot(
    ...
    await store.set_attached(thread, attached, lease=lease)
    await store.end_feed(thread, lease=lease, error=None)
    await store.record(thread, "s-1", [_event(1, harness_started=event_pb2.HarnessStarted())], lease=lease)
    ...
    try:
        async with engine.begin() as connection:
            ...
            write = asyncio.create_task(
                store.record(thread, "s-1", [_event(1, harness_lost=event_pb2.HarnessLost())], lease=lease)
            )
            async with asyncio.timeout(5):
                while True:
