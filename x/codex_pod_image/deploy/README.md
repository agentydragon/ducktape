...
**Retired, unwired from Flux** (kept in this source-owned `deploy/` directory; it has
no entry in the central Flux Kustomization chart). Its Deployment had no OVH nodeSelector,
so the scheduler could place it on non-OVH nodes (e.g. `wyrm2`) where the SeaweedFS CSI
driver isn't present, and its `codex-workspace` PVC then failed to attach — the Deployment
sat `ProgressDeadlineExceeded` rather than being fixed. `agent-sandbox`'s
