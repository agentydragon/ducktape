...
def _find_wt_cli_binary() -> str | None:
    ...
    # The path structure is: runfiles/_main/x/wt/wt_cli
    candidate = Path(runfiles_dir) / "_main" / "x" / "wt" / "wt_cli"
    if candidate.exists():
...
def _generate_wt_function_for_binary(binary_path: str) -> str:
    ...
    with importlib.resources.files("x.wt.shell").joinpath("wt.sh").open("r", encoding="utf-8") as f:
        tpl = f.read()
    # The template uses __PY__ -m x.wt.cli sh, replace with direct binary call
    return tpl.replace("__PY__ -m x.wt.cli sh", quoted_path + " sh")
...
def shell_runner(tmp_path: Path):
    ...
    class ShellRunner:
        def run_script(self, script_content: str, *, cwd: Path, env: dict[str, str] | None = None):
            ...
            assert importlib.util.find_spec("x.wt"), "wt package not installed - required for shell integration tests"
            ...
            if not wt_cli_path:
                raise RuntimeError(
                    "wt_cli binary not found in runfiles. Ensure //x/wt:wt_cli is in the test's data dependencies."
                )
