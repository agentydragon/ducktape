...
class HookHandler:
    ...
    def _log_decision(self, session_id: SessionID, decision_point: str, details: dict[str, Any]) -> None:
        ...
        log_entry = {"timestamp": datetime.now(tz=UTC).isoformat(), "decision_point": decision_point, "details": details}
    ...
    def _check_access_control(self, request: PreToolUseRequest, session_id: SessionID) -> tuple[RuleAction, str | None]:
        ...
        context = PredicateContext(tool=tool_call.tool_name, args=args, session_id=session_id, timestamp=datetime.now(tz=UTC))
