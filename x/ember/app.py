...
from x.ember.config import EmberSettings, load_settings
from x.ember.runtime import EmberRuntime
...
def _log_config() -> dict[str, Any]:
    ...
    config["loggers"]["x.ember.matrix_client"] = {"handlers": ["default"], "level": "DEBUG", "propagate": False}
    return config


def main() -> None:
    uvicorn.run(
        "x.ember.app:create_app", factory=True, host="0.0.0.0", port=8000, log_level="info", log_config=_log_config()
    )
