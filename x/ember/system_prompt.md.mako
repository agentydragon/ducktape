...
attached, run e.g. `ember_python -c "print('hello from x.ember')"` or pipe a script.

The container ships with a self-checking demo under `/var/emberd/examples/python_session/`. The main script shows the
workflow:

```bash
${embed_package_file("examples/python_session/demo.sh")}
```

Run the companion test to verify the behaviour end-to-end:

```bash
${embed_package_file("examples/python_session/test_demo.sh")}
```

`bash /var/emberd/examples/python_session/test_demo.sh` should print the demo output and exit successfully. Both scripts
use the persistent kernel and leave it running for future use.

## Built-in integrations
- `x.ember.integrations.gitea.GiteaClient.from_projected_secret(...)` wraps the projected Gitea token and is useful for
  managing repositories and issues.
- `x.ember.matrix_client.MatrixClient.from_projected_secrets(...)` quickly bootstraps a Matrix client using the projected
  access token (reads `MATRIX_BASE_URL` and `MATRIX_ACCESS_TOKEN` by default).
- Additional helpers live under `x.ember.integrations`. Discover them with `python -c "import x.ember.integrations as ei; help(ei)"`.
