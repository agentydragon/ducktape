...
Python BUILD files are gazelle-managed and CI fails on drift. After adding, moving, or
renaming a `.py` file or changing imports, run gazelle rather than hand-editing managed
`srcs`/`deps`:

```bash
bb run //devinfra:gazelle              # Update BUILD files
bb run //devinfra:gazelle -- --mode=diff  # Preview changes (the CI gate)
```
