...
from hack.gatelet.server.endpoints.webhook_view import PayloadSummary
from hack.gatelet.server.models import AuthCRSession, AuthKey, Base
from hack.gatelet.server.security import hash_password
from hack.gatelet.server.tests.utils import persist
...
def _patch_get_db_session(monkeypatch, db_session: AsyncSession) -> None:
    ...
    monkeypatch.setattr("hack.gatelet.server.database.get_db_session", _override)
    monkeypatch.setattr("hack.gatelet.server.app.get_db_session", _override)
    monkeypatch.setattr("hack.gatelet.server.lifespan.get_db_session", _override)
...
def _patch_get_settings(monkeypatch, test_settings: Settings) -> None:
    ...
    """
    monkeypatch.setattr("hack.gatelet.server.config.get_settings", lambda: test_settings)
    monkeypatch.setattr("hack.gatelet.server.lifespan.get_settings", lambda: test_settings)
...
async def _stub_data(monkeypatch):
    ...
    monkeypatch.setattr("hack.gatelet.server.endpoints.homeassistant.fetch_states", _states)
    monkeypatch.setattr("hack.gatelet.server.endpoints.webhook_view.get_latest_payloads", _payloads)
    # Also patch in lifespan where these are imported for the authenticated_root_handler
    monkeypatch.setattr("hack.gatelet.server.lifespan.fetch_states", _states)
    monkeypatch.setattr("hack.gatelet.server.lifespan.get_latest_payloads", _payloads)
