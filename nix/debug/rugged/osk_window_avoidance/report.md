...
- A helper script exists at `nix/debug/rugged/osk_window_avoidance/start_nested_plasma.sh`.
...
- Run:
  ...
  ```bash
  nix/debug/rugged/osk_window_avoidance/start_nested_plasma.sh
  ```
...
- Disable the automatic KWrite launch, if needed, with:
  ...
  ```bash
  NESTED_PLASMA_TEST_APP= nix/debug/rugged/osk_window_avoidance/start_nested_plasma.sh
  ```
