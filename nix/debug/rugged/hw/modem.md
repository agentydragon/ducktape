...
`nix/debug/rugged/modem.sh try-5g`): switched modem from `allowed: 4g;
...
`nix/debug/rugged/modem.sh` is the single entry point for diagnostics and
...
- **Re-verify the Google Fi QoS throttle is gone** — spot-check on
  ...
  measurement isn't conclusive. Run `nix/debug/rugged/modem.sh diagnose`,
