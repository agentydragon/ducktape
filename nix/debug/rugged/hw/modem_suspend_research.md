...
- `nix/debug/rugged/modem.sh dump` — snapshot everything to
  `nix/debug/rugged/hw/suspend_research/snapshots/<TS>/`: full dmesg,
  ...
  the failure trace.
- `nix/debug/rugged/modem.sh status` — fast read-only triage; surfaces
...
- `nix/debug/rugged/hw/suspend_research/flip_wwan_autosense.sh` — toggle
...
- This document: `nix/debug/rugged/hw/modem_suspend_research.md`
- Pointer in `nix/debug/rugged/hw/modem.md` "Where to find what" table.
- Cross-reference in `nix/debug/rugged/hw/foxflss_wwan.md` §SBL-stage
  firmware wedge.
- Capture script: `nix/debug/rugged/modem.sh` (`dump`, `status`, `recover`).
- BIOS toggle script: `nix/debug/rugged/hw/suspend_research/flip_wwan_autosense.sh`.
- Snapshots: `nix/debug/rugged/hw/suspend_research/snapshots/<TS>/`.
- Captured upstream sources: `nix/debug/rugged/hw/suspend_research/upstream_*.c`.
